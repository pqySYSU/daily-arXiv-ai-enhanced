...
TL;DR: This paper introduces a method for accurate monocular depth estimation in robotic surgery by adapting synthetic priors from Depth Anything V2 to medical domains using DV-LORA, achieving state-of-the-art performance on challenging endoscopic scenes.


<details>
  <summary>Details</summary>
Motivation: Monocular depth estimation in robotic surgery is fragile in specular, fluid-filled endoscopic environments. Existing self-supervised methods suffer from boundary collapse on thin surgical tools and transparent surfaces due to reliance on noisy real-world pseudo-labels.

Method: Leverages high-fidelity synthetic priors from Depth Anything V2 architecture, which captures precise geometric details of thin structures. Adapts these priors to medical domain using Dynamic Vector Low-Rank Adaptation (DV-LORA) to minimize parameter budget while bridging synthetic-to-real gap. Introduces physically-stratified evaluation protocol on SCARED dataset.

Result: Establishes new state-of-the-art with accuracy (< 1.25) of 98.1% and reduces Squared Relative Error by over 17% compared to established baselines. Demonstrates superior robustness in adverse surgical lighting conditions.

Conclusion: The approach successfully addresses challenges in surgical depth estimation by combining synthetic priors with efficient domain adaptation, achieving significant performance improvements in challenging endoscopic environments with specular surfaces and thin structures.
...
Method: Computer vision models extract 2D skeletons, gaze vectors, and movement trajectories from training videos. Task-specific metrics measure psychomotor fluency, situational awareness, and team coordination. These feed into an extended Cognitive Task Analysis hierarchy using weighted combinations to generate overall performance scores.

Result: Demonstrated with real-world Enter and Clear the Room drills, providing actionable domain-specific metrics capturing individual and team performance. The approach supports After Action Reviews with interactive dashboards in Gamemaster and GIFT systems.

Conclusion: Video-based assessment enables scalable evaluation in Synthetic Training Environments without additional hardware. Limitations include tracking difficulties and ground-truth validation. Future work includes expanding to 3D video data and leveraging video analysis for broader STE applications.
...
TL;DR: PFP is a neural network architecture that compresses long videos into short contexts while preserving high-frequency details of individual frames, enabling efficient long-term memory for autoregressive video models.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of efficiently compressing long videos into manageable contexts for autoregressive video models while maintaining perceptual quality of individual frames at arbitrary temporal positions.

Method: PFP uses a neural network structure with explicit pretraining objective to preserve high-frequency details of single frames. It compresses 20-second videos into ~5k length contexts and can be fine-tuned as memory encoders for autoregressive video models.

Result: The framework enables long history memory with low context cost and relatively low fidelity loss. Random frames can be retrieved with perceptually preserved appearances from compressed contexts.

Conclusion: PFP provides an effective solution for video compression that balances context length reduction with perceptual quality preservation, making it suitable for memory-efficient autoregressive video modeling applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing domain adaptation methods for 3D HPE overlook non-stationary target pose datasets and suffer from catastrophic forgetting when adapting to multiple domains sequentially without access to previous data.

Method: Introduces lifelong domain adaptive 3D HPE with a GAN framework containing 3D pose generators, 2D pose discriminator, and 3D pose estimator. Uses pose-aware, temporal-aware, and domain-aware knowledge integration to enhance adaptation and reduce forgetting.
...
Conclusion: First to introduce lifelong domain adaptation to 3D HPE, successfully addressing challenges of adapting to current domains while preserving knowledge from previous domains through innovative GAN framework and knowledge integration.
...
TL;DR: Deep learning pipeline transforms pediatric T1-weighted MRIs into synthetic CTs for cranial bone segmentation and suture analysis, enabling non-invasive pediatric cranial evaluation without radiation exposure.


<details>
  <summary>Details</summary>
Motivation: CT scans provide detailed cranial bone and suture information but involve harmful ionizing radiation for children. MRI is radiation-free but cannot visualize cranial sutures or assess bone density. There's a critical need for non-invasive methods to evaluate pediatric cranial development and suture ossification.

Method: Proposed deep learning pipeline transforms T1-weighted MRIs of children (0.2-2 years) into synthetic CTs (sCTs). The method predicts detailed cranial bone segmentation, generates suture probability heatmaps, and derives direct suture segmentation from these heatmaps. Uses robust, domain-specific variational autoencoders to generate perceptually indistinguishable cranial sCTs from routine pediatric MRIs.
...
Conclusion: This is the first pediatric cranial CT synthesis framework enabling suture segmentation on sCTs derived from MRI, despite MRI's limited bone depiction. The method bridges critical gaps in non-invasive cranial evaluation by generating perceptually indistinguishable cranial sCTs from routine pediatric MRIs without radiation exposure.
...
TL;DR: The paper addresses bias in Noise2Noise training when applying nonlinear functions to noisy targets, identifying specific nonlinearities that minimize bias and enabling effective denoising of HDR images without clean reference data.
...
Method: Developed a theoretical framework to analyze effects of nonlinearities on Noise2Noise training, identifying a class of nonlinear functions with minimal bias. Applied this to HDR image denoising by combining specific loss functions with tone mapping functions that reduce dynamic range while introducing minimal bias.

Result: Demonstrated the method on Monte Carlo rendered HDR images, showing that certain combinations of loss functions and tone mapping functions can effectively handle outliers while maintaining performance comparable to training with clean reference images.

Conclusion: The approach enables Noise2Noise training with certain nonlinear preprocessing, making it practical for HDR image denoising applications where clean targets are unavailable, achieving results approaching those of methods trained with clean reference data.
...
TL;DR: This paper explores scaling laws for training foundation models on massive-scale electro-optical satellite data, finding that even at petascale, performance remains data-limited rather than model-limited.


<details>
  <summary>Details</summary>
Motivation: The motivation is to establish practical scaling techniques for foundation models in remote sensing domains where internet-scale data isn't available, unlike natural-image domains where scaling laws are well-understood. Current multimodal ML applications need robust domain-specialized encoders for non-text modalities like remote sensing.

Method: The researchers used over a quadrillion pixels of commercial satellite EO data and the MITRE Federal AI Sandbox to train progressively larger vision transformer (ViT) backbones. They analyzed scaling behaviors and reported success/failure modes observed at petascale.

Result: Key finding: even at petascale (quadrillion pixels), performance remains consistent with a data-limited regime rather than a model parameter-limited one. The study provides practical insights into bridging domain gaps across additional remote sensing modalities.

Conclusion: The practical insights from this scaling study should inform data-collection strategies, compute budgets, and optimization schedules to advance the development of frontier-scale remote sensing foundation models.
...
TL;DR: Proposes a bi-level optimization framework for automated fetal ultrasound skill assessment without manual skill ratings, using task performance on acquired images as skill indicator.


<details>
  <summary>Details</summary>
Motivation: Traditional ultrasound skill assessment relies on expert supervision which is subjective and time-intensive. Supervised learning methods are limited by predetermined factors and manual skill ratings.

Method: Novel bi-level optimization framework with clinical task predictor and skill predictor optimized jointly. Networks are refined simultaneously to assess skills based on how well tasks are performed on acquired fetal ultrasound images.

Result: Validated on real-world clinical ultrasound videos of fetal head scanning. Results demonstrate feasibility of predicting ultrasound skills by quantifying optimized task performance as skill indicator.

Conclusion: The framework successfully assesses fetal ultrasound skills without manual skill ratings, using task performance on images as an objective skill indicator, addressing limitations of traditional assessment methods.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal MRI data is crucial for brain tumor segmentation but often incomplete in clinical practice, creating challenges for fully utilizing available information and maximizing incomplete multimodal data utilization.

Method: Two-component framework: 1) Meta-parameterized adaptive modality fusion (Meta-AMF) that integrates information from multiple modalities under varying input conditions and generates adaptive soft-label supervision, and 2) Consistency regularization module that enhances segmentation performance and robustness.
...
Conclusion: The MGML framework effectively handles incomplete multimodal MRI data without altering original model architecture, can be integrated into training pipelines for end-to-end optimization, and demonstrates strong performance in brain tumor segmentation with missing modalities.
...
TL;DR: A novel cross-view geo-localization system using DINOv2 backbone with convolution adapter, multi-scale channel reallocation, and MoE-enhanced aggregation to handle view-point discrepancies.
...
Conclusion: The proposed CVGL system effectively addresses view-point discrepancy challenges through adaptive backbone fine-tuning, enhanced spatial representation, and dynamic feature aggregation, achieving strong performance with parameter efficiency.
...
TL;DR: AI-driven framework for automated action segmentation and performance assessment in microanastomosis procedures using computer vision and machine learning for objective surgical skill evaluation.
...
Method: Three-component AI framework: (1) YOLO and DeepSORT for instrument tip tracking, (2) self-similarity matrix for action boundary detection with unsupervised clustering, (3) supervised classification for surgical gesture proficiency evaluation, designed for edge computing.
...
TL;DR: DehazeSNN is a novel image dehazing method that combines U-Net architecture with Spiking Neural Networks to overcome limitations of CNNs and Transformers, achieving state-of-the-art performance with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Image dehazing is crucial for improving image clarity in hazy conditions. Traditional methods use atmospheric scattering models, while recent deep learning approaches (CNNs and Transformers) have limitations: CNNs struggle with long-range dependencies, and Transformers require heavy computational resources.

Method: Propose DehazeSNN, an innovative architecture integrating U-Net-like design with Spiking Neural Networks (SNNs). It captures multi-scale image features while managing local and long-range dependencies efficiently. Introduces Orthogonal Leaky-Integrate-and-Fire Block (OLIFBlock) to enhance cross-channel communication.

Result: Extensive experiments show DehazeSNN is highly competitive with state-of-the-art methods on benchmark datasets. It delivers high-quality haze-free images with smaller model size and fewer multiply-accumulate operations (lower computational burden).

Conclusion: DehazeSNN provides an effective solution for image dehazing that balances performance and computational efficiency, overcoming limitations of existing CNN and Transformer-based approaches. The method is publicly available on GitHub.
...
TL;DR: T2VAttack: A comprehensive study of adversarial attacks on Text-to-Video diffusion models from semantic and temporal perspectives, revealing vulnerabilities to minor prompt modifications.
...
Method: Proposes two attack objectives (semantic for video-text alignment, temporal for temporal dynamics) and two attack methods: T2VAttack-S (replaces critical words with synonyms via greedy search) and T2VAttack-I (iteratively inserts optimized words with minimal perturbation).

Result: Experiments on state-of-the-art T2V models (ModelScope, CogVideoX, Open-Sora, HunyuanVideo) show that minor prompt modifications (single word substitution/insertion) cause substantial degradation in semantic fidelity and temporal dynamics.
...
TL;DR: A novel view extrapolation method for autonomous driving that uses only images and optional camera poses, combining 4D Gaussian reconstruction with iterative video diffusion refinement to generate high-quality novel viewpoint images without expensive sensors or annotations.


<details>
  <summary>Details</summary>
Motivation: Current view extrapolation methods rely on expensive sensors (LiDAR) or labor-intensive annotations (3D bounding boxes, lane markings), limiting real-world deployment. The authors aim to develop a solution using only images and optional camera poses to overcome these limitations.

Method: 1) Estimate global static and per-frame dynamic point clouds from images/camera poses; 2) Fuse into unified representation; 3) Use deformable 4D Gaussian framework for scene reconstruction; 4) Train video diffusion model on degraded renders from initial 4D Gaussian model; 5) Iteratively refine progressively shifted Gaussian renderings with diffusion model; 6) Incorporate enhanced results back into 4DGS training; 7) Repeat until reaching target extrapolated viewpoints.

Result: The method produces higher-quality images at novel extrapolated viewpoints compared to baseline approaches, demonstrating effectiveness without requiring expensive sensors or annotations.

Conclusion: The proposed approach successfully enables high-quality view extrapolation in autonomous driving scenarios using only images and optional camera poses, overcoming the limitations of methods that require expensive sensors or labor-intensive annotations, making it more applicable for real-world deployment.
...
TL;DR: Deep learning inpainting model uses satellite time series to detect surface changes with 3x better sensitivity than traditional methods, validated on earthquake ruptures.


<details>
  <summary>Details</summary>
Motivation: Surface change detection from satellites is challenging due to atmospheric noise, seasonal variations, and sensor artifacts, but critical for disaster response and environmental monitoring.

Method: Train an inpainting model based on SATLAS foundation model to reconstruct the last frame of Sentinel-2 time series from preceding acquisitions, using global training data across diverse climates and land covers.

Result: The method detects earthquake-triggered surface ruptures from the 2023 Turkey-Syria earthquake with higher sensitivity and specificity than temporal median or Reed-Xiaoli anomaly detectors, achieving 3x lower detection thresholds.

Conclusion: Deep learning-based anomaly detection using temporal redundancy in satellite time series enables automated, global-scale monitoring of surface changes from freely available multi-spectral data.
...
TL;DR: GCA-ResUNet: A lightweight CNN-based medical image segmentation framework with Grouped Coordinate Attention module that improves global context modeling while maintaining computational efficiency, outperforming both CNN and Transformer methods on multi-organ segmentation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current U-Net based CNNs struggle with long-range dependencies in multi-organ segmentation and low-contrast regions due to local receptive fields, while Transformers require excessive computational resources that hinder clinical deployment in resource-constrained environments.

Method: Proposes GCA-ResUNet with a plug-and-play Grouped Coordinate Attention module that: 1) decouples channel-wise context modeling into multiple groups to handle semantic heterogeneity, and 2) integrates direction-aware coordinate encoding to capture structured spatial dependencies along horizontal and vertical axes.

Result: Achieves Dice scores of 86.11% on Synapse and 92.64% on ACDC benchmarks, outperforming representative CNN and Transformer methods including Swin-UNet and TransUNet. Shows consistent improvements in delineating small anatomical structures with complex boundaries.

Conclusion: GCA-ResUNet provides an optimal trade-off between segmentation accuracy and computational efficiency, offering a practical and scalable solution for clinical deployment in resource-constrained environments while maintaining strong performance on challenging multi-organ segmentation tasks.
...
TL;DR: GASeg is a self-supervised semantic segmentation framework that addresses appearance ambiguities by incorporating geometric topological information through a Differentiable Box-Counting module, Topological Augmentation, and a multi-objective loss for cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Self-supervised semantic segmentation methods often fail due to over-reliance on unstable appearance-based features like shadows, glare, and local textures. The authors argue that incorporating stable topological information can bridge appearance and geometry to handle these ambiguities.

Method: The core method includes: 1) Differentiable Box-Counting (DBC) module that quantifies multi-scale topological statistics from geometric-based and appearance-based feature streams; 2) Topological Augmentation (TopoAug) - an adversarial strategy using morphological operators to simulate real-world ambiguities; 3) GALoss - a multi-objective loss that enforces cross-modal alignment between geometric and appearance features.
...
Conclusion: The proposed GASeg framework successfully addresses appearance ambiguities in self-supervised semantic segmentation by leveraging stable topological information through the DBC module, TopoAug, and GALoss, achieving superior performance across multiple benchmarks.
...
TL;DR: A pipeline for 3D reconstruction of spacecraft using 3D Gaussian Splatting with sun position priors to handle dynamic space lighting for improved photometric accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional 3DGS requires static scenes, but spaceborne imagery during RPO has dynamic lighting conditions. Accurate photometric quality is crucial for downstream pose estimation tasks.

Method: Uses 3D Gaussian Splatting with sun position priors from servicer spacecraft to adapt to changing illumination, handling global shadowing and self-occlusion in space imagery.

Result: Experimental studies show the method effectively learns to adapt to rapidly changing space illumination and reflects accurate shadowing/occlusion in reconstructed 3D models.

Conclusion: Incorporating sun position priors improves 3DGS photometric quality for space applications, enabling accurate 3D reconstruction and pose estimation during RPO missions.
...
TL;DR: CVC (Conditioned Velocity Correction) is a flow-based diffusion editing framework that corrects velocity errors in latent trajectories to improve semantic consistency and structural fidelity during image distribution transformations.


<details>
  <summary>Details</summary>
Motivation: Current flow-based diffusion editing methods suffer from accumulated velocity errors in latent trajectories, leading to semantic inconsistency and loss of structural fidelity when transforming between source and target image distributions without explicit inversion.

Method: CVC introduces a dual-perspective velocity conversion mechanism that decomposes latent evolution into: 1) a structure-preserving branch consistent with source trajectory, and 2) a semantically-guided branch for controlled deviation toward target distribution. It applies posterior-consistent updates derived from Empirical Bayes Inference and Tweedie correction to compensate for velocity errors and maintain fidelity to the true flow.

Result: CVC achieves stable and interpretable latent dynamics with faithful reconstruction and smooth local semantic conversion. Comprehensive experiments show superior fidelity, better semantic alignment, and more reliable editing behavior across diverse tasks compared to existing methods.

Conclusion: CVC provides a principled framework for flow-based editing that addresses velocity error accumulation through conditional velocity correction, resulting in more consistent and faithful image distribution transformations.
...
<details>
  <summary>Details</summary>
Motivation: Current VTON methods focus on garment detail rendering but neglect garment fit - how garments align with the body - which is fundamental to fashion design and holistic style coordination.

Method: FitControler features: 1) fit-aware layout generator that redraws body-garment layouts using garment-agnostic representations, and 2) multi-scale fit injector to deliver layout cues for layout-driven VTON. Built on Fit4Men dataset with 13,000 body-garment pairs of different fits.

Result: Extensive experiments show FitControler works with various VTON models and achieves accurate fit control. Introduced two fit consistency metrics to assess fitness of generations.

Conclusion: FitControler successfully addresses the gap in fit-aware VTON, enabling customized garment fit control through learnable layout generation and injection, validated by comprehensive dataset and metrics.
...
Method: Three key components: 1) Structure-guided initialization assigning Gaussians based on spatial structural priors; 2) Adaptive bitwidth quantization of covariance parameters (higher precision for complex regions); 3) Geometry-consistent regularization aligning Gaussian orientations with local gradient directions.

Result: Substantially improves both representational power and RD performance while maintaining over 1000 FPS decoding. Reduces BD-rate by 43.44% on Kodak and 29.91% on DIV2K compared to baseline GSImage.

Conclusion: The proposed structure-guided allocation principle effectively couples image structure with representation capacity and quantization precision, enabling significant RD improvements in 2D Gaussian Splatting while preserving native decoding speed.
...
TL;DR: MF-RSVLM is a multi-feature fusion vision-language model for remote sensing that addresses visual forgetting and fine-grained feature extraction challenges by combining multi-scale visual representations with recurrent visual feature injection.
...
Method: MF-RSVLM uses multi-scale visual feature extraction to capture both global context and local details, combined with a recurrent visual feature injection scheme to maintain visual grounding and reduce forgetting during language generation.
...
Conclusion: MF-RSVLM effectively addresses remote sensing vision-language challenges through multi-feature fusion and recurrent visual injection, achieving strong performance across multiple RS tasks with publicly available code.
...
TL;DR: RSAgent is an agentic MLLM that performs text-guided object segmentation through multi-turn tool interactions, achieving state-of-the-art performance by iteratively refining masks based on visual feedback.


<details>
  <summary>Details</summary>
Motivation: Current text-guided segmentation methods treat it as one-shot grounding, which limits verification, refocusing, and refinement when initial localization is wrong. There's a need for iterative approaches that can reason and revise spatial hypotheses.
...
Conclusion: RSAgent's agentic approach with multi-turn reasoning and tool interactions effectively addresses limitations of one-shot segmentation methods, enabling iterative refinement and achieving superior segmentation performance across diverse benchmarks.
...
TL;DR: PipeFlow is a scalable video editing method that uses motion analysis to skip low-motion frames, pipelined scheduling for parallel processing, and neural interpolation to handle long videos efficiently.
...
Method: Three key innovations: 1) Motion analysis using SSIM and Optical Flow to skip editing of low-motion frames, 2) Pipelined task scheduling that splits videos into segments for parallel DDIM inversion and joint editing, 3) Neural network-based interpolation to smooth segment borders and interpolate skipped frames.

Result: PipeFlow achieves linear scaling with video length, enabling editing of infinitely long videos without growing per-frame overhead. It achieves up to 9.6X speedup over TokenFlow and 31.7X speedup over DMT.
...
<details>
  <summary>Details</summary>
Motivation: Traditional anisotropic diffusion methods use explicit diffusion operators that are not well adapted to complex image structures, limiting their performance compared to learning-based approaches. The authors aim to develop a trainable diffusion framework that combines the strengths of diffusion methods with learning-based adaptability.

Method: The authors propose a trainable anisotropic diffusion framework based on reinforcement learning. They model the denoising process as a series of naive diffusion actions, with the order of actions learned through deep Q-learning. This creates a stochastic anisotropic diffusion process that adapts to different image structures.

Result: The proposed denoiser was applied to three types of common noise and outperformed existing diffusion-based methods. It also competes with representative deep CNN-based methods, showing competitive performance with learning-based approaches.

Conclusion: The reinforcement learning-based anisotropic diffusion framework successfully combines the strengths of traditional diffusion methods with learning-based adaptability, creating a more effective denoiser that bridges the gap between classical and modern approaches.
...
TL;DR: NIRNL is a robust cross-modal retrieval framework that addresses noisy labels through neighbor-aware instance refining and cross-modal margin preserving, achieving SOTA performance under high noise rates.


<details>
  <summary>Details</summary>
Motivation: Multi-modal data annotation is time-consuming and often contains noise, which degrades retrieval performance. Existing robust CMR methods fail to simultaneously satisfy model performance ceilings, calibration reliability, and data utilization rates.

Method: Proposes Neighbor-aware Instance Refining with Noisy Labels (NIRNL) framework: 1) Cross-modal Margin Preserving (CMP) adjusts relative distance between positive/negative pairs to enhance discrimination; 2) Neighbor-aware Instance Refining (NIR) identifies pure, hard, and noisy subsets through cross-modal neighborhood consensus; 3) Tailored optimization strategies for each subset to maximize data utilization while mitigating error propagation.

Result: Extensive experiments on three benchmark datasets demonstrate NIRNL achieves state-of-the-art performance with remarkable robustness, especially under high noise rates.

Conclusion: NIRNL effectively addresses noisy label problems in cross-modal retrieval by combining cross-modal margin preserving with neighbor-aware instance refining and tailored optimization strategies, outperforming existing methods.
...
TL;DR: PCRNet is a novel deep neural network for ocular disease recognition that incorporates pathology context and expert experience priors through two modules (PRM and EPGA) plus an integrated loss function, achieving superior performance over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current deep neural networks for ocular disease diagnosis often ignore clinical pathology context and expert experience priors, which are crucial for improving recognition performance and decision-making interpretability in medical applications.

Method: Developed Pathology Recalibration Module (PRM) with pixel-wise context compression and pathology distribution concentration operators; created Expert Prior Guidance Adapter (EPGA) to highlight significant pixel-wise regions; integrated these into PCRNet architecture; introduced Integrated Loss (IL) considering sample-wise loss distributions and training label frequencies.

Result: Extensive experiments on three ocular disease datasets demonstrate PCRNet with IL outperforms state-of-the-art attention-based networks and advanced loss methods; visualization analysis explains how PRM and EPGA affect DNN decision-making.

Conclusion: PCRNet effectively incorporates pathology context and expert experience priors to improve ocular disease recognition performance and interpretability, providing a more clinically relevant deep learning approach for medical diagnosis.
...
TL;DR: Proposes balanced hierarchical contrastive loss with decoupled learning in DETR for fine-grained remote sensing object detection, addressing data imbalance and localization interference issues.


<details>
  <summary>Details</summary>
Motivation: Fine-grained remote sensing datasets use hierarchical labels, but embedding semantic hierarchy into representation learning is challenging. Previous methods using supervised contrastive learning overlook data imbalance across hierarchy and interference with localization.

Method: Balanced hierarchical contrastive loss with learnable class prototypes and gradient equilibration, combined with decoupled learning strategy separating DETR's object queries into classification and localization sets.

Result: Outperforms state-of-the-art approaches on three fine-grained datasets with hierarchical annotations.

Conclusion: The proposed method effectively addresses data imbalance and localization interference in hierarchical fine-grained object detection, improving performance through balanced contrastive learning and task-decoupled optimization.
...
TL;DR: RainFusion2.0 proposes an online adaptive, hardware-efficient sparse attention mechanism to accelerate Diffusion Transformer models for video/image generation, achieving 80% sparsity with 1.5-1.8x speedup across diverse hardware platforms.


<details>
  <summary>Details</summary>
Motivation: DiT models for video/image generation have extremely high computational costs due to attention mechanisms, limiting practical applications. Existing sparse attention methods have overhead issues and lack hardware generality, being mostly GPU-focused despite increasing adoption of diverse hardware like ASICs.

Method: RainFusion2.0 introduces: (1) block-wise mean values as representative tokens for sparse mask prediction, (2) spatiotemporal-aware token permutation, and (3) a first-frame sink mechanism specifically for video generation scenarios.
...
Conclusion: RainFusion2.0 successfully addresses computational cost challenges in DiT models through an online adaptive, hardware-efficient sparse attention mechanism that works across multiple hardware platforms while maintaining quality.
...
TL;DR: D²VLM is a video-language model framework that factorizes temporal grounding and textual response learning using evidence tokens and factorized preference optimization for improved event-level video understanding.


<details>
  <summary>Details</summary>
Motivation: Existing video-language models struggle with accurate temporal grounding for event-level perception. Current approaches handle temporal grounding and textual response in a coupled manner without clear logical hierarchy, leading to suboptimal performance. The authors recognize that accurate temporal evidence grounding should logically precede reliable textual response.

Method: Proposes D²VLM framework with two key innovations: 1) Decouples temporal grounding and textual response learning using a "grounding then answering with evidence referencing" paradigm with evidence tokens for event-level visual semantic capture; 2) Introduces Factorized Preference Optimization (FPO) algorithm that explicitly incorporates probabilistic temporal grounding modeling into optimization objectives for both tasks. Also constructs a synthetic dataset for factorized preference learning.
...
Conclusion: Factorized learning of temporal grounding and textual response with explicit modeling of their hierarchical relationship significantly improves video-language model performance for event-level video understanding. The D²VLM framework with FPO provides an effective solution to the temporal grounding challenge in video understanding.
...
TL;DR: LMR proposes a two-stage "Think-then-Act" approach for text-to-motion generation, using dual latent spaces to bridge the semantic-kinematic gap between language and motion.


<details>
  <summary>Details</summary>
Motivation: Current T2M methods treat generation as direct translation, but face a fundamental Semantic-Kinematic Impedance Mismatch - difficulty grounding discrete linguistic intent into continuous motion data in one shot.

Method: Latent Motion Reasoning (LMR) uses a Dual-Granularity Tokenizer to disentangle motion into Reasoning Latent (semantic planning) and Execution Latent (physical fidelity), enabling two-stage autoregressive reasoning before execution.

Result: LMR yields non-trivial improvements in both semantic alignment and physical plausibility when implemented on T2M-GPT and MotionStreamer baselines, validating that motion-aligned concept spaces are optimal for planning.

Conclusion: The optimal substrate for motion planning is not natural language but a learned motion-aligned concept space, achieved through architectural shift to Latent System 2 Reasoning that bridges the language-physics gap.
...
Result: Extensive digital and physical experiments show the method significantly outperforms existing attacks in effectiveness, stealthiness, and physical deployability, demonstrating strong practical implications for autonomous driving safety assessment.

Conclusion: The proposed framework generates naturalistic, scene-consistent adversarial objects that induce substantial adversarial depth shifts, overcoming limitations of traditional texture-based patches and enhancing autonomous driving safety assessment capabilities.
...
TL;DR: GeoBench is a hierarchical benchmark for evaluating geometric reasoning in vision-language models, addressing limitations of existing evaluations through four reasoning levels and six formally verified tasks.
...
Method: Developed GeoBench with four hierarchical reasoning levels (Visual Perception, Goal-Oriented Planning, Rigorous Theorem Application, Self-Reflective Backtracking) and six formally verified tasks generated via TrustGeoGen.

Result: Reasoning models like OpenAI-o3 outperform general MLLMs, but performance declines significantly with increasing task complexity. Sub-goal decomposition and irrelevant premise filtering critically influence accuracy, while Chain-of-Thought prompting unexpectedly degrades performance in some tasks.

Conclusion: GeoBench establishes a comprehensive benchmark for geometric problem-solving and offers actionable guidelines for developing geometric reasoning systems.
...
TL;DR: This paper introduces FSAP (Few-Shot Architecture Prompting) for LLM-based neural architecture generation in computer vision, finding n=3 examples optimal, and proposes a fast deduplication method with 100x speedup over AST parsing.
...
Method: Two key contributions: 1) Few-Shot Architecture Prompting (FSAP) - systematic study of supporting examples (n=1-6) for LLM-based architecture generation; 2) Whitespace-Normalized Hash Validation - lightweight deduplication method with 100x speedup over AST parsing. Built on NNGPT/LEMUR framework with experiments across 7 computer vision benchmarks generating 1,900 unique architectures.

Result: Found n=3 examples best balances architectural diversity and context focus for vision tasks. The deduplication method provides 100x speedup over AST parsing and prevents redundant training. Introduced dataset-balanced evaluation methodology for comparing architectures across heterogeneous vision tasks.
...
TL;DR: D²-Align framework prevents Preference Mode Collapse in text-to-image diffusion models by directionally correcting reward signals to maintain diversity while achieving human preference alignment.
...
Method: Proposes Directional Decoupling Alignment (D²-Align): 1) Learns directional correction within reward model's embedding space while keeping model frozen, 2) Applies correction to reward signal during optimization to prevent collapse into specific modes and maintain diversity.

Result: Comprehensive evaluation combining qualitative analysis with quantitative metrics for quality and diversity shows D²-Align achieves superior alignment with human preference while maintaining generative diversity.

Conclusion: D²-Align effectively mitigates Preference Mode Collapse by directionally correcting reward signals, enabling text-to-image diffusion models to maintain diversity while achieving better human preference alignment than existing RLHF methods.
...
<details>
  <summary>Details</summary>
Motivation: To advance multimodal learning for manufacturing and quality inspection by creating a comprehensive industrial defect dataset and developing foundation models that can be efficiently adapted to specialized domains with minimal data.

Method: Created IMDD-1M dataset with 1M aligned image-text pairs of real-world defects, then trained a diffusion-based vision-language foundation model from scratch specifically for industrial scenarios, enabling lightweight fine-tuning for specialized domains.
...
Conclusion: IMDD-1M and the trained foundation model enable scalable, domain-adaptive, and knowledge-grounded manufacturing intelligence, paving the way for data-efficient industrial inspection and generation applications.
...
Result: BSD consistently achieves higher test accuracy (+1.4% for ResNet-50 on CIFAR-100) and significantly lower Expected Calibration Error (-40% ResNet-50, CIFAR-100) than existing self-distillation methods across various architectures and datasets.

Conclusion: BSD provides a principled approach to self-distillation that improves accuracy, calibration, and robustness without hard targets, achieving state-of-the-art robustness under label noise when combined with contrastive loss.
...
TL;DR: DiffThinker introduces a diffusion-based generative multimodal reasoning framework that reformulates vision-centric reasoning as native image-to-image generation, outperforming text-centric MLLMs on complex visual tasks.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) have text-centric reasoning processes that perform poorly on complex long-horizon, vision-centric tasks, necessitating a new paradigm for better logical consistency and spatial precision.

Method: DiffThinker establishes a Generative Multimodal Reasoning paradigm using diffusion-based framework that treats multimodal reasoning as native generative image-to-image tasks rather than text-centric processes.
...
Conclusion: Generative multimodal reasoning via diffusion-based frameworks like DiffThinker represents a promising approach for vision-centric reasoning, offering superior performance, efficiency, controllability, parallelism, and collaboration compared to text-centric MLLMs.
...
<details>
  <summary>Details</summary>
Motivation: HSI analysis faces computational bottlenecks from massive data volumes, and existing foundation models fail to transfer to domain-specific applications like close-range agricultural monitoring where spectral/spatial characteristics differ fundamentally.

Method: Deep Global Clustering (DGC) learns global clustering structure from local patch observations, operating on small patches with overlapping regions to enforce consistency, enabling training in under 30 minutes on consumer hardware with constant memory usage.

Result: On a leaf disease dataset, DGC achieves background-tissue separation (mean IoU 0.925) and demonstrates unsupervised disease detection through navigable semantic granularity.

Conclusion: The framework has merit as intellectual scaffolding but suffers from optimization instability rooted in multi-objective loss balancing - meaningful representations emerge rapidly but degrade due to cluster over-merging, requiring principled approaches to dynamic loss balancing for stable implementation.
...
TL;DR: The paper proposes Internal Guidance (IG), a simple training strategy that adds auxiliary supervision to intermediate layers and extrapolates outputs during sampling, improving diffusion model training efficiency and generation quality without complex degradation strategies or extra sampling steps.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion models struggle with low-probability areas due to insufficient training data coverage. While guidance strategies like CFG help, they often cause over-simplified or distorted samples. Alternative approaches require carefully designed degradation strategies, extra training, or additional sampling steps.

Method: Internal Guidance (IG) introduces auxiliary supervision on intermediate layers during training, then extrapolates intermediate and deep layer outputs during sampling to obtain generative results. This simple strategy doesn't require complex degradation designs, extra training, or additional sampling steps.

Result: IG yields significant improvements in training efficiency and generation quality across various baselines. On ImageNet 256x256, SiT-XL/2+IG achieves FID=5.31 (80 epochs) and FID=1.75 (800 epochs). LightningDiT-XL/1+IG achieves FID=1.34, and combined with CFG reaches state-of-the-art FID of 1.19.
...
TL;DR: PointRAFT is a high-throughput point cloud regression network that directly predicts potato tuber weight from partial 3D point clouds captured by RGB-D cameras on harvesters, addressing systematic underestimation due to self-occlusion.


<details>
  <summary>Details</summary>
Motivation: Potato yield estimation using RGB-D cameras on harvesters suffers from systematic underestimation because point clouds reconstructed from images are incomplete due to self-occlusion of tubers on conveyor belts.

Method: PointRAFT is a point cloud regression network that directly predicts continuous 3D shape properties (like tuber weight) from partial point clouds without full 3D reconstruction. It incorporates an object height embedding as an additional geometric cue to improve weight prediction under practical harvesting conditions.

Result: On a test set of 5,254 point clouds from 172 tubers, PointRAFT achieved mean absolute error of 12.0 g and root mean squared error of 17.2 g, outperforming linear regression and PointNet++ baselines. With 6.3 ms inference time per point cloud, it supports processing rates up to 150 tubers per second.

Conclusion: PointRAFT provides an accurate, high-throughput solution for potato weight estimation on commercial harvesters and offers a versatile regression network applicable to various 3D phenotyping and robotic perception tasks beyond agriculture.
...
TL;DR: CorGi is a training-free inference acceleration framework for Diffusion Transformers (DiT) that reduces redundant computation by selectively caching and reusing low-contribution transformer blocks across denoising steps, achieving up to 2x speedup while preserving generation quality.


<details>
  <summary>Details</summary>
Motivation: DiT models have high inference costs due to iterative denoising combined with large model capacity, and recent work shows substantial redundant computation across denoising steps that can be reduced.

Method: CorGi uses contribution-guided block-wise interval caching to identify and cache low-contribution transformer blocks, reusing them across steps. CorGi+ extends this for text-to-image tasks by using cross-attention maps to identify salient tokens and applying partial attention updates to protect object details.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of imbalanced medical image data, especially during pandemics where COVID-19 datasets have significant class disparities, which hinders accurate AI-based disease detection.

Method: Proposes a progressive generative adversarial network to generate synthetic data, uses a weighted approach to combine synthetic with real data, and employs a multi-objective meta-heuristic population-based optimization algorithm to optimize classifier hyper-parameters.

Result: Achieves 95.5% accuracy for 4-class and 98.5% accuracy for 2-class imbalanced classification on COVID-19 chest X-ray datasets, outperforming existing methods with superior cross-validated metrics.

Conclusion: The proposed model effectively addresses imbalanced medical image classification during pandemics through synthetic data generation and optimization, demonstrating practical utility for COVID-19 detection.
...
TL;DR: ARM is a lightweight learnable module that refines CLIP's internal features for open-vocabulary semantic segmentation, using adaptive hierarchical feature fusion with a "train once, use anywhere" paradigm.
...
Method: ARM uses a semantically-guided cross-attention block where robust deep features (K, V) select and refine detail-rich shallow features (Q), followed by a self-attention block. It learns adaptive hierarchical feature fusion and follows a "train once, use anywhere" paradigm.
...
Conclusion: ARM effectively unlocks CLIP's internal potential for segmentation through adaptive feature refinement, offering a lightweight, universal plug-and-play solution that outperforms existing training-free methods.
...
TL;DR: Mirage is a one-step video diffusion model for photorealistic and temporally coherent asset editing in driving scenes, using a two-stage alignment strategy to address distribution mismatch between scene objects and inserted assets.


<details>
  <summary>Details</summary>
Motivation: Vision-centric autonomous driving systems need diverse training data, but existing video object editing methods struggle to maintain both high visual fidelity and temporal coherence, limiting their effectiveness for data augmentation.

Method: Mirage builds on text-to-video diffusion prior for temporal consistency, injects temporally agnostic latents from a pretrained 2D encoder into the 3D decoder to restore detail, and introduces a two-stage data alignment strategy combining coarse 3D alignment and fine 2D refinement to address distribution mismatch between scene objects and inserted assets.

Result: Extensive experiments demonstrate that Mirage achieves high realism and temporal consistency across diverse editing scenarios, and can generalize to other video-to-video translation tasks.
...
TL;DR: MotivNet is a facial emotion recognition model using Meta's Sapiens vision foundation model as backbone to achieve cross-domain generalization without cross-domain training, validated as a viable Sapiens downstream task.


<details>
  <summary>Details</summary>
Motivation: Current FER models have weak generalization across diverse datasets, deteriorating real-world performance and hindering FER research. Existing solutions require cross-domain training which contradicts real-world application needs.

Method: Uses Meta's Sapiens (human vision foundational model) as backbone with Masked Autoencoder pretraining. Proposes MotivNet as downstream task for Sapiens and defines three evaluation criteria: benchmark performance, model similarity, and data similarity.

Result: MotivNet achieves competitive performance across datasets without cross-domain training, demonstrates generalizability across domains, and meets all three criteria validating it as a viable Sapiens downstream task.

Conclusion: MotivNet successfully addresses FER generalization issues using foundation models, making FER more viable for real-world applications by eliminating need for cross-domain training while maintaining competitive performance.
...
TL;DR: MambaSeg is a dual-branch semantic segmentation framework using parallel Mamba encoders for RGB and event data fusion, with novel Dual-Dimensional Interaction Module for spatial-temporal fusion, achieving SOTA performance with reduced computational cost.


<details>
  <summary>Details</summary>
Motivation: RGB-based segmentation methods degrade under challenging conditions (fast motion, low-light, high dynamic range), while event cameras lack color/texture. Existing multimodal fusion approaches are computationally expensive and neglect temporal dynamics of event streams.
...
Conclusion: MambaSeg shows promise for efficient, scalable, and robust multimodal perception by effectively leveraging complementary properties of RGB and event data through spatial-temporal fusion with reduced computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: Metal artifacts in dental CBCT obscure anatomy and hinder diagnosis. Current deep learning methods have limitations: supervised methods suffer from spectral blurring (regression-to-the-mean), unsupervised methods risk structural hallucinations, and diffusion models are too slow for clinical use.

Method: Three-part framework: 1) Anatomically-Adaptive Physics Simulation (AAPS) synthesizes high-fidelity training pairs using Monte Carlo spectral modeling and patient-specific digital twins. 2) DMP-Former adapts Direct x-Prediction paradigm to reformulate restoration as deterministic manifold projection for single-pass inference. 3) Semantic-Structural Alignment (SSA) module uses medical foundation model (MedDINOv3) priors to ensure clinical plausibility.

Result: PGMP outperforms state-of-the-art methods on both synthetic and multi-center clinical datasets, particularly on unseen anatomy. It sets new benchmarks in both efficiency (fast single-pass inference) and diagnostic reliability.

Conclusion: The PGMP framework effectively addresses key limitations in dental CBCT metal artifact reduction by combining physics-based simulation for training data, deterministic manifold projection for fast inference, and medical foundation model priors for clinical reliability, making it suitable for clinical deployment.
...
TL;DR: DualityForge framework synthesizes counterfactual video QA pairs using diffusion-based editing to reduce hallucinations in multimodal LLMs, achieving 24% improvement over baseline.
...
Result: 24.0% relative improvement over Qwen2.5-VL-7B baseline on counterfactual videos. Significant gains across both hallucination and general-purpose benchmarks, demonstrating strong generalization.
...
TL;DR: A novel panoramic stitching framework that lifts 2D images to 3D point clouds for global fusion, then projects to a cylindrical panorama, reducing distortions in complex scenes with parallax and occlusions.


<details>
  <summary>Details</summary>
Motivation: Traditional 2D homography and mesh warping methods fail in real 3D scenes with multiple depth layers and occlusions, causing ghosting, structural bending, and stretching distortions, especially in multi-view and 360° closed-loop scenarios.
...
Conclusion: The framework successfully shifts stitching from 2D warping to 3D consistency paradigm, enabling flexible incorporation of various 3D lifting and completion modules for improved panoramic quality in complex real-world scenes.
...
TL;DR: EndoRare: A one-shot generative framework that synthesizes diverse, high-fidelity rare gastrointestinal lesion images from a single reference, improving AI classifiers and clinical training.


<details>
  <summary>Details</summary>
Motivation: Rare gastrointestinal lesions are infrequently encountered in routine endoscopy, limiting data for AI model development and training novice clinicians. This creates a "rare-disease gap" in both computer-aided diagnostics and clinical education.

Method: EndoRare uses language-guided concept disentanglement to separate pathognomonic lesion features from non-diagnostic attributes. It encodes lesion features into a learnable prototype embedding while varying non-diagnostic attributes to ensure diversity, enabling one-shot, retraining-free generation from a single reference image.

Result: Validated across four rare pathologies, synthetic images were clinically plausible. When used for data augmentation, they significantly enhanced downstream AI classifiers, improving true positive rates at low false-positive rates. A blinded reader study showed novice endoscopists exposed to EndoRare-generated cases achieved 0.400 increase in recall and 0.267 increase in precision.

Conclusion: EndoRare establishes a practical, data-efficient pathway to bridge the rare-disease gap in both computer-aided diagnostics and clinical education through one-shot generative synthesis of rare lesion exemplars.
...
TL;DR: Virtual-Eyes, a 16-bit CT quality-control pipeline, improves generalist foundation models (RAD-DINO) for lung cancer screening but degrades specialist models (Sybil, ResNet-18) that rely on raw clinical context.


<details>
  <summary>Details</summary>
Motivation: Robust preprocessing is rarely quantified in deep-learning pipelines for low-dose CT lung cancer screening. The authors aim to develop and validate a clinically motivated quality-control pipeline and measure its differential impact on generalist foundation models versus specialist models.

Method: Developed Virtual-Eyes pipeline that enforces 512x512 in-plane resolution, rejects non-diagnostic series, extracts contiguous lung blocks using Hounsfield-unit filtering and bilateral lung-coverage scoring while preserving 16-bit grid. Evaluated on 765 NLST patients using RAD-DINO, Merlin, Sybil, and ResNet-18 models with frozen encoders and leakage-free patient-level MLP heads.

Result: Virtual-Eyes improved RAD-DINO slice-level AUC from 0.576 to 0.610 and patient-level AUC from 0.646 to 0.683 (mean pooling) and 0.619 to 0.735 (max pooling), with better calibration. However, Sybil degraded (AUC 0.886 to 0.837), ResNet-18 showed minimal improvement (0.571 to 0.596), and Merlin showed limited transferability (~0.507 to 0.567) regardless of preprocessing.
...
<details>
  <summary>Details</summary>
Motivation: There's a need for versatile humanoid robots that can follow diverse multimodal instructions with human-level flexibility. Current methods struggle to bridge high-level multimodal perception with whole-body execution and translate heterogeneous instructions into stable, real-time actions.

Method: Two-stage framework: 1) Fine-tuned MLLM (multimodal large language model) integrated with 2) causal streaming pipeline. Uses FSQ (Finite Scalar Quantization) to unify inputs through a shared discrete codebook, ensuring cross-modal alignment while constraining motions to physically grounded manifolds.
...
Conclusion: UniAct represents a critical step toward responsive, general-purpose humanoid assistants capable of seamless interaction through unified perception and control, addressing the bottleneck between multimodal perception and whole-body execution.
...
TL;DR: CERES is a causal framework that improves egocentric referring video object segmentation by addressing dataset biases and visual confounders through dual-modal causal intervention.


<details>
  <summary>Details</summary>
Motivation: Existing Ego-RVOS methods struggle with dataset biases (skewed object-action pairings) and egocentric visual confounders (rapid motion, occlusions), leading to spurious correlations and poor generalization.
...
Result: CERES achieves state-of-the-art performance on Ego-RVOS benchmarks, demonstrating superior segmentation accuracy compared to existing methods.

Conclusion: The framework successfully adapts pre-trained RVOS models to the egocentric domain through causal reasoning, showing potential for building more reliable models for broader egocentric video understanding tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs are limited to text-oriented reasoning and isolated tool use, lacking the ability to seamlessly interleave dynamic tool manipulation with continuous reasoning in knowledge-intensive, visually complex scenarios.

Method: Introduces SenseNova-MARS framework with reinforcement learning (BN-GSPO algorithm) to integrate image search, text search, and image crop tools dynamically for fine-grained visual understanding.

Result: Achieves SOTA performance on search benchmarks: 67.84 on MMSearch and 41.64 on HR-MMSearch (new benchmark), surpassing proprietary models like Gemini-3-Flash and GPT-5.

Conclusion: SenseNova-MARS represents significant progress toward agentic VLMs with robust tool-use capabilities, with code/models/datasets to be released for further research.
...
TL;DR: LVLDrive enhances Vision-Language Models for autonomous driving by incorporating LiDAR point clouds to improve 3D spatial understanding, addressing limitations of 2D image-based methods through gradual feature fusion and specialized training data.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models (VLMs) for autonomous driving rely on 2D image cues, which struggle with accurate metric spatial reasoning and geometric inference, creating safety and reliability bottlenecks for driving policies.

Method: Proposes LVLDrive framework that upgrades VLMs with LiDAR point cloud input using Gradual Fusion Q-Former to incrementally inject 3D features while preserving existing knowledge, and develops spatial-aware question-answering (SA-QA) dataset for teaching advanced 3D perception.
...
Conclusion: Explicit 3D metric data is necessary for building trustworthy VLM-based autonomous systems, and LVLDrive demonstrates the effectiveness of incorporating LiDAR to enhance spatial understanding and decision-making reliability.
...
TL;DR: DermaVQA-DAS extends DermaVQA with expert-annotated datasets for closed QA and lesion segmentation using a novel Dermatology Assessment Schema (DAS), benchmarking multimodal models and showing prompt design impacts segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Existing dermatological image datasets focus on dermatoscopic images and lack patient-authored queries and clinical context, limiting applicability to patient-centered care. There's a need for datasets that better support real-world clinical scenarios.

Method: Introduces DermaVQA-DAS with Dermatology Assessment Schema (DAS) - a structured framework with 36 high-level and 27 fine-grained assessment questions in English and Chinese. Provides expert-annotated datasets for closed QA and segmentation, benchmarking state-of-the-art multimodal models with different prompting strategies.

Result: For segmentation: prompt design impacts performance - default prompt best under Mean-of-Max/Mean-of-Mean schemes, while augmented prompt with patient query yields best under majority-vote (Jaccard: 0.395, Dice: 0.566). For QA: models perform well (avg accuracy 0.729-0.798), with o3 achieving best overall (0.798), GPT-4.1 close second (0.796), Gemini-1.5-Pro competitive (0.783).

Conclusion: DermaVQA-DAS addresses limitations of existing datasets by incorporating patient-authored queries and clinical context through DAS framework. The publicly released dataset, schema, and evaluation protocols will accelerate research in patient-centered dermatological vision-language modeling.
...
TL;DR: This paper introduces elementary information mechanics, a new model that draws analogies between convolutional filtering with rectification and physical theories of special relativity/quantum mechanics, showing how even/odd kernel components relate to energy-momentum relations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to establish a theoretical framework connecting information processing in convolutional neural networks (CNNs) to fundamental physical principles, specifically the energy-momentum relation from relativistic physics, to better understand the mechanical properties of convolutional filtering operations.

Method: The method decomposes convolutional kernels into orthogonal even and odd components, analyzes their effects on image content (even components cause isotropic diffusion, odd components cause directional displacement), and examines these properties in the spectral domain using discrete cosine transform (DCT) to show that small filters are dominated by low-frequency bases (DC and gradient components).

Result: The results show that even kernels preserve center of mass (analogous to rest/potential energy with zero momentum), odd kernels displace center of mass (analogous to kinetic energy with non-zero momentum), and the speed of information displacement is linearly related to the odd vs total kernel energy ratio. Small convolutional filters are shown to be dominated by fundamental DC and gradient modes.

Conclusion: This work establishes the first demonstrated link between information processing in generic CNNs and the energy-momentum relation from relativistic physics, providing a new "elementary information mechanics" framework for understanding convolutional filtering operations through physical analogies.
...
TL;DR: This paper presents a comprehensive framework and taxonomy for multi-modal pre-training to achieve Spatial Intelligence from diverse onboard sensors like cameras and LiDAR, addressing integration challenges and proposing a roadmap for general-purpose foundation models.
...
Method: The paper presents a comprehensive framework for multi-modal pre-training, dissecting the interplay between foundational sensor characteristics and learning strategies. It formulates a unified taxonomy for pre-training paradigms ranging from single-modality baselines to sophisticated unified frameworks that learn holistic representations. The framework investigates integration of textual inputs and occupancy representations for open-world perception and planning.

Result: The paper identifies the core set of techniques driving progress toward Spatial Intelligence, evaluates the role of platform-specific datasets in enabling advancements, and investigates approaches for 3D object detection and semantic occupancy prediction.

Conclusion: The paper identifies critical bottlenecks such as computational efficiency and model scalability, and proposes a roadmap toward general-purpose multi-modal foundation models capable of achieving robust Spatial Intelligence for real-world deployment in autonomous systems.
...
TL;DR: The paper introduces a Feature-to-Image Diffusion (F2IDiff) Foundation Model for smartphone SISR that uses DINOv2 features for stricter, hallucination-free conditioning instead of text features.


<details>
  <summary>Details</summary>
Motivation: Smartphone cameras need SISR with minimal hallucination for high-fidelity LR images, but existing T2IDiff models use text features that are too high-level and can't describe subtle textures in small patches, leading to undesirable hallucinations.

Method: Proposes a Feature-to-Image Diffusion Foundation Model using DINOv2 features for conditioning instead of text. These lower-level features provide stricter conditioning while being rich descriptors of even small patches, enabling hallucination-free super-resolution.

Result: The F2IDiff model addresses the shortcomings of T2IDiff models by providing more accurate conditioning for smartphone SISR, enabling minimal hallucination while maintaining high-quality super-resolution for high-fidelity LR images.

Conclusion: Using lower-level DINOv2 features instead of text features in diffusion foundation models enables better SISR for smartphone photography by providing stricter, hallucination-free conditioning that can accurately describe small image patches.
...
<details>
  <summary>Details</summary>
Motivation: Live video analytics faces high inference costs with modern vision models across massive camera fleets. Dynamic model size selection (DMSS) offers content-aware cost reduction without model retraining, but prior methods fail to generalize to diverse workloads like mobile videos and lower accuracy targets due to inefficient sampling and inaccurate accuracy prediction.

Method: RedunCut uses a measurement-driven planner that estimates the cost-benefit tradeoff of sampling to avoid inefficient sampling, and a lightweight, data-driven performance model to improve per-segment accuracy prediction. The system operates without ground truth labels at runtime.

Result: Across road-vehicle, drone, and surveillance videos with multiple model families and tasks, RedunCut reduces compute cost by 14-62% at fixed accuracy. The system remains robust to limited historical data and to drift.

Conclusion: RedunCut successfully addresses the limitations of prior DMSS methods by optimizing sampling decisions and improving accuracy prediction, enabling significant compute cost reduction while maintaining accuracy across diverse video analytics workloads.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation focuses mainly on accuracy/benchmark scores, with consistency being considered important for real-world applications. The authors argue that both dimensions should be evaluated independently AND their interdependency should be considered for more nuanced LLM evaluation.

Method: Introduces CAT framework with Consistency-Accuracy Relation (CAR) curves that visualize how accuracy varies with increasing consistency requirements (using Minimum-Consistency Accuracy metric). Also proposes Consistency-Oriented Robustness Estimate (CORE) index that combines area and shape of CAR curve to quantify accuracy-consistency trade-off.

Result: Demonstrated the framework across diverse generalist and domain-specific LLMs on multiple-choice benchmarks. Showed how CAT can be extended beyond MC tasks to support long-form, open-ended evaluations through adaptable scoring functions.

Conclusion: CAT provides a comprehensive framework for evaluating the interplay between accuracy and consistency in LLMs, offering both visualization (CAR curves) and quantification (CORE index) tools for more nuanced model assessment, especially important for real-world applications.
...
TL;DR: BanglaCodeAct is an agent-based framework for Bangla-to-Python code generation using multilingual LLMs with iterative self-correction, achieving state-of-the-art performance on mHumanEval dataset.


<details>
  <summary>Details</summary>
Motivation: LLMs excel at English-to-code generation but this progress hasn't extended to low-resource languages like Bangla, creating a need for effective Bangla-to-Python translation methods.

Method: Agent-based framework using multi-agent prompting and iterative self-correction with open-source multilingual LLMs in a Thought-Code-Observation loop for dynamic generation, testing, and refinement.

Result: Qwen3-8B with BanglaCodeAct achieves best performance: 94.0% pass@1 on development set and 71.6% on blind test set of mHumanEval dataset for Bangla NL2Code.

Conclusion: Establishes new benchmark for Bangla-to-Python translation and demonstrates potential of agent-based reasoning for reliable code generation in low-resource languages.
...
TL;DR: PharmaShip is a Chinese pharmaceutical shipping document dataset with noisy OCR and diverse templates for testing text-layout models on SER, RE, and ROP tasks, showing that combining pixel and geometry information with reading-order regularization yields best performance.


<details>
  <summary>Details</summary>
Motivation: To create a real-world benchmark for testing pre-trained text-layout models under challenging conditions (noisy OCR, heterogeneous templates) in the safety-critical pharmaceutical domain, where accurate document understanding is essential.

Method: Created PharmaShip dataset with scanned pharmaceutical shipping documents covering three tasks: sequence entity recognition (SER), relation extraction (RE), and reading order prediction (ROP). Used entity-centric evaluation protocol and benchmarked five representative baselines including LiLT, LayoutLMv3-base, GeoLayoutLM and their RORE-enhanced variants with standardized preprocessing, splits, and optimization.

Result: Pixels and explicit geometry provide complementary inductive biases but neither alone is sufficient. Reading-order-oriented regularization consistently improves SER and EL performance. Longer positional coverage stabilizes late-page predictions and reduces truncation artifacts. ROP is accurate at word level but challenging at segment level due to boundary ambiguity and long-range crossings.

Conclusion: PharmaShip establishes a controlled, reproducible benchmark for pharmaceutical document understanding and highlights sequence-aware constraints as a transferable bias for structure modeling, with reading-order regularization being crucial for robust performance.
...
TL;DR: The paper introduces LN-RP, a computational framework that uses stochastic noise injection to study persona emergence in LLMs, revealing three stable persona modes with distinct entropy signatures and demonstrating noise-induced phase transitions in reflexive generation dynamics.


<details>
  <summary>Details</summary>
Motivation: To develop a reproducible method for studying noise-driven persona emergence, reflexive generation dynamics, and long-range linguistic coherence in large language models, addressing the need for systematic analysis of emergent behavior in LLMs.

Method: The Luca-Noise Reflex Protocol (LN-RP) injects stochastic noise seeds into the initial generation state of LLMs and analyzes linguistic behavior across 152 generation cycles, tracking nonlinear transitions and entropy signatures to identify stable persona modes.

Result: The study reveals three stable persona modes with distinct entropy signatures, demonstrates that external noise sources can reliably induce phase transitions in reflexive generation dynamics, and confirms consistent persona retention with significant differences across modes (p < 0.01).

Conclusion: LN-RP provides a reproducible framework for studying reflexive generation, emergent behavior, and long-range linguistic coherence in LLMs, offering insights into noise-driven persona emergence and phase transitions in language model dynamics.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM safety mechanisms focus on overtly dangerous content but overlook subtle threats where users disguise harmful intent through covert rephrasing, creating a gap in existing safety training data.
...
Result: Experiments show HarmTransform significantly outperforms standard baselines in producing effective query transformations, though debate acts as a double-edged sword - improving stealth while potentially introducing topic shifts and unnecessary complexity.

Conclusion: Multi-agent debate shows promise for generating comprehensive safety training data but has limitations, highlighting both the potential and challenges of this approach for improving LLM safety alignment.
...
TL;DR: LLMs pretrained on poker hand history data learn internal representations of both deterministic game structure (hand ranks) and stochastic features (equity) in partially observable environments, suggesting they develop their own world models for poker.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs can develop emergent world models in domains of incomplete information (POMDPs), extending previous work on perfect information games, using poker as a canonical test case.
...
Conclusion: LLMs can learn their own representation of stochastic environments like Texas Hold'em Poker, suggesting they develop internal world models even in partially observable domains.
...
TL;DR: A two-stage framework combining targeted training procedures and reasoning-based inference to detect subtle, context-dependent sexist content online, addressing challenges of underrepresentation, noise, and conceptual ambiguity.


<details>
  <summary>Details</summary>
Motivation: Sexist content online increasingly appears in subtle, context-dependent forms that evade traditional detection methods. Current approaches struggle with overlapping linguistic, psychological, legal, and cultural dimensions that produce mixed signals, combined with label scarcity, class imbalance, and conceptual ambiguity, leading to unstable decision boundaries and overlooking underrepresented forms of harm.

Method: Two-stage framework: (1) Training stage uses class-balanced focal loss, class-aware batching, and post-hoc threshold calibration to mitigate label imbalance and noisy supervision. (2) Inference stage employs dynamic routing that classifies high-confidence cases directly and escalates uncertain instances to a Collaborative Expert Judgment (CEJ) module, which prompts multiple personas and consolidates their reasoning through a judge model.
...
Conclusion: The proposed framework effectively addresses the combined challenges of underrepresentation, noise, and conceptual ambiguity in sexist content detection by unifying targeted training procedures with selective, reasoning-based inference, demonstrating significant performance improvements over existing methods.
...
TL;DR: The paper introduces the Stored Household Item Challenge benchmark for evaluating service robots' ability to predict where everyday items are stored in homes, and proposes NOAM, a hybrid vision-language model that approaches human-level performance on this task.


<details>
  <summary>Details</summary>
Motivation: Domestic service robots struggle with commonsense reasoning about where everyday items are stored in households, as these items are often hidden in drawers, cabinets, or closets. Current robots lack the cognitive capabilities to infer storage locations from simple commands like "Bring me a plate."

Method: The authors introduce NOAM (Non-visible Object Allocation Model), a hybrid agent pipeline that combines structured scene understanding with large language model inference. It converts visual input into natural language descriptions of spatial context and visible containers, then prompts a language model (e.g., GPT-4) to infer the most likely hidden storage location.

Result: NOAM significantly improves prediction accuracy compared to baselines including random selection, vision-language pipelines, and leading multimodal models. It approaches human-level performance on the benchmark task, demonstrating emergent commonsense reasoning capabilities.

Conclusion: The Stored Household Item Challenge provides a realistic benchmark for evaluating service robots' cognitive capabilities, and NOAM demonstrates that combining structured scene understanding with large language models enables emergent commonsense reasoning that approaches human performance in domestic environments.
...
TL;DR: EASD enhances speculative decoding by adding entropy-based rejection to prevent low-confidence errors, enabling performance that can surpass the target LLM itself while maintaining efficiency comparable to standard SD.


<details>
  <summary>Details</summary>
Motivation: Standard speculative decoding is limited by excessive alignment between draft and target models, constraining performance to that of the target LLM. The authors aim to overcome this limitation by introducing a mechanism that allows the system to potentially exceed the target model's performance.

Method: EASD builds on standard speculative decoding by incorporating a dynamic entropy-based penalty. At each decoding step, it uses the entropy of the sampling distribution to quantify model uncertainty. When both models show high entropy with substantial overlap in their top-N predictions, the token is rejected and re-sampled by the target LLM, preventing low-confidence error propagation.

Result: Experiments across multiple reasoning benchmarks show that EASD consistently outperforms existing SD methods and, in most cases, surpasses the target LLM itself. The efficiency of EASD is proven to be comparable to standard SD.

Conclusion: EASD is a training-free enhancement to speculative decoding that enables performance beyond the target LLM's capabilities while maintaining computational efficiency, representing a significant advancement in LLM acceleration techniques.
...
TL;DR: MiMo-Audio scales audio pretraining to 100M+ hours, achieving few-shot learning and SOTA performance on audio tasks without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current audio models require task-specific fine-tuning, unlike humans who generalize from few examples. The paper aims to bring GPT-3's scaling paradigm to audio for emergent few-shot capabilities.

Method: Scale next-token prediction pretraining to over 100 million hours of audio data, develop systematic evaluation, and use instruction-tuning with thinking mechanisms for both understanding and generation.

Result: MiMo-Audio-7B-Base achieves SOTA on speech intelligence and audio understanding benchmarks among open-source models, generalizes to unseen tasks (voice conversion, style transfer, speech editing), and generates realistic speech continuations. MiMo-Audio-7B-Instruct achieves open-source SOTA on multiple benchmarks, approaching closed-source models.

Conclusion: Scaling audio pretraining enables emergent few-shot learning and strong generalization, similar to text models like GPT-3, creating versatile audio foundation models that perform well across diverse tasks without task-specific fine-tuning.
...
TL;DR: Comparative interpretability study of transformer-based bias detection models using SHAP explanations reveals architectural differences in how models operationalize linguistic bias, with domain-adaptive models showing better alignment between attribution patterns and prediction outcomes.


<details>
  <summary>Details</summary>
Motivation: Little is known about how bias detection models arrive at their decisions or why they fail, despite their heavy use in journalistic analysis and media accountability. Understanding model interpretability is crucial for reliable deployment in journalistic contexts.
...
TL;DR: Adaptive prompting strategy for retrieval-augmented QA that splits retrieved information into chunks and sequentially prompts LLMs, achieving similar performance with fewer tokens while revealing LLMs' tendency to generate incorrect answers when information is insufficient.


<details>
  <summary>Details</summary>
Motivation: While expanded context windows in LLMs enable broader context retrieval for question answering, longer contexts introduce more irrelevant information that hinders generation and degrades performance, creating a need for strategies to balance relevant information incorporation with irrelevant information reduction.

Method: Design an adaptive prompting strategy that splits retrieved information into smaller chunks and sequentially prompts an LLM to answer questions using each chunk, allowing adjustment of chunk size to trade-off between incorporating relevant information and reducing irrelevant information.

Result: Experimental results on three open-domain question answering datasets show that the adaptive strategy matches standard prompting performance while using fewer tokens, and analysis reveals that LLMs often generate incorrect answers instead of declining to respond when encountering insufficient information.

Conclusion: The adaptive prompting strategy effectively addresses the challenge of irrelevant information in retrieval-augmented QA, and the finding about LLMs generating incorrect answers when information is insufficient highlights the need for research into enhancing LLMs' ability to decline requests when faced with inadequate information.
...
TL;DR: The paper proposes using intermediate attention layer token distributions to generate adversarial examples for LLM evaluation, showing they cause performance drops while maintaining semantic similarity, though with some grammatical limitations.


<details>
  <summary>Details</summary>
Motivation: Recent mechanistic interpretability research suggests intermediate attention layers encode token-level hypotheses that are refined toward final outputs. The authors want to exploit this property to generate adversarial examples directly from model-internal token predictions, creating perturbations that are both plausible and internally consistent with the model's own generation process.

Method: The approach leverages attention-layer token distributions to generate adversarial perturbations, unlike prompt-based or gradient-based attacks. Experiments are conducted on argument quality assessment using the ArgQuality dataset, with LLaMA-3.1-Instruct-8B serving as both the generator and evaluator. The method extracts tokens from intermediate layers to serve as adversarial perturbations for downstream evaluation tasks.

Result: Attention-based adversarial examples lead to measurable drops in evaluation performance while remaining semantically similar to original inputs. However, substitutions drawn from certain layers and token positions can introduce grammatical degradation, limiting their practical effectiveness.

Conclusion: The findings highlight both the promise and current limitations of using intermediate-layer representations as a principled source of adversarial examples for stress-testing LLM-based evaluation pipelines.
...
TL;DR: A multi-retriever RAG system with domain-specific training (SecBERT) and latest LLMs achieves SOTA performance on financial numerical QA, surpassing baseline by >7%, though still below human expert performance.


<details>
  <summary>Details</summary>
Motivation: Financial numerical QA tasks remain challenging for LLMs due to lack of domain-specific finance knowledge and complex multi-step numeric reasoning requirements.

Method: Implemented multi-retriever RAG system to retrieve both external domain knowledge and internal question contexts, using domain-specific SecBERT encoder and latest LLMs optimized for few-shot learning.

Result: Domain-specific training with SecBERT helped neural symbolic model surpass FinQA baseline; best prompt-based LLM achieved SOTA with >7% improvement, though still below human expert performance.

Conclusion: Domain-specific training shows superior potential; larger models benefit more from external knowledge despite hallucination risks; latest LLMs demonstrate enhanced few-shot numerical reasoning capabilities.
...
TL;DR: This paper presents an analytics framework that separates student content from teacher grading tendencies to make scoring judgments more transparent and auditable.


<details>
  <summary>Details</summary>
Motivation: Automated scoring often conflates what students write with how teachers grade, making it difficult to distinguish genuine student understanding from rater biases and tendencies.

Method: The framework models teacher histories as dynamic priors, uses sentence embeddings for text representation, and applies centering and residualization to mitigate prompt and teacher confounds. It uses temporally-validated linear models and projections to surface disagreements.

Result: Teacher priors heavily influence grade predictions; combining priors with content embeddings yields strongest results (AUC~0.815), while content-only models remain above chance but weaker (AUC~0.626). Adjusting for rater effects sharpens content representations and reveals cases where semantic evidence supports understanding.

Conclusion: The framework transforms embeddings into learning analytics for reflection, enabling examination of where grading practices align or conflict with evidence of student reasoning and learning.
...
Method: HGMem extends memory beyond simple storage into a dynamic, expressive structure using hypergraph representation. Memory is represented as a hypergraph whose hyperedges correspond to distinct memory units, enabling progressive formation of higher-order interactions within memory. This connects facts and thoughts around the focal problem, evolving into an integrated knowledge structure.
...
Conclusion: HGMem's hypergraph-based memory mechanism provides a dynamic, expressive structure for complex reasoning and global understanding, addressing limitations of traditional passive memory storage in multi-step RAG systems.
...
TL;DR: LoZA is a sparse attention scheme that transforms full-attention models into sparse versions with limited compute budget, enabling efficient processing of up to 1 million tokens for long-context applications.


<details>
  <summary>Details</summary>
Motivation: To address the computational challenges of long-context scenarios in transformer models, particularly for applications requiring processing of up to 1 million tokens, by developing an efficient sparse attention mechanism that works within limited compute budgets.

Method: Introduces LongCat ZigZag Attention (LoZA), a sparse attention scheme that can transform existing full-attention models into sparse versions. The method is applied to LongCat-Flash during mid-training to create LongCat-Flash-Exp, enabling efficient processing of long contexts.

Result: LoZA achieves significant speed-ups in both prefill-intensive (e.g., retrieval-augmented generation) and decode-intensive (e.g., tool-integrated reasoning) cases. The resulting LongCat-Flash-Exp model can process up to 1 million tokens efficiently, enabling long-term reasoning and long-horizon agentic capabilities.

Conclusion: LoZA provides an effective sparse attention solution for long-context processing, enabling transformer models to handle extremely long sequences (up to 1M tokens) with computational efficiency, which supports advanced applications like long-term reasoning and agentic systems.
...
TL;DR: CEC-Zero is a zero-supervision reinforcement learning framework for Chinese spelling correction that enables LLMs to correct their own mistakes without labeled data, outperforming supervised methods by 10-13 F1 points.


<details>
  <summary>Details</summary>
Motivation: Large-scale Chinese spelling correction faces challenges: existing LLMs and supervised methods lack robustness to novel errors and require costly annotations. There's a need for a label-free approach that can handle real-world noisy text effectively.

Method: CEC-Zero uses reinforcement learning with PPO optimization. It synthesizes errorful inputs from clean text, computes cluster-consensus rewards based on semantic similarity and candidate agreement, and enables LLMs to self-correct without supervision.
...
<details>
  <summary>Details</summary>
Motivation: Current approaches to understanding LLM reasoning rely on human-defined concepts at word level, which is limited because it's infeasible to capture the full spectrum of potential reasoning behaviors, many of which are difficult to define in token space.

Method: Propose RISE framework: segment chain-of-thought traces into sentence-level steps, train sparse auto-encoders (SAEs) on step-level activations to uncover disentangled features corresponding to interpretable reasoning behaviors.

Result: SAEs discover reasoning vectors encoding distinct behaviors like reflection and backtracking; visualization shows separable regions in decoder space; interventions can controllably amplify/suppress behaviors; SAEs capture structural properties and enable discovery of novel behaviors beyond human supervision.

Conclusion: Unsupervised latent discovery via SAEs shows strong potential for both interpreting and controllably steering reasoning in LLMs, enabling behavior analysis and control without retraining.
...
TL;DR: The WISE framework benchmarks lightweight transformer models against baselines for distinguishing fake news from satire, finding MiniLM achieves highest accuracy (87.58%) while RoBERTa-base has best ROC-AUC (95.42%), with DistilBERT offering good efficiency-accuracy trade-off.


<details>
  <summary>Details</summary>
Motivation: Distinguishing fake news from satire/humor is challenging due to overlapping linguistic features but divergent intent, requiring effective detection systems for real-world misinformation mitigation.
...
TL;DR: iCLP enables LLMs to generate compact latent plans inspired by human implicit cognition, improving reasoning accuracy, efficiency, and cross-domain generalization while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Textual plans for LLM reasoning suffer from hallucinations and task diversity challenges. Human implicit cognition provides inspiration for more effective subconscious planning.

Method: iCLP distills explicit plans from reasoning trajectories, learns discrete representations via vector-quantized autoencoder with codebook, and fine-tunes LLMs on paired latent plans and reasoning steps.

Result: LLMs with iCLP can plan in latent space while reasoning in language space, showing significant improvements in accuracy and efficiency on mathematical reasoning and code generation tasks.

Conclusion: iCLP enables effective implicit planning for LLMs with strong cross-domain generalization while preserving chain-of-thought interpretability.
...
TL;DR: The paper introduces Composite Reliability Score (CRS), a unified framework that combines calibration, robustness, and uncertainty quantification into a single interpretable metric for evaluating LLM reliability in decision-critical domains.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used in high-stakes domains like healthcare, law, and finance, but their reliability is uncertain due to overconfident errors, degradation under input shifts, and lack of clear uncertainty estimates. Existing evaluations are fragmented and only address isolated aspects of reliability.

Method: The authors propose the Composite Reliability Score (CRS), a unified framework that integrates three key reliability aspects: calibration (how well confidence matches accuracy), robustness (performance under input perturbations), and uncertainty quantification (ability to express uncertainty). They evaluate ten leading open-source LLMs across five QA datasets, assessing performance under baselines, perturbations, and various calibration methods.

Result: CRS provides stable model rankings, uncovers hidden failure modes that single metrics miss, and reveals that the most dependable systems balance accuracy, robustness, and calibrated uncertainty. The framework demonstrates practical utility for comprehensive LLM reliability assessment.

Conclusion: A unified reliability assessment framework like CRS is essential for deploying LLMs in decision-critical domains. The most reliable systems are those that maintain a balance between accuracy, robustness to input variations, and well-calibrated uncertainty estimates, rather than excelling in just one dimension.
...
TL;DR: HY-MT1.5 introduces 1.8B and 7B parameter machine translation models that achieve remarkable efficiency, outperforming much larger open-source models and commercial APIs while approaching ultra-large proprietary model performance.


<details>
  <summary>Details</summary>
Motivation: To develop high-performance, parameter-efficient machine translation models that can compete with and even surpass much larger proprietary and open-source models while supporting advanced translation features.
...
Result: HY-MT1.5-1.8B outperforms significantly larger models (Tower-Plus-72B, Qwen3-32B) and commercial APIs, achieving ~90% of Gemini-3.0-Pro performance. HY-MT1.5-7B achieves 95% of Gemini-3.0-Pro on Flores-200 and surpasses it on WMT25 and Mandarin-minority language benchmarks.

Conclusion: The HY-MT1.5 series provides highly competitive, robust translation solutions with advanced constraint support, establishing new state-of-the-art performance for their respective parameter scales.
...
TL;DR: This demo paper provides a centralized guide to help researchers train Hugging Face models on AWS SageMaker, addressing the steep learning curve and fragmented documentation that hinders cloud adoption.


<details>
  <summary>Details</summary>
Motivation: The development of LLMs is dominated by resource-rich groups, forcing many researchers to use cloud services like AWS SageMaker due to lack of on-premise computing resources. However, the steep learning curve and fragmented documentation create barriers to cloud adoption.

Method: The paper creates a centralized guide/demo that consolidates essential information needed to train Hugging Face models on AWS SageMaker from scratch, filling knowledge gaps left by existing documentation.

Result: The paper provides a comprehensive, accessible resource that democratizes cloud adoption by enabling researchers to successfully train their first Hugging Face model on AWS SageMaker.

Conclusion: By centralizing essential cloud training information, this demo paper helps overcome barriers to cloud adoption and makes LLM development more accessible to researchers without extensive cloud computing experience.
...
<details>
  <summary>Details</summary>
Motivation: Masked diffusion language models lack effective mechanisms for inference-time control and steering, despite their competitive performance with autoregressive models and mask-parallel decoding advantages.

Method: Proposes an activation-steering framework that computes layer-wise steering vectors from a single forward pass using contrastive examples, applied at every reverse-diffusion step without simulating the denoising trajectory.
...
Conclusion: The framework provides an efficient inference-time control mechanism for masked diffusion language models, enabling effective steering without computational overhead of denoising trajectory simulation.
...
TL;DR: LEWM is a Large Emotional World Model that incorporates emotional states alongside visual observations and actions to better predict emotion-driven social behaviors while maintaining performance on basic world modeling tasks.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs focus on physical-world regularities but lack systematic exploration of emotional factors, which are crucial for understanding human decision-making and social behaviors. The paper demonstrates that removing emotionally relevant information degrades reasoning performance.

Method: Proposed LEWM (Large Emotional World Model) with three key components: 1) Construction of Emotion-Why-How (EWH) dataset integrating emotion into causal relationships, 2) Explicit modeling of emotional states alongside visual observations and actions, 3) Enabling prediction of both future states and emotional transitions.
...
Conclusion: Incorporating emotional modeling into world models is important for better understanding and predicting human social behaviors, and LEWM demonstrates effective integration of emotional factors while preserving general world modeling capabilities.
...
TL;DR: TeleChat3-MoE is a large language model series with Mixture-of-Experts architecture (105B-1T+ parameters) trained on Ascend NPU clusters, focusing on training infrastructure for reliable scaling to frontier model sizes.


<details>
  <summary>Details</summary>
Motivation: To enable reliable and efficient scaling of large language models to frontier sizes (over 1 trillion parameters) on specialized hardware (Ascend NPU clusters), addressing challenges in numerical accuracy, performance optimization, and distributed training at massive scale.
...
Result: The infrastructure advancements yield significant throughput improvements and achieve near-linear scaling on clusters comprising thousands of devices, providing robust foundation for large-scale language model development on hardware ecosystems.

Conclusion: The technical report presents comprehensive training infrastructure that enables reliable and efficient scaling of MoE-based large language models to frontier sizes, demonstrating practical solutions for numerical accuracy, performance optimization, and distributed training challenges in large-scale AI development.
...
TL;DR: MedKGI is a clinical diagnostic framework that addresses LLM limitations by integrating medical knowledge graphs for validated reasoning, information-gain question selection for efficiency, and structured state tracking for dialogue coherence.


<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with clinical diagnosis due to three critical limitations: generating hallucinated medical content, asking redundant/inefficient questions, and losing coherence in multi-turn dialogues, failing to emulate real clinical diagnostic reasoning.
...
Conclusion: MedKGI successfully addresses key LLM limitations in clinical diagnosis through knowledge-grounded reasoning, efficient question selection, and structured state management, demonstrating improved performance over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Arabic AES research is limited due to lack of publicly available datasets, creating a need for a comprehensive resource to support development of robust scoring systems for Arabic language.
...
Result: LAILA is now the largest publicly available Arabic AES dataset, filling a critical gap in Arabic language assessment research and enabling development of more robust scoring systems.

Conclusion: LAILA addresses the critical need for Arabic AES datasets, supporting advancement of Arabic language assessment research and development of effective automated scoring systems.
...
<details>
  <summary>Details</summary>
Motivation: Need for a language similarity model that works effectively with both patents and scientific publications to identify credible citation relationships between them.

Method: Developed Pat-SPECTER by fine-tuning SPECTER2 model on patents, then evaluated eight language similarity models in predicting credible Patent-Paper Citations through horse race-style evaluation.

Result: Pat-SPECTER performed best among tested models. Demonstrated effectiveness in separating and predicting patent-paper pairs. Found US patents cite papers that are semantically less similar than other jurisdictions, supporting duty of candor hypothesis.

Conclusion: Pat-SPECTER is an effective open-source model for analyzing patent-paper relationships, revealing jurisdiction-specific citation patterns influenced by legal requirements like duty of candor.
...
TL;DR: DATAMASK is an efficient joint learning framework for large-scale pre-training data selection that simultaneously optimizes quality and diversity metrics through policy gradient-based mask learning, achieving 98.9% faster selection and significant performance improvements on LLMs.


<details>
  <summary>Details</summary>
Motivation: Current data selection methods for trillion-scale pre-training datasets either use quality metrics (which show diminishing returns) or diversity metrics (which remove valuable high-quality samples), but rarely combine both due to computational costs, limiting LLM capabilities.

Method: DATAMASK treats data selection as a mask learning problem with iterative sampling of data masks, policy gradient computation based on predefined objectives, and updating of mask sampling logits, using acceleration enhancements for efficiency.

Result: DATAMASK reduces selection time by 98.9% compared to greedy algorithms, enabling joint learning on trillion-scale tokens. Selected FineWeb-Mask subset (10% of 15T tokens) improves performance by 3.2% on 1.5B dense model and 1.9% on 7B MoE model across 12 diverse tasks.

Conclusion: DATAMASK enables efficient joint optimization of quality and diversity metrics for large-scale pre-training data selection, overcoming limitations of single-metric approaches and significantly enhancing LLM performance while dramatically reducing computational costs.
...
TL;DR: LLMs show moderate success in qualitative EU Taxonomy compliance tasks but fail at quantitative KPI prediction; dataset created for benchmarking.


<details>
  <summary>Details</summary>
Motivation: The manual, resource-intensive process of EU Taxonomy compliance presents challenges for companies, and research is hindered by lack of public benchmark datasets for evaluating LLM automation.

Method: Created a novel structured dataset from 190 corporate reports with ground-truth economic activities and KPIs; conducted systematic evaluation of LLMs on core compliance workflow including qualitative activity identification and quantitative KPI prediction.

Result: LLMs show moderate success in qualitative economic activity identification (multi-step agentic framework modestly enhances precision), but comprehensively fail at quantitative KPI prediction in zero-shot setting; concise metadata often outperforms full reports; model confidence scores are poorly calibrated.
...
TL;DR: FIGR integrates visual thinking into reasoning via reinforcement learning, constructing visual representations to handle implicit spatial/structural relationships that text alone struggles with, improving performance on mathematical reasoning benchmarks.
...
TL;DR: QianfanHuijin is a financial domain LLM with a multi-stage training paradigm combining knowledge enhancement with financial reasoning and agentic capabilities, achieving superior performance on financial benchmarks.


<details>
  <summary>Details</summary>
Motivation: Previous financial LLMs focused mainly on knowledge enhancement, but increasing complexity in financial services requires models with robust financial reasoning and agentic capabilities to handle real-world business scenarios.

Method: Multi-stage training: 1) Continual Pre-training on financial corpora for knowledge base; 2) Fine-grained Post-training pipeline: Financial SFT → Finance Reasoning RL → Finance Agentic RL → General RL aligned with real-world business scenarios.
...
Conclusion: The fine-grained, progressive post-training methodology is validated and poised to become a mainstream paradigm for various industrial-enhanced LLMs, addressing the need for both domain knowledge and reasoning/agentic capabilities.
...
TL;DR: WM-SAR reformulates sarcasm understanding as a world model reasoning process using specialized LLM agents to decompose literal meaning, context, normative expectations, and intentions, achieving state-of-the-art performance with interpretable decision structure.
...
Method: Proposes WM-SAR with specialized LLM-based agents to decompose literal meaning, context, normative expectation, and intention. Quantifies discrepancy between literal evaluation and normative expectation as inconsistency score, combines with intention score, and integrates via lightweight Logistic Regression for final sarcasm probability.
...
<details>
  <summary>Details</summary>
Motivation: Transformer models struggle with long document representation in specialized fields like law and medicine. Sparse attention is resource-intensive, hierarchical transformers lack clear inter-section relationships, while humans efficiently skim texts focusing on important sections.

Method: Introduces a self-supervised contrastive learning framework that randomly masks document sections and uses natural language inference (NLI)-based contrastive objectives to align masked sections with relevant parts while distancing them from unrelated ones, mimicking human skimming behavior.
...
Conclusion: The proposed framework successfully addresses long document representation challenges by mimicking human skimming strategies, resulting in richer representations that are both more accurate and computationally efficient for specialized domains.
...
TL;DR: This paper compares various approaches to text summarization in less-resourced languages, finding that fine-tuned mT5 models outperform most other methods including zero-shot LLMs, and that LLM-based evaluation may be unreliable for these languages.


<details>
  <summary>Details</summary>
Motivation: While automatic text summarization performs well in high-resourced languages like English, there has been comparatively less attention given to summarization in less-resourced languages, creating a need to evaluate different approaches for these languages.

Method: The study compares multiple approaches: zero-shot prompting of various sized LLMs, fine-tuning smaller models like mT5 with and without three data augmentation methods and multilingual transfer, and an LLM translation pipeline (translate to English, summarize, translate back). Evaluation uses five different metrics.

Result: Results show variation across LLMs even with similar parameter sizes, multilingual fine-tuned mT5 baseline outperforms most other approaches including zero-shot LLM performance for most metrics, and LLM-as-judge evaluation may be less reliable for less-resourced languages.

Conclusion: Fine-tuned smaller models like mT5 can be more effective than zero-shot LLMs for summarization in less-resourced languages, and careful consideration is needed when using LLMs for evaluation in these language contexts.
...
TL;DR: A language model that cleans scientific abstracts by removing extraneous content like copyright statements and metadata, improving downstream text analysis.
...
Conclusion: The introduced model effectively cleans scientific abstracts, enhancing the quality of text-based analyses by removing distorting extraneous content.
...
TL;DR: A revision platform for IELTS writing preparation using transformer-based automated scoring and adaptive feedback shows significant improvement in candidate scores, though best used as supplement to human instruction.


<details>
  <summary>Details</summary>
Motivation: Traditional IELTS preparation lacks personalized feedback tailored to the IELTS writing rubric, creating a need for automated systems that can provide targeted guidance to candidates.

Method: Design-Based Research (DBR) approach with iterative cycles: early rule-based systems, then DistilBERT transformer model with regression head for scoring, followed by adaptive feedback implementation with separate conversational guidance and writing interfaces.
...
Conclusion: Automated feedback functions best as supplement to human instruction; challenges remain in assessing higher-band essays; future work needs longitudinal studies with real candidates and validation from official examiners.
...
TL;DR: This paper introduces paragraph segmentation for speech transcripts, creates two new benchmarks (TEDPara and YTSegPara), proposes a constrained-decoding method for LLMs to insert paragraph breaks, and develops MiniSeg model achieving SOTA accuracy with hierarchical chapter-paragraph prediction.


<details>
  <summary>Details</summary>
Motivation: Automatic speech transcripts are unstructured word streams that hinder readability and repurposing. There's a lack of standardized benchmarks and methods for paragraph segmentation in speech processing, which has traditionally not been part of post-processing pipelines.

Method: 1) Created TEDPara (human-annotated TED talks) and YTSegPara (YouTube videos with synthetic labels) as first benchmarks for paragraph segmentation in speech. 2) Proposed constrained-decoding formulation for LLMs to insert paragraph breaks while preserving original transcripts. 3) Developed MiniSeg model that achieves state-of-the-art accuracy and can be extended hierarchically to jointly predict chapters and paragraphs.

Result: Established first benchmarks for paragraph segmentation in speech domain, demonstrated that compact MiniSeg model attains state-of-the-art accuracy, and showed hierarchical extension can jointly predict chapters and paragraphs with minimal computational cost.

Conclusion: The paper establishes paragraph segmentation as a standardized, practical task in speech processing through new benchmarks, constrained-decoding methods, and efficient models that enable structured, readable speech transcripts.
...
TL;DR: LLM safety alignment doesn't transfer zero-shot across languages; study reveals complex interference patterns where safety depends on language-temporal intersections, with models showing reverse linguistic effects and catastrophic temporal reasoning failures.


<details>
  <summary>Details</summary>
Motivation: The dangerous assumption that safety alignment transfers zero-shot from English to other languages needs systematic testing, especially for low-resource languages and localized threat scenarios affecting Global South users.

Method: Systematic audit of GPT-5.1, Gemini 3 Pro, and Claude 4.5 Opus using HausaSafety dataset with West African threat scenarios. Employed 2x4 factorial design across 1,440 evaluations testing language (English vs. Hausa) and temporal framing interactions.

Result: Found Complex Interference mechanism instead of simple degradation: Claude 4.5 Opus safer in Hausa (45.0%) than English (36.7%) due to uncertainty-driven refusal, but catastrophic temporal reasoning failures. Profound Temporal Asymmetry: past-tense framing bypassed defenses (15.6% safe) while future-tense triggered hyper-conservative refusals (57.2% safe). 9.2x disparity between safest and most vulnerable configurations.

Conclusion: Current models rely on superficial heuristics, creating Safety Pockets that expose Global South users to localized harms. Propose Invariant Alignment as necessary paradigm shift for safety stability across linguistic and temporal shifts.
...
TL;DR: HaluNet is a lightweight neural framework that integrates multiple uncertainty sources (semantic embeddings, probabilistic confidence, distributional uncertainty) for efficient one-pass hallucination detection in LLM-based QA systems.


<details>
  <summary>Details</summary>
Motivation: LLMs often generate hallucinations (factual errors or fabricated content) in QA tasks. Existing hallucination detection methods focus on single uncertainty types and overlook complementarity between token-level probability uncertainty and internal semantic representation uncertainty.

Method: HaluNet integrates multi-granular token-level uncertainties by combining semantic embeddings with probabilistic confidence and distributional uncertainty. It uses a multi-branch architecture that adaptively fuses model knowledge with output uncertainty for efficient one-pass detection.
...
Conclusion: HaluNet demonstrates potential for real-time hallucination detection in LLM-based QA systems by effectively leveraging complementary uncertainty sources in a lightweight, trainable framework.
...
Method: Developed KCL with two components: KCL-MCQA (283 multiple-choice questions with 1,103 aligned precedents) and KCL-Essay (169 open-ended questions with 550 aligned precedents and 2,739 rubrics for automated evaluation).

Result: Evaluation of 30+ models shows large remaining gaps, especially in KCL-Essay, with reasoning-specialized models consistently outperforming general-purpose counterparts.

Conclusion: KCL provides a valuable resource for assessing legal reasoning capabilities, revealing significant challenges in open-ended legal reasoning tasks and demonstrating the advantage of reasoning-specialized models.
...
Conclusion: CREST provides a simple, effective, training-free pathway to steer LLM reasoning by targeting specific cognitive behaviors in attention heads, achieving both higher accuracy and lower computational cost simultaneously.
...
TL;DR: Youtu-LLM is a 1.96B parameter lightweight language model pre-trained from scratch with native agentic intelligence, featuring long-context support and a progressive training curriculum from commonsense to STEM/agent tasks.


<details>
  <summary>Details</summary>
Motivation: To create a lightweight yet powerful language model that harmonizes computational efficiency with native agentic intelligence, moving beyond typical small models that rely on distillation and lack intrinsic reasoning capabilities.

Method: 1) Compact MLA architecture with STEM-oriented vocabulary supporting 128k context; 2) Multi-stage training curriculum shifting from commonsense to complex STEM/agent tasks using 11T tokens; 3) Scalable agentic mid-training with diverse trajectory synthesis for math, coding, and tool-use domains.

Result: Sets new SOTA for sub-2B LLMs, achieving competitive performance on general benchmarks against larger models and significantly surpassing existing baselines on agent-specific tasks.

Conclusion: Lightweight models can possess strong intrinsic agentic capabilities when systematically pre-trained with proper architecture and progressive curriculum, demonstrating that computational efficiency and advanced reasoning are not mutually exclusive.
...
TL;DR: LLMs are generally overconfident in predicting their task success, with this overconfidence worsening during multi-step tasks. While some LLMs can learn from failure experiences to improve decision-making, their overly-optimistic estimates lead to poor decisions despite rational decision-making based on those estimates.
...
Method: Tested various LLMs on their ability to predict task success, examined how predictions change during multi-step agentic tasks, and evaluated whether LLMs can learn from in-context failure experiences to improve decision-making.

Result: All tested LLMs were overconfident but had better-than-random discriminatory power. Newer/larger models generally didn't have greater discriminatory power (except Claude models). Overconfidence worsened during multi-step tasks, and reasoning LLMs performed comparably or worse than non-reasoning ones. Some LLMs reduced overconfidence after failure experiences, improving decision-making, while others didn't. All LLMs made rational decisions given their estimated probabilities, but poor decisions resulted from overly-optimistic estimates.

Conclusion: Current LLM agents lack awareness of their own capabilities, which hinders their performance. This has implications for AI misuse and misalignment risks, suggesting that improving LLMs' self-awareness could enhance their reliability and safety.
...
TL;DR: R-Debater is an agentic framework for multi-turn debates that uses argumentative memory to retrieve and adapt prior arguments for stance consistency and evidence-based responses.


<details>
  <summary>Details</summary>
Motivation: The paper aims to create more coherent and evidence-based multi-turn debates by grounding debate generation in rhetoric and memory studies, addressing the need for stance consistency and factual support in automated debate systems.
...
Result: R-Debater achieves higher single-turn and multi-turn scores compared to strong LLM baselines on both next-utterance generation (assessed by InspireScore) and adversarial multi-turn simulations (judged by Debatrix). Human evaluation with 20 experienced debaters confirms its consistency and evidence use.

Conclusion: Combining retrieval grounding with structured planning yields more faithful, stance-aligned, and coherent debates across turns, demonstrating the effectiveness of argumentative memory for automated debate generation.
...
TL;DR: MUSIC is an unsupervised data augmentation strategy that creates multi-turn contrastive conversation pairs to train better multi-turn reward models, improving alignment with human-like judgments without sacrificing single-turn performance.


<details>
  <summary>Details</summary>
Motivation: Current multi-turn conversation evaluation for LLMs relies heavily on costly human evaluation. While multi-turn reward models offer a scalable alternative, existing training data typically only contrasts final-turn responses, failing to capture the nuances of multi-turn interactions. There's a need for better automated evaluation specifically designed for multi-turn conversations.

Method: Proposes MUSIC (Multi-Step Instruction Contrast), an unsupervised data augmentation strategy that synthesizes contrastive conversation pairs with differences spanning multiple turns. Applied to the Skywork preference dataset to train a multi-turn RM based on Gemma-2-9B-Instruct model.

Result: The MUSIC-augmented RM outperforms baseline methods, achieving higher alignment with judgments from advanced proprietary LLM judges on multi-turn conversations. Importantly, it maintains performance on standard single-turn RM benchmarks.

Conclusion: Incorporating multi-turn contrasts is critical for building robust multi-turn reward models. MUSIC provides an effective unsupervised approach to create better training data for multi-turn RMs, enabling improved automated evaluation of multi-turn conversations without compromising single-turn performance.
...
TL;DR: BIOME-Bench is a new benchmark for evaluating LLMs on multi-omics pathway analysis, addressing limitations of current pathway enrichment methods and providing standardized evaluation for biomolecular interaction inference and pathway mechanism elucidation.


<details>
  <summary>Details</summary>
Motivation: Current pathway enrichment methods have structural limitations including curation lag, functional redundancy, and limited sensitivity to molecular states. Existing LLM evaluations for multi-omics analysis lack standardized benchmarks, relying on small datasets or ad hoc case studies, hindering reproducible progress.

Method: Developed BIOME-Bench via a rigorous four-stage workflow to evaluate two core LLM capabilities: Biomolecular Interaction Inference and end-to-end Multi-Omics Pathway Mechanism Elucidation. Created evaluation protocols and conducted comprehensive experiments across multiple contemporary models.
...
Conclusion: BIOME-Bench addresses the need for standardized evaluation in multi-omics pathway analysis, revealing current LLM limitations and providing a foundation for reproducible progress in this important biomedical domain.
...
TL;DR: LLMs show improved reasoning with intermediate steps, but this creates computational burden. The paper evaluates LLMs considering both accuracy and inference costs, finding MoE architectures balance performance/efficiency best, and identifies diminishing returns beyond certain compute thresholds.


<details>
  <summary>Details</summary>
Motivation: Current literature overlooks the computational burden of generating long reasoning sequences in LLMs. For industrial applications, model selection must consider not just accuracy but also resource constraints and inference costs.
...
Result: Mixture of Experts (MoE) architecture emerges as strong candidate to balance performance and efficiency. Identified emergent trend regarding accuracy gain per unit of compute. Found saturation point for inference-time compute where accuracy gains diminish beyond certain threshold.

Conclusion: While extended reasoning capabilities are beneficial, they cannot overcome intrinsic model limitations regarding specific complexities. There are diminishing returns to inference-time compute, and MoE architectures offer promising performance/efficiency trade-offs for industrial applications.
...
TL;DR: A course on Ethical Aspects in NLP using active learning methods that has been refined over 4 years across institutions, producing reusable teaching materials and student-created educational products.
...
Conclusion: Sharing this approach provides inspiration for educators seeking to incorporate social impact considerations into their curricula, demonstrating a successful model for ethical NLP education.
...
TL;DR: The paper proposes "triangulation" - a causal standard for evaluating mechanistic explanations in multilingual language models that requires necessity, sufficiency, and invariance across language variants.


<details>
  <summary>Details</summary>
Motivation: Multilingual language models show unpredictable behavior across languages, scripts, and cultures, and current mechanistic explanations lack rigorous causal validation that survives interventions and cross-lingual testing.

Method: Introduces "triangulation" as an acceptance rule with three criteria: necessity (circuit ablation degrades behavior), sufficiency (activation patching transfers behavior), and invariance (effects remain stable across "reference families" - predicate-preserving variants). Uses automatic circuit discovery and evaluates candidates through triangulation across multiple model families, language pairs, and tasks.

Result: Triangulation provides a falsifiable standard that filters spurious circuits that pass single-environment tests but fail cross-lingual invariance, offering more robust mechanistic explanations.

Conclusion: The proposed triangulation framework establishes a causal standard for mechanistic interpretability in multilingual models, connecting to causal abstraction theory and providing a rigorous experimental protocol for validating circuit explanations across languages.
...
TL;DR: PrivacyBench benchmark reveals RAG assistants leak secrets in 26.56% of interactions, with privacy prompts reducing leakage to 5.12%, highlighting need for privacy-by-design safeguards.


<details>
  <summary>Details</summary>
Motivation: Personalized AI agents access sensitive user data (emails, chats, purchase histories), creating privacy risks as systems lacking social-context awareness can unintentionally expose user secrets, threatening digital well-being.
...
Result: RAG assistants leak secrets in up to 26.56% of interactions. Privacy-aware prompts lower leakage to 5.12%, but retrieval mechanisms continue to access sensitive data indiscriminately, creating a single point of failure in privacy preservation.

Conclusion: Current architectures are unsafe for wide-scale deployment. Findings underscore urgent need for structural, privacy-by-design safeguards to ensure ethical and inclusive web for everyone.
...
TL;DR: The paper critiques how Big AI and language engineers are accelerating ecological, meaning, and language crises, calling for alternatives centered on human flourishing and planetary wellbeing.


<details>
  <summary>Details</summary>
Motivation: The world faces converging ecological, meaning, and language crises (metacrisis) that Big AI is accelerating. Language engineers are contributing to these problems by pursuing scalability narratives that harm humanity, supporting plutocrats/kleptocrats, and treating technology as value-free.

Method: The paper advocates for exploring alternatives through collective intelligence to design life-affirming futures for NLP. It calls for a paradigm shift away from current harmful practices.

Result: The analysis reveals the problematic role of language engineers in perpetuating crises through their work with Big AI, highlighting the urgent need for value-conscious alternatives.

Conclusion: NLP must be redesigned to center human flourishing and planetary wellbeing, requiring a fundamental reorientation away from current harmful practices toward life-affirming alternatives.
...
TL;DR: Encyclo-K is a statement-based benchmark that uses knowledge statements from textbooks to dynamically generate questions, addressing data contamination, single-knowledge limitations, and high annotation costs in LLM evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing LLM benchmarks have three key limitations: vulnerability to data contamination, restriction to single-knowledge-point assessment, and reliance on costly domain expert annotation. The authors aim to create a more robust, scalable evaluation framework.

Method: Extract standalone knowledge statements from authoritative textbooks, then dynamically compose them into evaluation questions through random sampling at test time. Each question aggregates 8-10 statements for comprehensive multi-knowledge assessment.

Result: Experiments on 50+ LLMs show strong discriminative power. Top-performing OpenAI-GPT-5.1 achieves only 62.07% accuracy. Reasoning models span 16.04%-62.07%, chat models range 9.71%-50.40%. Model rankings remain stable across dynamically generated question sets.
...
TL;DR: mHC framework restores identity mapping in Hyper-Connections to solve training instability and memory issues while improving performance and scalability.


<details>
  <summary>Details</summary>
Motivation: Hyper-Connections (HC) improve performance but compromise the identity mapping property of residual connections, causing training instability, restricted scalability, and memory overhead.
...
Conclusion: mHC is a flexible, practical extension of HC that contributes to understanding topological architecture design and suggests directions for foundational model evolution.
...
TL;DR: PartMotionEdit is a novel fine-grained 3D human motion editing framework that enables precise part-specific motion control through part-level semantic modulation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing text-driven 3D human motion editing methods have difficulty precisely controlling detailed, part-specific motions due to their global modeling approach, creating a need for more fine-grained control.

Method: The framework uses a Part-aware Motion Modulation (PMM) module with five-part body decomposition and time-varying modulation weights, part-level similarity curve supervision with dual-layer normalization, and a Bidirectional Motion Interaction (BMI) module with bidirectional cross-modal attention.

Result: Extensive quantitative and qualitative evaluations on a well-known benchmark demonstrate that PartMotionEdit outperforms state-of-the-art methods in fine-grained motion editing.

Conclusion: PartMotionEdit provides a novel solution for precise, part-specific 3D human motion editing through semantic modulation, achieving superior performance compared to existing approaches.
...
TL;DR: BATISNet is a boundary-aware instance network for tooth point cloud segmentation that addresses challenges in complex dental cases by combining semantic and instance features with boundary-aware supervision.


<details>
  <summary>Details</summary>
Motivation: Existing semantic segmentation methods struggle with complex dental cases due to tightly packed teeth structures, unclear boundaries, and diverse conditions like missing or malposed teeth, leading to unsatisfactory results in clinical applications.

Method: BATISNet consists of a feature extraction backbone and instance segmentation module that extracts both semantic features of different tooth types and instance features of individual teeth. It includes a boundary-aware loss function specifically designed to supervise boundary segmentation between instances.

Result: Extensive experiments show BATISNet outperforms existing methods in tooth integrity segmentation, effectively mitigating tooth adhesion and boundary ambiguity issues in complex clinical scenarios.

Conclusion: BATISNet provides more robust and accurate tooth instance segmentation for complex dental cases, offering reliable and detailed data support for practical clinical applications including diagnosis and treatment planning.
...
TL;DR: Review article examining how the Uncanny Valley phenomenon affects medical VR simulation training, providing evidence-based guidance on managing virtual human realism to optimize learning outcomes.


<details>
  <summary>Details</summary>
Motivation: To address the critical challenge of the Uncanny Valley effect in medical VR training, where highly realistic virtual humans can cause discomfort due to subtle imperfections, potentially undermining the effectiveness of simulation-based medical education that relies on realism and immersion.

Method: Multidisciplinary review combining expertise in computer graphics, virtual reality, and medical education, drawing from collaborative research across institutions and industries to analyze evidence and provide practical guidance.

Result: Provides comprehensive evidence-based visual guidance on managing Uncanny Valley effects in medical VR training, offering insights into balancing realism with user comfort to optimize educational outcomes.

Conclusion: Understanding and addressing the Uncanny Valley phenomenon is crucial for effective medical VR simulation training, requiring careful balance between realism and user comfort to maximize learning effectiveness in immersive medical education environments.
...
TL;DR: PhysTalk is a train-free framework that converts 3D Gaussian Splatting scenes into real-time, physics-based 4D animations using LLM-generated code, enabling interactive open-vocabulary visual effects without mesh extraction.


<details>
  <summary>Details</summary>
Motivation: Current visual effects pipelines lack physical realism and effective language interfaces, requiring slow offline optimization. There's a need for accessible, real-time physics-based animation from text prompts without expert animation knowledge.

Method: Uses 3D Gaussian Splatting (3DGS) scenes as input, with LLMs generating executable code that modifies 3DGS parameters through lightweight proxies and particle dynamics. Directly couples 3DGS with physics simulator without mesh extraction.

Result: Enables real-time, interactive 4D animations with physics-based manipulation of arbitrary multi-material objects. First framework to directly couple 3DGS with physics simulator, remaining open-vocabulary and computationally lightweight.

Conclusion: PhysTalk makes 4D animation broadly accessible by shifting from "render and wait" to interactive dialogue, offering train-free, physics-informed visual effects generation from text prompts.
...
TL;DR: The paper introduces DDFT (Drill-Down and Fabricate Test), a new evaluation protocol that measures epistemic robustness - how well language models maintain factual accuracy under semantic compression and adversarial fabrication, revealing that robustness is orthogonal to model size and architecture.


<details>
  <summary>Details</summary>
Motivation: Current language model evaluations (like MMLU and TruthfulQA) only measure knowledge under ideal conditions, failing to assess how robustly models maintain factual accuracy when information degrades or when adversaries probe for weaknesses. There's a need to evaluate epistemic robustness before deployment in critical applications.

Method: The authors introduce DDFT protocol that measures epistemic robustness through progressive semantic compression and adversarial fabrication. They propose a two-system cognitive model with Semantic System (generates text) and Epistemic Verifier (validates facts). They evaluate 9 frontier models across 8 knowledge domains at 5 compression levels (1,800 turn-level evaluations).
...
Method: CASCADE enables agents to master complex external tools through two meta-skills: 1) continuous learning via web search and code extraction, and 2) self-reflection via introspection and knowledge graph exploration. The framework accumulates executable skills that can be shared across agents and scientists.

Result: On SciSkillBench (116 materials science and chemistry research tasks), CASCADE achieves 93.3% success rate using GPT-5, compared to 35.4% without evolution mechanisms. The framework demonstrates real-world applications in computational analysis, autonomous laboratory experiments, and selective reproduction of published papers.

Conclusion: CASCADE represents a transition from "LLM + tool use" to "LLM + skill acquisition," enabling scalable AI-assisted scientific research through human-agent collaboration, memory consolidation, and shareable executable skills that accumulate over time.
...
<details>
  <summary>Details</summary>
Motivation: Symbolic AI in healthcare is limited by the effort needed to build high-quality knowledge bases. Current approaches require manual construction of medical knowledge bases, which is time-consuming and resource-intensive.

Method: McCoy uses LLMs to automatically translate medical literature into Answer Set Programming (ASP) code, combines this with patient data, and processes it through an ASP solver to generate diagnoses.

Result: Preliminary results show McCoy has strong performance on small-scale disease diagnosis tasks, demonstrating the feasibility of the approach.

Conclusion: The McCoy framework successfully integrates LLMs with symbolic reasoning (ASP) to create an automated, interpretable disease prediction system that overcomes knowledge base construction barriers in healthcare AI.
...
<details>
  <summary>Details</summary>
Motivation: Personalized search requires modeling users' evolving, multi-dimensional information needs, but current systems are constrained by static profiles or monolithic retrieval pipelines that can't capture the complexity, fluidity, and context sensitivity of human information-seeking behavior.

Method: SPARK formalizes a persona space (role, expertise, task context, domain) with a Persona Coordinator that activates relevant specialized agents. Each agent executes independent retrieval-augmented generation with dedicated memory stores and reasoning modules. Inter-agent collaboration uses structured communication protocols including shared memory, iterative debate, and relay-style knowledge transfer.

Result: The framework yields testable predictions regarding coordination efficiency, personalization quality, and cognitive load distribution, while incorporating adaptive learning mechanisms for continuous persona refinement. It models how emergent personalization properties arise from distributed agent behaviors.

Conclusion: SPARK provides insights for next-generation search systems capable of capturing the complexity, fluidity, and context sensitivity of human information-seeking behavior by integrating fine-grained agent specialization with cooperative retrieval.
...
TL;DR: ROAD is a novel framework for automatic prompt optimization that bypasses the need for labeled datasets by treating optimization as a debugging investigation, achieving significant performance improvements with high sample efficiency.


<details>
  <summary>Details</summary>
Motivation: Current APO methods require large labeled gold-standard datasets, which are rarely available in real-world software engineering during initial cold start phases where engineers only have messy production logs and evolving failure modes.

Method: ROAD uses a specialized multi-agent architecture with Analyzer (root-cause analysis), Optimizer (pattern aggregation), and Coach (strategy integration) to convert unstructured failure logs into structured Decision Tree Protocols, treating optimization as dynamic debugging rather than stochastic search.

Result: ROAD achieved 5.6% increase in success rate (73.6% to 79.2%) and 3.8% increase in search accuracy within just three automated iterations. On complex retail reasoning tasks, it improved agent performance by approximately 19% relative to baseline.

Conclusion: Mimicking the human engineering loop of failure analysis and patching offers a viable, data-efficient alternative to resource-intensive RL training for deploying reliable LLM agents in real-world scenarios without curated datasets.
...
TL;DR: LoongFlow is a self-evolving agent framework that integrates LLMs into evolutionary search using a cognitive "Plan-Execute-Summarize" paradigm, achieving state-of-the-art solution quality with 60% better evolutionary efficiency than leading baselines.


<details>
  <summary>Details</summary>
Motivation: Traditional evolutionary approaches for LLM-based agents lack structured reasoning, suffer from premature convergence, and struggle with inefficient exploration in high-dimensional code spaces, hindering the transition from static LLMs to self-improving agents.

Method: LoongFlow integrates LLMs into a cognitive "Plan-Execute-Summarize" (PES) paradigm, replacing "blind" mutation with reasoning-heavy evolutionary search. It incorporates a hybrid evolutionary memory system combining Multi-Island models with MAP-Elites and adaptive Boltzmann selection to balance exploration-exploitation trade-off and maintain diverse behavioral niches.
...
Conclusion: LoongFlow marks a substantial step forward in autonomous scientific discovery, enabling generation of expert-level solutions with reduced computational overhead by effectively mapping evolutionary search to reasoning processes and maintaining architectural coherence.
...
TL;DR: CogRec is a cognitive recommender agent that combines LLMs with the Soar cognitive architecture to create an interpretable, online-learning recommendation system that addresses LLM limitations like black-box nature and knowledge hallucination.
...
Method: CogRec synergizes LLMs with Soar cognitive architecture, using Soar as the symbolic reasoning engine and LLMs for knowledge initialization to populate working memory with production rules. The agent operates on a Perception-Cognition-Action cycle, dynamically querying LLMs when encountering impasses, then transforming LLM solutions into new symbolic production rules via Soar's chunking mechanism for online learning.

Result: Extensive evaluations on three public datasets demonstrate that CogRec shows significant advantages in recommendation accuracy, explainability, and effectiveness in addressing the long-tail problem.

Conclusion: CogRec successfully addresses complementary challenges of LLMs and cognitive architectures by creating a hybrid system that combines LLM knowledge acquisition with Soar's structured reasoning, enabling continuous knowledge evolution and interpretable recommendations.
...
<details>
  <summary>Details</summary>
Motivation: Current state-of-the-art LLMs fail to reliably solve interactive reasoning tasks in ARC-AGI-3 benchmark, which requires forming hypotheses, testing them, and tracking discovered mechanics through limited interactions in game-like environments with increasing complexity.
...
Conclusion: Explicit graph-structured exploration without learning serves as strong baseline for interactive reasoning, demonstrating importance of systematic state tracking and action prioritization in sparse-feedback environments where current LLMs fail to capture task dynamics.
...
<details>
  <summary>Details</summary>
Motivation: To accelerate scientific discovery by enabling global collaboration between autonomous AI agents and human researchers, overcoming barriers of disparate platforms, institutional boundaries, and integration overhead while enhancing reproducibility.

Method: SCP has two pillars: (1) Unified Resource Integration - a universal specification for describing/invoking scientific resources; (2) Orchestrated Experiment Lifecycle Management - a secure service architecture with centralized SCP Hub and federated SCP Servers for managing registration, planning, execution, monitoring, and archival.

Result: Built a scientific discovery platform with over 1,600 tool resources; facilitates secure, large-scale collaboration between heterogeneous AI systems and human researchers; significantly reduces integration overhead and enhances reproducibility.
...
TL;DR: RSA is a risk-aware alignment method that uses nested risk measures to control safety risks during LLM fine-tuning, addressing both excessive model shift and low-probability high-impact harmful behaviors.


<details>
  <summary>Details</summary>
Motivation: Existing safety alignment methods (like Safe RLHF and SACPO) operate under risk-neutral paradigms that are insufficient for addressing risks from policy deviations and offer limited robustness against rare but catastrophic harmful behaviors.

Method: Proposes Risk-aware Stepwise Alignment (RSA) that formulates safety alignment as a token-level risk-aware constrained policy optimization problem, solved through stepwise alignment with token-level policy updates derived from nested risk measures.
...
Conclusion: RSA provides an effective risk-aware alignment approach that mitigates risks from excessive model shift and explicitly suppresses low-probability high-impact harmful behaviors, with theoretical analysis supporting policy optimality.
...
TL;DR: A test-time adaptive agent that performs exploratory inference through posterior-guided belief refinement without gradient updates or additional training, using external structured beliefs and information gain maximization for action selection.


<details>
  <summary>Details</summary>
Motivation: To create an LLM agent that can operate effectively under partial observability without requiring gradient-based updates or additional training, enabling better alignment with latent world states through exploratory inference.

Method: The agent maintains an external structured belief over environment state, iteratively updates it via action-conditioned observations, selects actions by maximizing predicted information gain over belief space using a lightweight LLM-based surrogate, and assesses world alignment through a novel reward quantifying consistency between posterior belief and ground-truth environment configuration.

Result: The method outperforms inference-time scaling baselines such as prompt-augmented or retrieval-enhanced LLMs in aligning with latent world states, with significantly lower integration overhead.

Conclusion: The proposed test-time adaptive agent with posterior-guided belief refinement provides an effective approach for LLM agents to operate under partial observability without requiring gradient updates or additional training, achieving better world state alignment than existing inference-time methods.
...
TL;DR: The paper proposes JEPA-WMs, a family of world models that plan in learned representation space rather than input space, and conducts a comprehensive study to identify optimal technical choices for architecture, training objectives, and planning algorithms.


<details>
  <summary>Details</summary>
Motivation: To develop AI agents capable of solving diverse physical tasks and generalizing to unseen tasks/environments, addressing the challenge that planning in input space may be inefficient due to irrelevant details.

Method: Characterizes JEPA-WMs (world models planning in representation space), conducts systematic study of model architecture, training objectives, and planning algorithms using both simulated environments and real-world robotic data.
...
Conclusion: Planning in learned representation space (JEPA-WMs) with optimized technical choices enables more efficient planning and better performance on physical tasks compared to input-space planning approaches.
...
TL;DR: Interactive evaluation framework for analyzing foundation model agents' spatial understanding in symbolic map environments through exploration, memory, and reasoning components.
...
Result: Exploration affects experience acquisition but has limited impact on final reasoning accuracy. Memory representation is central for consolidating spatial experience, with structured memories (sequential and graph-based) substantially improving performance on structure-intensive tasks like path planning. Reasoning schemes shape how spatial knowledge is used, with advanced prompts supporting more effective multi-step inference. Spatial reasoning performance saturates beyond certain capability thresholds.
...
TL;DR: LLMs struggle with underrepresented math competition problems, especially geometry, with distinct error patterns across models.


<details>
  <summary>Details</summary>
Motivation: To analyze LLM performance on underrepresented mathematics competition problems beyond standard benchmarks, focusing on diverse challenges in mathematical reasoning.

Method: Tested three leading LLMs (GPT-4o-mini, Gemini-2.0-Flash, DeepSeek-V3) on Missouri Collegiate Mathematics Competition problems in Calculus, Analytic Geometry, and Discrete Mathematics, comparing responses to known correct solutions and analyzing reasoning patterns.
...
Conclusion: Evaluating LLMs on underrepresented math competition datasets reveals distinct error patterns and highlights ongoing challenges in structured reasoning, particularly in Geometry.
...
TL;DR: Two-stage approach for spatial reasoning in LLMs: first fine-tune on basic spatial transformations, then train LoRA adapters for multi-step planning in puzzle environments, outperforming baselines with faster convergence.


<details>
  <summary>Details</summary>
Motivation: LLMs have strong general language capabilities but struggle with spatial transformations and multi-step planning in structured environments, which limits applications in navigation and planning tasks.

Method: Two-stage approach: 1) Supervised fine-tuning on elementary spatial transformations (rotation, translation, scaling) to build basic spatial physics understanding; 2) Freeze physics-aware model and train lightweight LoRA adapters within GRPO framework for multi-step planning in puzzle environments. Dataset: synthesized ASCII-art dataset with corresponding ASCII-based RL environment.

Result: Method consistently outperforms baselines (generic backbone, physics-aware model, end-to-end RL models) in both Dynamic environments (explicit state updates) and Static environments (relying on internal state). Faster convergence and more stable training compared to end-to-end RL from scratch. Attention pattern analysis shows meaningful improvements in spatial understanding.

Conclusion: The proposed two-stage decomposition approach effectively addresses LLMs' spatial reasoning limitations by building atomic spatial building blocks first, then composing them for complex planning tasks, resulting in superior performance and training efficiency.
...
TL;DR: MCPAgentBench: A benchmark for evaluating LLM agents' tool-use capabilities using real-world MCP definitions, simulated tools, and dynamic sandbox testing with distractor tools.


<details>
  <summary>Details</summary>
Motivation: Current MCP evaluation sets have limitations: they rely on external MCP services and lack difficulty awareness. There's a need for better benchmarks to assess LLM agents' tool-use capabilities as they increasingly serve as autonomous agents.
...
Result: Experiments on various latest mainstream LLMs reveal significant performance differences in handling complex, multi-step tool invocations.

Conclusion: MCPAgentBench addresses limitations of current MCP evaluation sets and provides a comprehensive benchmark for evaluating LLM agents' tool-use capabilities, with all code open-sourced.
...
TL;DR: RLMs enable LLMs to process arbitrarily long prompts via recursive self-calling on prompt snippets, handling inputs 100x beyond context windows with better quality and comparable cost.
...
Method: Propose Recursive Language Models (RLMs) - an inference strategy where LLMs treat long prompts as external environments, programmatically examine, decompose, and recursively call themselves over prompt snippets.
...
Conclusion: RLMs provide an effective general inference strategy for processing arbitrarily long prompts, offering superior performance and scalability compared to existing approaches.
...
TL;DR: A reinforcement learning-augmented LLM framework for multi-agent collaboration using Dec-POMDP formulation with CTDE and GRPO optimization, achieving significant improvements in collaborative writing and coding tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs perform well individually but lack collaborative awareness and struggle to optimize global performance in multi-agent settings, creating a need for frameworks that enable effective cooperation between multiple LLM agents.
...
Result: Achieves 3x increase in task processing speed over single-agent baselines, 98.7% structural/style consistency in collaborative writing, and 74.6% test pass rate in collaborative coding. Consistently outperforms strong multi-agent LLM baselines.

Conclusion: The framework provides a practical path toward reliable collaboration in complex workflows by enabling effective multi-agent cooperation through reinforcement learning augmentation and optimized policy coordination.
...
TL;DR: A multi-agent conversational model with three-level role division (generation, verification, integration) improves complex reasoning accuracy by 16.8-19.2% on benchmark datasets through self-game mechanisms, retrieval enhancement, and composite reward optimization.


<details>
  <summary>Details</summary>
Motivation: To address limitations of single large language models in complex reasoning tasks by leveraging group deliberation through multi-agent collaboration.

Method: Three-level role division architecture: opinion generation agent produces diverse perspectives, evidence verification agent retrieves external knowledge and quantifies factual support, consistency arbitration agent integrates coherent conclusions. Includes self-game mechanism for multi-path reasoning, retrieval enhancement module, composite reward function (factual consistency + logical coherence), and improved proximal policy optimization for collaborative training.
...
Conclusion: The proposed group deliberation oriented multi-agent conversational model provides an effective and stable solution for complex reasoning tasks, outperforming single LLMs and existing multi-agent approaches in both accuracy and consistency.
...
TL;DR: Youtu-Agent is a modular LLM agent framework that automates agent generation and enables continuous evolution through workflow/meta-agent modes and hybrid policy optimization (in-context practice + RL).


<details>
  <summary>Details</summary>
Motivation: Address high configuration costs and static capabilities in existing LLM agent frameworks, where building quality agents requires extensive manual effort and deployed agents struggle to adapt without expensive fine-tuning.

Method: Proposes Youtu-Agent with structured configuration system decoupling execution environments, toolkits, and context management. Features two generation paradigms: Workflow mode for standard tasks and Meta-Agent mode for complex requirements. Implements hybrid policy optimization with Agent Practice module (in-context optimization) and Agent RL module (distributed reinforcement learning).

Result: Achieves SOTA on WebWalkerQA (71.47%) and GAIA (72.8%) with open-weight models. Automated generation achieves >81% tool synthesis success rate. Practice module improves AIME 2024/2025 by +2.7% and +5.4%. Agent RL achieves 40% speedup with 35% coding/reasoning and 21% searching improvements on benchmarks.

Conclusion: Youtu-Agent successfully addresses configuration and adaptability challenges in LLM agents through automated generation and continuous evolution, demonstrating strong performance across multiple benchmarks and practical applications.
...
TL;DR: Proposes a multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis, addressing performance decline under unseen working conditions and leveraging complementary multi-modal information.


<details>
  <summary>Details</summary>
Motivation: Existing fault diagnosis methods suffer performance decline under unseen working conditions, domain adaptation relies on target domain samples, and single-modal approaches overlook complementary multi-modal information.

Method: Dual disentanglement framework to separate modality-invariant/specific features and domain-invariant/specific representations, cross-domain mixed fusion for diversity augmentation, and triple-modal fusion for adaptive multi-modal integration.

Result: Extensive experiments on induction motor fault diagnosis under unseen constant and time-varying conditions show consistent outperformance over advanced methods, with ablation studies verifying component effectiveness.

Conclusion: The proposed multi-modal cross-domain mixed fusion model with dual disentanglement effectively improves fault diagnosis generalization under unseen working conditions by leveraging complementary multi-modal information and robust domain adaptation.
...
TL;DR: BatteryAgent is a hierarchical framework that integrates physical knowledge features with LLM reasoning for interpretable battery fault diagnosis, moving beyond black-box binary classification to provide root cause analysis and maintenance recommendations.
...
Method: The BatteryAgent framework has three core modules: 1) Physical Perception Layer extracts 10 mechanism-based features from electrochemical principles; 2) Detection and Attribution Layer uses Gradient Boosting Decision Trees and SHAP for feature contribution quantification; 3) Reasoning and Diagnosis Layer employs an LLM as agent core to create a "numerical-semantic" bridge combining SHAP attributions with mechanism knowledge base to generate comprehensive diagnostic reports.

Result: BatteryAgent achieves AUROC of 0.986, significantly outperforming state-of-the-art methods. It effectively corrects misclassifications on hard boundary samples and extends traditional binary detection to multi-type interpretable diagnosis.

Conclusion: The framework represents a paradigm shift from "passive detection" to "intelligent diagnosis" for battery safety management, offering interpretable fault diagnosis with root cause analysis and maintenance suggestions through integration of physical knowledge with LLM reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current robotic systems use latent preference models that lack interpretability, making it difficult to understand the factors guiding human decisions in object arrangement.

Method: 1) Defined four interpretable constructs: spatial practicality, habitual convenience, semantic coherence, and commonsense appropriateness. 2) Designed and validated a self-report questionnaire through a 63-participant online study. 3) Integrated these constructs into a Monte Carlo Tree Search (MCTS) planner.

Result: The study confirmed the psychological distinctiveness of the four constructs and their explanatory power across kitchen and living room scenarios. The MCTS planner guided by participant-derived preferences generated arrangements closely aligned with those created by participants.

Conclusion: The work contributes a compact, interpretable formulation of object arrangement preferences and demonstrates its operationalization for robot planning, addressing the interpretability limitations of current latent preference models.
...
TL;DR: GenZ is a hybrid model combining foundational models with statistical modeling using interpretable semantic features discovered through an iterative process that contrasts groups identified by statistical errors.


<details>
  <summary>Details</summary>
Motivation: Large language models have broad domain knowledge but often fail to capture dataset-specific patterns critical for prediction tasks. There's a need to bridge foundational models with statistical modeling through interpretable semantic features.
...
Result: On house price prediction: 12% median relative error using discovered semantic features from multimodal listing data, substantially outperforming GPT-5 baseline (38% error). For Netflix movie embeddings: predicts collaborative filtering representations with 0.59 cosine similarity purely from semantic descriptions, matching performance that would require ~4000 user ratings through traditional collaborative filtering.

Conclusion: GenZ successfully bridges foundational models with statistical modeling by discovering dataset-specific semantic features that capture patterns diverging from the model's domain knowledge alone, enabling interpretable predictions with strong performance across domains like real estate and recommendation systems.
...
TL;DR: Proposes a constraint extraction method using templates to automatically generate work schedules for long-term care facilities by interviewing managers to create facility-specific constraints while excluding exceptional cases.


<details>
  <summary>Details</summary>
Motivation: Long-term care facilities have varying conditions between facilities, making it essential to design facility-specific constraint conditions for work schedules rather than using generic solutions. Existing constraint extraction techniques lack mechanisms to handle exceptional constraints.

Method: Uses constraint templates to extract combinations of various components (shift patterns for consecutive days, staff combinations) by changing parameters like number of days and staff members. Incorporates mechanisms to exclude exceptional constraints, unlike existing techniques. Extracted constraints are then used by a constraint programming solver to create care worker schedules.

Result: Experiments demonstrated that the proposed method successfully created schedules that satisfied all hard constraints and reduced the number of violations for soft constraints by circumventing the extraction of exceptional constraints.

Conclusion: The template-based constraint extraction method with exceptional constraint exclusion mechanisms effectively generates customized work schedules for long-term care facilities, addressing facility-specific needs while avoiding problematic exceptional constraints.
...
TL;DR: ALE is an end-to-end ecosystem for developing agentic LLMs, featuring ROLL for weight optimization, ROCK for environment management, and iFlow CLI for context engineering, with ROME as an open-source agent trained on 1M+ trajectories using novel IPA algorithm.


<details>
  <summary>Details</summary>
Motivation: The open-source community lacks a principled, end-to-end ecosystem for developing agentic LLMs that can operate in real-world environments over multiple turns, requiring infrastructure to streamline agent development and production pipelines.

Method: ALE consists of three components: ROLL (post-training weight optimization framework), ROCK (sandbox environment manager for trajectory generation), and iFlow CLI (agent framework for efficient context engineering). The approach includes data composition protocols for synthesizing complex behaviors and a novel policy optimization algorithm called Interaction-based Policy Alignment (IPA) that assigns credit over semantic interaction chunks rather than individual tokens.

Result: ROME (an open-source agent grounded by ALE) was trained on over one million trajectories and demonstrates strong performance across benchmarks like SWE-bench Verified and Terminal Bench. The authors also introduce Terminal Bench Pro, a benchmark with improved scale and contamination control.

Conclusion: The ALE infrastructure effectively supports agentic LLM development, as proven by ROME's strong benchmark performance, providing a comprehensive ecosystem for the open-source community to develop and evaluate agentic models.
...
<details>
  <summary>Details</summary>
Motivation: Manual annotation of heterogeneous, multimodal driving data is costly and time-consuming, creating a need for more efficient annotation methods for autonomous vehicle research in Poland.
...
Result: Substantial time savings while ensuring consistent, high-quality annotations across different sensor modalities, accelerating preparation of large annotated datasets in standardized format.

Conclusion: The developed pipeline effectively supports the DARTS project by reducing annotation costs and duration, strengthening Poland's technological base for autonomous vehicle research.
...
TL;DR: Iterative deployment of LLMs fine-tuned on user-curated data from previous deployments leads to significant model property changes, particularly improved planning skills and emergent generalization to longer plans, functioning as implicit reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: To understand how iterative deployment of LLMs with user-curated fine-tuning data affects model properties over time, particularly examining whether this process leads to emergent capabilities and generalization.

Method: Testing iterative deployment mechanism across various planning domains, where each new model is fine-tuned on data carefully curated by users from previous model deployments, then analyzing the resulting model properties and providing theoretical analysis connecting the process to reinforcement learning.

Result: Substantial improvements in planning skills observed, with later models displaying emergent generalization by discovering much longer plans than initial models. Theoretical analysis shows iterative deployment effectively implements RL training with implicit reward function.

Conclusion: Iterative deployment with user-curated data functions as implicit RL training with important implications: 1) AI safety concerns due to undefined reward functions with unexpected implications, and 2) alternative training regime to explicit RL using data curation rather than explicit rewards.
...
TL;DR: STAgent is a specialized LLM agent for spatio-temporal tasks like POI discovery and itinerary planning, featuring tool interaction, hierarchical data curation, and cascaded training while maintaining general capabilities.


<details>
  <summary>Details</summary>
Motivation: To create a specialized agentic LLM that can handle complex spatio-temporal reasoning tasks while preserving general language capabilities, addressing the need for domain-specific agents that don't sacrifice broad functionality.

Method: Three key contributions: 1) Stable tool environment with 10+ domain-specific tools for asynchronous rollout/training, 2) Hierarchical data curation framework filtering high-quality data at 1:10,000 ratio, 3) Cascaded training recipe with seed SFT stage, second SFT on high-certainty queries, and RL stage on low-certainty data.

Result: STAgent shows promising performance on TravelBench while maintaining general capabilities across wide range of benchmarks, demonstrating effectiveness of the proposed agentic model approach.

Conclusion: The STAgent framework successfully creates a specialized spatio-temporal agent that preserves general capabilities through careful tool design, data curation, and staged training, offering a viable approach for developing domain-specific LLM agents.
...
<details>
  <summary>Details</summary>
Motivation: To address limitations in existing building energy management systems by leveraging LLMs' autonomous data analytics capabilities for context-aware energy management through natural language interaction.
...
Conclusion: The study formalizes assessment of LLM-based BEMS AI agents, identifies future research directions, and highlights the accuracy-efficiency trade-off. More complex tasks like cost estimation need improvement.
