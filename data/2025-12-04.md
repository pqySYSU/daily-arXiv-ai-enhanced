...
TL;DR: Deep learning model trained on synthetic+real images predicts camera calibration parameters from single images, overcoming traditional multi-image requirements.


<details>
  <summary>Details</summary>
Motivation: Traditional camera calibration requires multiple images of calibration objects from different orientations, which is often unavailable in public datasets. This creates a need for single-image calibration methods using deep learning.

Method: Developed comprehensive synthetic dataset using AILiveSim simulation platform with variations in focal length and lens distortion parameters. Trained ResNet-based deep learning network on this synthetic dataset (with small real image subset) to predict Brown-Conrady lens model parameters.

Result: Demonstrated that deep learning models trained on synthetic data can accurately predict camera and lens parameters from single images, enabling calibration without multiple calibration object images.

Conclusion: Deep learning approach using synthetic training data enables practical single-image camera calibration, overcoming limitations of traditional methods and benefiting applications like autonomous driving, robotics, and AR.
...
TL;DR: A two-stage LLM method that infers material composition from 3D point clouds by first predicting object semantics, then assigning materials to segments, achieving high plausibility without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Existing 3D shape datasets and models focus only on geometry while ignoring material properties that determine object appearance, creating a gap in comprehensive 3D understanding.
...
<details>
  <summary>Details</summary>
Motivation: Symbolic computer vision offers interpretable diagram understanding through logical rules and structured representations, which differs fundamentally from pixel-based approaches that focus on textures and colors. There's a need for learning paradigms that can parse diagrams into geometric primitives and their relationships while maintaining interpretability.

Method: Proposes a self-supervised symbolic auto-encoder that encodes diagrams into structured primitives (points, lines, shapes) and their interrelationships in latent space, then decodes them through an executable engine. Uses Symbolic Hierarchical Process Reward Modeling with hierarchical step-level parsing rewards to enforce geometric consistency. Introduces stabilization mechanisms to balance exploration and exploitation during reconstruction, and fine-tunes the encoder on downstream tasks to create a neuro-symbolic system.
...
Conclusion: The proposed symbolic auto-encoder with hierarchical reward modeling and stabilization mechanisms effectively integrates neural network reasoning with symbolic interpretability, demonstrating superior performance across multiple diagram understanding tasks compared to existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in deep learning with noisy labels, class ambiguity, and the need to robustly reject out-of-distribution or corrupted samples in a unified manner.

Method: Propose a drainage node added at the network output that reallocates probability mass toward uncertainty, preserving end-to-end training and differentiability, providing an escape route for ambiguous, anomalous, or noisy samples.

Result: Achieves up to 9% accuracy increase over existing approaches in high-noise regimes on CIFAR-10/100 with instance-dependent or asymmetric noise, matches or surpasses state-of-the-art on real-world datasets (mini-WebVision, mini-ImageNet, Clothing-1M), and shows qualitative denoising effects where drainage neuron absorbs corrupt data.

Conclusion: The drainage formulation provides an effective unified solution for handling noisy labels and ambiguous samples, with applications extending beyond classification to web-scale dataset cleaning, semi-supervised learning, and open-set problems.
...
Method: Used model-agnostic forensic evaluation pipeline to test three restoration approaches: 3D reconstruction (NextFace), 2D frontalization (CFR-GAN), and feature enhancement (CodeFormer).

Result: Naive application of restoration techniques degrades accuracy, but selective application of CFR-GAN combined with CodeFormer yields meaningful improvements.

Conclusion: Targeted, selective application of AI-driven restoration techniques can enhance facial recognition accuracy for challenging real-world images.
...
TL;DR: Flux4D is a simple, scalable framework for 4D reconstruction of large-scale dynamic scenes that directly predicts 3D Gaussians and motion dynamics in a fully unsupervised manner, enabling efficient reconstruction within seconds and strong generalization to unseen environments.


<details>
  <summary>Details</summary>
Motivation: Existing differentiable rendering methods like NeRF and 3DGS have scalability limitations and require annotations to decouple actor motion, while self-supervised methods are constrained by per-scene optimization and sensitivity to hyperparameter tuning.

Method: Flux4D directly predicts 3D Gaussians and their motion dynamics using only photometric losses and "as static as possible" regularization, learning to decompose dynamic elements from raw data without pre-trained models or foundational priors by training across many scenes.

Result: Experiments on outdoor driving datasets show Flux4D significantly outperforms existing methods in scalability, generalization, and reconstruction quality, enabling efficient reconstruction of dynamic scenes within seconds.

Conclusion: Flux4D provides a simple and scalable framework for 4D reconstruction of large-scale dynamic scenes that works in a fully unsupervised manner, scales effectively to large datasets, and generalizes well to unseen environments including rare and unknown objects.
...
TL;DR: Zero-shot object counting using multi-modal LLMs (GPT-4o and GPT-5) with only textual prompts, achieving performance comparable to state-of-the-art methods on FSC-147 dataset.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot object counting methods require annotated data and visual exemplars. LLMs have powerful reasoning and data understanding capabilities that could enable counting without supervision using only textual prompts.

Method: Leverage visual capabilities of multi-modal LLMs (GPT-4o and GPT-5) to perform object counting in zero-shot manner using only textual prompts, without any visual exemplars or supervision.

Result: Models achieve performance comparable to state-of-the-art zero-shot approaches on FSC-147 dataset, and in some cases even surpass them. Also evaluated on CARPK dataset with comparative analysis.

Conclusion: Multi-modal LLMs can effectively perform zero-shot object counting using only textual prompts, demonstrating their potential for unsupervised counting tasks without requiring visual exemplars or extensive annotated data.
...
<details>
  <summary>Details</summary>
Motivation: Learning-based denoisers need large paired datasets of clean-noisy images which are difficult to collect in low-light conditions. Noise synthesis can provide training data but existing methods rely on simplified models or large datasets.

Method: Proposes a practical noise synthesis method using only one noisy image and one dark frame per ISO. Uses Poisson distribution for signal-dependent noise and Fourier-domain spectral sampling algorithm to model signal-independent noise, generating diverse realizations that preserve spatial and statistical properties of real sensor noise.

Result: The method accurately models sensor noise without simplified parametric models or large clean-noisy image pairs. Leads to state-of-the-art performances on multiple low-light denoising benchmarks.

Conclusion: The proposed noise synthesis method is accurate, practical, and enables effective training of low-light denoisers with minimal data requirements, achieving superior denoising performance compared to existing approaches.
...
TL;DR: PixPerfect is a pixel-level refinement framework that eliminates artifacts in LDM-based image editing by using discriminative pixel space, artifact simulation, and direct refinement for seamless, high-fidelity results.


<details>
  <summary>Details</summary>
Motivation: Latent Diffusion Models (LDMs) produce pixel-level inconsistencies like chromatic shifts, texture mismatches, and visible seams in image inpainting and local editing. Existing solutions fail to fully eliminate these artifacts and lack generalization across different latent representations and tasks.

Method: PixPerfect uses three key components: 1) a differentiable discriminative pixel space that amplifies/suppresses subtle color and texture discrepancies, 2) a comprehensive artifact simulation pipeline exposing the refiner to realistic local editing artifacts during training, and 3) a direct pixel-space refinement scheme ensuring broad applicability across diverse latent representations and tasks.
...
Conclusion: PixPerfect delivers seamless, high-fidelity local edits across diverse LDM architectures and tasks, effectively addressing the pixel-level inconsistencies that plague current LDM-based image editing approaches.
...
TL;DR: Two-stage deep learning pipeline (PyroFocus) for real-time wildfire classification and intensity estimation from thermal imagery, achieving good speed-accuracy tradeoffs for edge deployment.


<details>
  <summary>Details</summary>
Motivation: Increasing wildfire frequency/severity requires low-latency, computationally efficient onboard detection methods for airborne/spaceborne missions. Current multispectral/hyperspectral thermal imagers provide rich data but face challenges with high dimensionality and limited onboard resources for real-time processing.

Method: Systematic evaluation of deep learning architectures (custom CNNs and Transformers) for multi-class fire classification, plus PyroFocus - a two-stage pipeline that performs fire classification followed by fire radiative power (FRP) regression/segmentation to reduce inference time and computational cost.
...
Conclusion: PyroFocus shows promise for enabling real-time wildfire detection and intensity estimation in resource-constrained airborne/spaceborne platforms, addressing critical needs for emergency response and environmental management.
...
TL;DR: H²U3D is a new 3D visual question answering dataset for house-scale scenes, and SpatialReasoner is an active perception framework that achieves state-of-the-art performance with efficient exploration using only 3-4 images on average.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models are limited to room-scale scenarios and struggle with spatial reasoning in large-scale 3D environments spanning entire houses with multiple floors.

Method: Created H²U3D dataset with automated annotation pipeline for hierarchical coarse-to-fine visual representations and chain-of-thought QA pairs. Proposed SpatialReasoner framework with two-stage training: supervised cold start followed by reinforcement learning with adaptive exploration reward.

Result: SpatialReasoner achieves state-of-the-art performance on H²U3D, outperforming GPT-4o and Gemini-2.5-Pro while using only 3-4 images on average compared to baselines requiring 16+ images.

Conclusion: The coarse-to-fine active exploration paradigm enables efficient spatial reasoning in large-scale 3D environments, demonstrating significant improvements over existing methods while requiring substantially fewer visual observations.
...
TL;DR: NavMapFusion is a diffusion-based framework that fuses low-fidelity navigation maps with high-fidelity sensor data for online HD map construction in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: HD maps are essential for autonomous driving but require online construction due to constant real-world changes. While navigation-grade SD maps are widely available, they lack sufficient resolution for direct use but can serve as coarse priors to guide online map construction.

Method: Proposes NavMapFusion, a diffusion-based framework that performs iterative denoising conditioned on both high-fidelity sensor data and low-fidelity navigation maps. The method treats discrepancies between prior maps and online perception as noise within the diffusion process, where consistent regions reinforce map construction while outdated segments are suppressed.
...
Conclusion: Diffusion-based map construction provides a robust framework for map fusion, generating accurate and up-to-date environment representations by fusing low-fidelity priors with high-fidelity sensor data, leading to safer and more reliable autonomous driving.
...
TL;DR: SLEDGE: A step-by-step layered design generation framework that models design updates as atomic changes based on sequential instructions, addressing the gap between single-step generation and the iterative nature of design processes.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat design synthesis as single-step generation, underestimating the complexity of the creative process where designers progressively refine work through careful modifications. There's a need to better model the iterative nature of design.

Method: Proposes Step-by-Step Layered Design Generation problem setting and SLEDGE framework that uses multi-modal LLMs to model each design update as an atomic, layered change over previous state, grounded in sequential instructions.

Result: Exhaustive experimental analysis and comparison with state-of-the-art approaches demonstrate the efficacy of SLEDGE. The paper also introduces a new evaluation suite including dataset and benchmark for this problem setting.

Conclusion: The work addresses a pragmatic and under-explored research area by modeling the step-by-step nature of design generation, and hopes to attract more attention to this important aspect of creative processes.
...
TL;DR: ProtoEFNet is a video-based prototype learning model for continuous ejection fraction regression that provides interpretable predictions by learning dynamic spatiotemporal prototypes of cardiac motion patterns.


<details>
  <summary>Details</summary>
Motivation: Traditional EF estimation requires manual tracing and expertise, making it time-consuming and variable. Current deep learning methods are black-box models with limited transparency, reducing clinical trust. Post-hoc explainability methods don't guide internal reasoning, limiting reliability.

Method: ProtoEFNet uses prototype learning for continuous EF regression, learning dynamic spatiotemporal prototypes that capture clinically meaningful cardiac motion patterns. It employs Prototype Angular Separation (PAS) loss to enforce discriminative representations across the continuous EF spectrum.
...
Conclusion: ProtoEFNet offers an interpretable alternative to black-box EF prediction models, maintaining accuracy while providing clinically meaningful explanations through learned cardiac motion prototypes, potentially increasing clinical trust and adoption.
...
TL;DR: HalluGen is a diffusion-based framework that synthesizes realistic hallucinations with controllable attributes to create the first large-scale hallucination dataset for safety-critical image restoration, enabling systematic evaluation and detection.


<details>
  <summary>Details</summary>
Motivation: Generative models in safety-critical image restoration (medical imaging, industrial inspection, remote sensing) produce hallucinations that undermine reliability, but evaluation is hindered by the circular dependency of needing labeled data that is costly and subjective to obtain.

Method: HalluGen uses a diffusion-based framework to synthesize realistic hallucinations with controllable type, location, and severity, producing perceptually realistic but semantically incorrect outputs. This enabled creation of a large-scale hallucination dataset (4,350 annotated images from 1,450 brain MR images).

Result: Created the first large-scale hallucination dataset for low-field MRI enhancement. Developed SHAFE (Semantic Hallucination Assessment via Feature Evaluation), a feature-based metric with soft-attention pooling that improves hallucination sensitivity over traditional metrics. Trained reference-free hallucination detectors that generalize to real restoration failures.

Conclusion: HalluGen and its open dataset establish the first scalable foundation for evaluating hallucinations in safety-critical image restoration, addressing the critical need for systematic assessment of generative model reliability in domains where hallucinations can lead to serious errors.
...
TL;DR: Hierarchical attention models outperform 2D/3D CNNs and ViTs for detecting subtle, sparse anomalies in 3D medical imaging, achieving 21-23% higher sensitivity/specificity for subclinical keratoconus detection.


<details>
  <summary>Details</summary>
Motivation: Current deep learning architectures have suboptimal inductive biases for detecting weak, spatially distributed anomalies in volumetric medical imaging. 2D/3D CNNs impose excessive locality, while ViTs use diffuse global attention, both failing to capture the intermediate spatial scale of early disease signals.

Method: Controlled comparison of 16 modern deep learning architectures (2D/3D CNNs, hybrid models, volumetric transformers) for subclinical keratoconus detection from 3D anterior segment OCT volumes. Mechanistic analyses include attention-distance measurements, representational similarity, and auxiliary age/sex prediction tasks.

Result: Hierarchical attention models achieved 21-23% higher sensitivity and specificity in the sparse anomaly regime compared to other architectures. These models produce effective receptive fields matched to the intermediate, multi-slice extent of subclinical abnormalities. Attention-distance measurements show spatial integration length shifts based on signal strength, with subclinical cases requiring longer integration than healthy or manifest disease states.
...
TL;DR: SeeU is a novel 2D→4D→2D framework that learns continuous 4D dynamics from sparse 2D observations to generate unseen visual content with physical consistency.


<details>
  <summary>Details</summary>
Motivation: Current visual understanding, prediction, and generation operate directly on 2D observations, leading to suboptimal performance. The authors propose to model the continuous 4D world (3D space + time) instead of discrete 2D projections.

Method: Three-step framework: 1) Reconstruct 4D world from sparse monocular 2D frames (2D→4D), 2) Learn continuous 4D dynamics using low-rank representation and physical constraints (discrete 4D→continuous 4D), 3) Roll world forward in time and re-project to 2D at sampled times/viewpoints with spatial-temporal context awareness (4D→2D).

Result: SeeU achieves continuous and physically-consistent novel visual generation, demonstrating strong potentials in multiple tasks including unseen temporal generation, unseen spatial generation, and video editing.

Conclusion: By modeling dynamics in 4D rather than operating directly on 2D observations, SeeU enables superior visual generation with physical consistency across various applications.
...
TL;DR: Deep learning system using DenseNet169 with attention mechanisms and SVM with MobileNetV2 achieves 98% accuracy for lung cancer classification from CT scans, with interpretability tools Grad-CAM and SHAP for clinical transparency.


<details>
  <summary>Details</summary>
Motivation: Lung cancer is highly lethal worldwide, and early diagnosis is critical for improving patient survival rates. Manual interpretation of CT scans is time-consuming and prone to human error, necessitating automated systems for more accurate and efficient diagnosis.

Method: The study uses the IQOTHNCCD lung cancer dataset with CT scans categorized as Normal, Benign, and Malignant. Two approaches are developed: 1) DenseNet169 with Squeeze-and-Excitation blocks for attention-based feature extraction, Focal Loss for handling class imbalance, and Feature Pyramid Network for multi-scale feature fusion; 2) SVM model using MobileNetV2 for feature extraction. For interpretability, Grad-CAM visualizes decision-making regions in CT scans, and SHAP explains feature contributions in the SVM model.

Result: Both DenseNet169 and SVM models achieved 98% accuracy, demonstrating high performance for lung cancer classification. The models show robustness suitable for real-world medical practice.

Conclusion: Deep learning approaches can significantly improve lung cancer diagnosis with high accuracy, transparency, and robustness. The integration of interpretability tools (Grad-CAM and SHAP) enhances clinical trust and adoption potential for automated diagnostic systems.
...
TL;DR: FireSentry introduces a high-resolution wildfire dataset and FiReDiff model for fine-grained fire spread prediction, achieving significant performance gains over existing methods.
...
Method: Created FireSentry dataset with sub-meter spatial and sub-second temporal resolution using synchronized UAVs, then developed FiReDiff - a dual-modality paradigm that predicts future infrared video sequences first, then segments fire masks based on generated dynamics.

Result: FiReDiff achieves state-of-the-art performance with video quality gains of 39.2% PSNR, 36.1% SSIM, 50.0% LPIPS, 29.4% FVD, and mask accuracy gains of 3.3% AUPRC, 59.1% F1, 42.9% IoU, and 62.5% MSE.

Conclusion: The FireSentry benchmark dataset and FiReDiff paradigm collectively advance fine-grained wildfire forecasting and dynamic disaster simulation, with the dataset publicly available.
...
TL;DR: ShelfGaussian is an open-vocabulary 3D scene understanding framework using Gaussian representations supervised by vision foundation models, enabling multi-modal perception without 3D annotations.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian-based methods have limitations: closed-set semantic Gaussians require 3D labels and neglect rendering ability, while open-set methods use only 2D self-supervision leading to degraded geometry and camera-only limitations. There's a need to fully exploit Gaussian potential for multi-modal scene understanding.

Method: Proposes Multi-Modal Gaussian Transformer allowing Gaussians to query features from diverse sensor modalities, and Shelf-Supervised Learning Paradigm that efficiently optimizes Gaussians with vision foundation model features at both 2D image and 3D scene levels.

Result: Achieves state-of-the-art zero-shot semantic occupancy prediction on Occ3D-nuScenes benchmark. Successfully evaluated on unmanned ground vehicle across diverse urban scenarios, demonstrating strong in-the-wild performance.

Conclusion: ShelfGaussian effectively bridges the gap between closed-set and open-set Gaussian methods, enabling open-vocabulary multi-modal 3D scene understanding without requiring 3D annotations, with practical applications in autonomous systems.
...
TL;DR: MOS is a novel framework for cross-modal ship re-identification between optical and SAR imagery that addresses the modality gap through denoising, alignment loss, and cross-modal data generation.


<details>
  <summary>Details</summary>
Motivation: Cross-modal ship re-identification between optical and SAR imagery is critical for maritime intelligence but underexplored. The substantial modality gap between optical and SAR images poses a major challenge for robust identification.

Method: MOS consists of two core components: (1) Modality-Consistent Representation Learning (MCRL) with denoise SAR image processing and class-wise modality alignment loss, and (2) Cross-modal Data Generation and Feature fusion (CDGF) using a brownian bridge diffusion model to synthesize cross-modal samples for feature fusion.

Result: Extensive experiments on HOSS ReID dataset show MOS significantly surpasses state-of-the-art methods with improvements of +3.0%, +6.2%, and +16.4% in R1 accuracy under ALL to ALL, Optical to SAR, and SAR to Optical settings respectively.

Conclusion: MOS effectively mitigates the optical-SAR modality gap and achieves modality-consistent feature learning for cross-modal ship re-identification, demonstrating superior performance over existing methods.
...
TL;DR: The paper introduces ViDiC (Video Difference Captioning) task and ViDiC-1K dataset to evaluate MLLMs' ability to describe similarities and differences between video pairs, addressing limitations of existing static image difference captioning approaches.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language systems lack capability for comparative perception of compositional, spatial, and temporal changes in dynamic scenes. Prior Image Difference Captioning (IDC) approaches fail to capture motion continuity, event evolution, or editing consistency over time in videos.

Method: Introduces ViDiC-1K dataset with 1,000 curated video pairs annotated with over 4,000 comparative checklist items across seven categories. Proposes dual-checklist evaluation framework using LLM-as-a-Judge protocol to separately measure similarity and difference accuracy.

Result: Experiments on nineteen representative multimodal models reveal significant performance gap in their comparative description and difference perception abilities, highlighting the challenge of the new benchmark.

Conclusion: ViDiC-1K serves as a challenging benchmark to advance video understanding, edit awareness, and comparative reasoning in multimodal intelligence, laying foundation for future research in video difference captioning.
...
TL;DR: YOLOA is a real-time affordance detection model that jointly handles object detection and affordance learning using an LLM adapter, achieving state-of-the-art accuracy while maintaining high frame rates.
...
Result: Achieves 52.8 mAP on ADG-Det and 73.1 mAP on IIT-Heat benchmarks while maintaining real-time performance up to 89.77 FPS (846.24 FPS for lightweight variant), showing excellent accuracy-efficiency trade-off.

Conclusion: YOLOA successfully addresses the "what-where-how" challenge in affordance detection by jointly handling object detection and affordance learning with real-time capability through LLM adapter refinement.
...
TL;DR: A label-efficient hyperspectral image classification framework using frozen diffusion model features with spectral-spatial fusion, achieving SOTA performance with sparse annotations.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging faces challenges of low spatial resolution and sparse annotations, requiring label-efficient methods that can leverage existing pretrained models for better feature extraction.

Method: Extracts low-level spatial features from frozen diffusion model pretrained on natural images, using early denoising timesteps. Introduces lightweight FiLM-based fusion module to adaptively combine spectral information with frozen spatial features for multimodal learning under sparse supervision.

Result: Outperforms state-of-the-art approaches on two recent hyperspectral datasets using only provided sparse training labels. Ablation studies confirm benefits of diffusion-derived features and spectral-aware fusion.

Conclusion: Pretrained diffusion models can support domain-agnostic, label-efficient representation learning for remote sensing and broader scientific imaging tasks, demonstrating effective transfer of spatial features to hyperspectral data.
...
TL;DR: A novel vision-language pretraining framework for medical imaging that addresses noisy web data and complex medical texts through multi-agent data generation and ontology-based knowledge enhancement, achieving SOTA zero-shot performance in dermatology tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLP methods struggle with noise in web-collected medical data and the complexity of unstructured long medical texts, limiting their effectiveness in medical image analysis applications.

Method: Proposes MAGEN (Multi-Agent data GENeration) for enhancing data quality via foundation model-assisted captioning and retrieval-based verification, and O-MAKE (Ontology-based Multi-Aspect Knowledge-Enhanced) pretraining that decomposes long texts into knowledge aspects for fine-grained alignment and ontology-guided concept modeling.

Result: Achieves state-of-the-art zero-shot performance on disease classification and cross-modal retrieval tasks across eight dermatology datasets, with the release of Derm1M-AgentAug dataset containing over 400k skin-image-text pairs.

Conclusion: The proposed framework effectively addresses challenges in medical VLP by improving data quality through multi-agent generation and enabling better learning from complex medical texts through ontology-based knowledge decomposition and alignment.
...
TL;DR: LM-CartSeg is an automatic pipeline for knee MRI cartilage/bone segmentation, lateral/medial compartmentalization, and radiomics analysis, achieving high accuracy and stable results across datasets.


<details>
  <summary>Details</summary>
Motivation: Existing knee MRI radiomics work relies on manual ROIs with poor quality control reporting. There's a need for robust, automatic segmentation that captures both cartilage and subchondral bone in anatomically meaningful compartments.

Method: Two 3D nnU-Net models trained on different datasets (SKM-TEA and OAIZIB-CM) with zero-shot predictions fused and refined using geometric rules: connected-component cleaning, 10mm subchondral bone bands, and PCA/k-means based tibial lateral/medial split.

Result: Post-processing improved segmentation accuracy significantly (ASSD: 2.63→0.36mm, HD95: 25.2→3.35mm, DSC: 0.91). Geometric L/M rule produced stable compartments across datasets. Only 6-12% of radiomic features correlated with volume/thickness, showing discriminative information beyond simple morphometry.

Conclusion: LM-CartSeg provides automatic, quality-controlled ROIs and radiomic features that carry discriminative information beyond simple morphometry, offering a practical foundation for multi-center knee OA radiomics studies.
...
TL;DR: Unsupervised framework learns structured 3D keypoints from point clouds to condition diffusion models for shape reconstruction, improving consistency by 6 percentage points.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised keypoint methods are not designed for unconditional generative settings, creating a gap in modern 3D generative pipelines. The paper aims to bridge this gap by learning interpretable 3D keypoints that can condition generative models.

Method: Unsupervised framework learns spatially structured 3D keypoints from point cloud data. These keypoints serve as a compact representation that conditions an Elucidated Diffusion Model (EDM) to reconstruct full shapes.

Result: The learned keypoints exhibit repeatable spatial structure across object instances and support smooth interpolation in keypoint space, capturing geometric variation. The method achieves strong performance across diverse object categories with 6 percentage-point improvement in keypoint consistency compared to prior approaches.

Conclusion: The framework successfully bridges the gap between unsupervised keypoint learning and modern 3D generative pipelines, providing interpretable structured representations that effectively condition diffusion models for shape reconstruction.
...
TL;DR: GalaxyDiT is a training-free acceleration method for video diffusion models that uses guidance alignment and proxy selection to achieve 1.87-2.37× speedup with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Diffusion models for video generation (especially transformer-based DiTs with classifier-free guidance) produce high-quality results but are computationally expensive, requiring dozens of iterative steps and doubled compute from CFG, limiting broader adoption.

Method: Training-free method using guidance alignment and systematic proxy selection for reuse metrics; identifies optimal proxy for each video model via rank-order correlation analysis across model families and parameter scales to ensure optimal computational reuse.

Result: Achieves 1.87× speedup on Wan2.1-1.3B and 2.37× speedup on Wan2.1-14B with only 0.97% and 0.72% drops on VBench-2.0; maintains superior fidelity at high speedup rates, exceeding prior SOTA by 5-10 dB in PSNR.
...
TL;DR: ThinkDeeper is a visual grounding framework for autonomous vehicles that uses a spatial-aware world model to reason about future scene states before making localization decisions, achieving state-of-the-art performance on multiple benchmarks.
...
Method: Proposes ThinkDeeper framework with: 1) Spatial-Aware World Model (SA-WM) that distills current scene into command-aware latent states and rolls out future latent states for forward-looking reasoning, 2) Hypergraph-guided decoder that hierarchically fuses these states with multimodal input to capture higher-order spatial dependencies, and 3) DrivePilot dataset created using RAG and CoT-prompted LLM pipeline for multi-source visual grounding in autonomous driving.

Result: ThinkDeeper ranks #1 on Talk2Car leaderboard and surpasses state-of-the-art baselines on DrivePilot, MoCAD, and RefCOCO/+/g benchmarks. Shows strong robustness and efficiency in challenging scenes (long-text, multi-agent, ambiguity) and retains superior performance even when trained on only 50% of the data.

Conclusion: The ThinkDeeper framework successfully addresses the limitations of existing visual grounding methods by incorporating future spatial reasoning through world models, demonstrating superior performance and robustness in autonomous driving scenarios. The approach represents a significant advancement in enabling autonomous vehicles to interpret complex natural language commands effectively.
...
TL;DR: This paper introduces geometric regularization losses for video generation by augmenting latent diffusion models with per-frame depth prediction to improve 3D structural consistency and temporal coherence.


<details>
  <summary>Details</summary>
Motivation: Current video generation approaches operate in 2D pixel space without explicit 3D structure modeling, resulting in temporal inconsistencies, implausible motions, and structural artifacts. There's a need to bridge the gap between appearance generation and 3D structure modeling.

Method: The method augments latent diffusion models with per-frame depth prediction and introduces a multi-view geometric loss that aligns predicted depth maps across frames within a shared 3D coordinate system to enforce structural consistency over time.

Result: Experiments across multiple datasets show the approach produces significantly more stable and geometrically consistent results than existing baselines, with improved spatio-temporal coherence, shape consistency, and physical plausibility.

Conclusion: The paper successfully bridges the gap between appearance generation and 3D structure modeling in video generation, demonstrating that geometric regularization through depth prediction leads to more coherent and physically plausible video synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Task-specific fine-tuning of LVLMs requires costly image-text pairs. Text is more available, editable, and scalable than images, but raw text training has limited gains due to the image-text modality gap.

Method: Proposes Text-Printed Image (TPI) - generating synthetic images by directly rendering textual descriptions on plain white canvases. This simple rendering projects text into image modality and preserves text semantics better than text-to-image models.
...
Conclusion: TPI highlights significant potential of text-centric training and charts a path toward fully automated data generation for LVLMs, offering scalable, low-cost alternative to image collection.
...
TL;DR: NAS-LoRA is a parameter-efficient fine-tuning method that integrates neural architecture search with LoRA to adapt SAM to specialized domains while reducing training costs by 24.14% without increasing inference overhead.


<details>
  <summary>Details</summary>
Motivation: SAM lacks spatial priors in its Transformer encoder, hindering acquisition of high-level semantic information for specialized domains like medical and agricultural imaging. Existing LoRA methods don't incorporate inductive bias needed for better adaptation.

Method: Proposes NAS-LoRA that adds a lightweight Neural Architecture Search block between encoder and decoder components of LoRA to dynamically optimize prior knowledge integration. Uses stage-wise optimization strategy to balance weight updates and architectural adjustments.

Result: NAS-LoRA improves existing PEFT methods while reducing training cost by 24.14% without increasing inference cost. Demonstrates better adaptation performance for specialized domains.

Conclusion: NAS-LoRA effectively bridges semantic gap between pre-trained SAM and specialized domains by incorporating NAS into PEFT, showing potential of NAS in enhancing visual foundation model adaptation.
...
TL;DR: PRIS is a framework that adaptively revises prompts during inference to improve text-to-visual generation by identifying failure patterns in generated content and redesigning prompts accordingly.


<details>
  <summary>Details</summary>
Motivation: Current text-to-visual generation approaches scale visual generation processes (more sampling steps/seeds) but hit quality plateaus because prompts remain fixed. There's a need to adapt prompts during inference to better align generated visuals with user intent.

Method: PRIS (Prompt Redesign for Inference-time Scaling) reviews generated visuals, identifies recurring failure patterns, and redesigns prompts accordingly before regenerating. It introduces element-level factual correction verifier for fine-grained alignment assessment between prompt attributes and generated visuals.

Result: Extensive experiments on text-to-image and text-to-video benchmarks show effectiveness, including 15% gain on VBench 2.0. Demonstrates that jointly scaling prompts and visuals is key to leveraging scaling laws at inference-time.

Conclusion: Adaptive prompt revision during inference (jointly scaling prompts and visuals) is crucial for improving text-to-visual generation alignment and overcoming quality plateaus in current scaling approaches.
...
TL;DR: Proposes Basis Decomposition Module (BDM) for infrared small target detection, extending to spatial/temporal difference modules, achieving SOTA results on single-frame and multi-frame datasets.


<details>
  <summary>Details</summary>
Motivation: Infrared small target detection faces challenges due to lack of target texture and severe background clutter, making targets difficult to distinguish from background noise.

Method: Develops Basis Decomposition Module (BDM) that decomposes features into basis features to enhance target information while eliminating redundancy. Extends BDM to create Spatial Difference Decomposition Module (SD²M), Spatial Difference Decomposition Downsampling Module (SD³M), and Temporal Difference Decomposition Module (TD²M). Builds SD²Net for single-frame detection using U-shaped architecture, and STD²Net for multi-frame detection by incorporating motion information via TD²M.

Result: Achieves state-of-the-art performance on both single-frame and multi-frame ISTD datasets. On multi-frame datasets, STD²Net achieves mIoU of 87.68%, significantly outperforming SD²Net's 64.97%.

Conclusion: The proposed basis decomposition approach effectively addresses infrared small target detection challenges by enhancing targets while suppressing background clutter, with temporal information providing substantial performance improvements for multi-frame detection.
...
TL;DR: CookAnything is a diffusion-based framework that generates coherent image sequences from cooking instructions of arbitrary length, addressing limitations of current methods in handling structured multi-step scenarios and variable recipe lengths.
...
Method: Introduces three key components: (1) Step-wise Regional Control (SRC) aligning textual steps with image regions in a single denoising process; (2) Flexible RoPE, a step-aware positional encoding mechanism enhancing temporal coherence and spatial diversity; (3) Cross-Step Consistency Control (CSCC) maintaining fine-grained ingredient consistency across steps.

Result: Experimental results on recipe illustration benchmarks show CookAnything performs better than existing methods in both training-based and training-free settings.

Conclusion: The framework supports scalable, high-quality visual synthesis of complex multi-step instructions and holds significant potential for broad applications in instructional media and procedural content creation.
...
TL;DR: AEM framework jointly models action execution and outcomes for mistake detection, achieving SOTA on EgoPER and CaptainCook4D benchmarks under one-class classification.


<details>
  <summary>Details</summary>
Motivation: Existing approaches focus only on how actions are performed while ignoring what they produce (action effects), but many errors manifest in outcomes rather than execution itself.

Method: Action Effect Modeling (AEM) uses probabilistic formulation to capture execution and outcomes: identifies effect frames, extracts visual grounding and symbolic scene graph cues, aligns them in shared latent space, and uses prompt-based detector with task-specific prompts.

Result: Achieves state-of-the-art performance on EgoPER and CaptainCook4D benchmarks under challenging one-class classification setting.
...
TL;DR: V-ITI: A lightweight framework that detects visual neglect in MLLMs via head-level activation patterns and intervenes only when needed to reduce hallucinations while preserving general performance.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs suffer from hallucinations due to visual neglect (failing to prioritize input images), and existing methods cause "over-intervention" by intervening at all times rather than only when needed.

Method: Proposes V-ITI with two components: 1) Visual Neglect Detector that identifies visual neglect using head-level discriminative probes, and 2) Visual Recall Intervenor that modulates activations with prestored visual activation information only when neglect is detected.
...
Conclusion: V-ITI addresses the "when to intervene" problem in MLLM hallucination mitigation, providing a lightweight, effective solution that avoids over-intervention and computational overhead.
...
TL;DR: Fairness-aware Low-Rank Adaptation (LoRA) for medical vision-language models reduces diagnostic accuracy disparities across demographic groups by 69% while maintaining performance, using only 0.24% trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Medical vision-language models show significant diagnostic accuracy disparities across demographic groups, creating fairness concerns in healthcare AI applications that need to be addressed while maintaining practical deployment feasibility.

Method: Three fairness-aware LoRA methods: FR-LoRA (MaxAccGap regularization), GR-LoRA (inverse frequency gradient weighting), and Hybrid-LoRA (combination). Uses differentiable MaxAccGap loss for end-to-end optimization of accuracy parity across groups.
...
Conclusion: The proposed fairness-aware LoRA approach enables practical deployment of fair medical AI in resource-constrained settings by requiring only 0.24% trainable parameters while significantly reducing demographic disparities in diagnostic accuracy.
...
Method: Hybrid framework with two pipelines: supervised classification for known violations and reference-based similarity matching for novel/subtle cases. Uses multimodal inputs (text, audio, visual) processed through both pipelines, with MLLM knowledge distillation to boost accuracy while keeping inference lightweight.
...
Conclusion: Demonstrates scalable and adaptable approach to multimodal content governance capable of addressing both explicit violations and emerging adversarial behaviors.
...
TL;DR: Object-centric reasoning benchmark for procedural videos with agent framework for state transition analysis


<details>
  <summary>Details</summary>
Motivation: Existing action-centric methods fail to handle flexible real-world procedures where step order depends on object states, requiring a shift to object-centric reasoning

Method: Introduce Object-IVQA benchmark with 107 instructional videos and 514 QA pairs, plus an agent framework with object-centric planning, perception, analysis and generation tools for explicit evidence retrieval and multi-hop reasoning
...
Conclusion: Object-centric paradigm with state transition focus and explicit reasoning tools enables better understanding of procedural activities compared to action-centric approaches
...
Method: EEA uses semantic guidance with hierarchical tree search, autonomously discovers task-relevant semantic queries, collects matching frames as semantic anchors, preferentially explores semantically relevant frames, and combines VLM intrinsic rewards with semantic priors using uncertainty modeling.
...
TL;DR: MKSNet introduces a Multi-Kernel Selection mechanism with large convolutional kernels and dual attention for improved small object detection in remote sensing imagery.


<details>
  <summary>Details</summary>
Motivation: Small object detection in remote sensing images faces challenges due to information loss in deep CNN layers, spatial redundancy, and complex backgrounds that obscure small targets.

Method: Proposes Multi-Kernel Selection Network (MKSNet) with: 1) Multi-Kernel Selection mechanism using large convolutional kernels to capture extensive contextual information with adaptive kernel size selection; 2) Dual attention mechanism combining spatial attention (fine-tuning spatial weights) and channel attention (optimizing channel information selection).
...
Conclusion: MKSNet effectively addresses small object detection challenges in remote sensing by managing multi-scale and high-resolution image complexities through adaptive kernel selection and attention mechanisms.
...
TL;DR: DPMFormer introduces domain-aware prompt learning and contrastive learning with texture perturbation to address semantic misalignment in domain generalized semantic segmentation, achieving SOTA results.
...
Method: Three key components: 1) Domain-aware prompt learning for semantic alignment, 2) Domain-aware contrastive learning with texture perturbation to diversify observable domains, 3) Domain-robust consistency learning to minimize prediction discrepancies between original and augmented images.
...
Conclusion: DPMFormer effectively addresses semantic misalignment in DGSS through domain-aware prompt learning and contrastive learning, creating a robust framework resilient to diverse environmental changes.
...
TL;DR: ToG-Bench is a new benchmark for task-oriented spatio-temporal video grounding in egocentric videos, focusing on identifying objects based on intended tasks rather than descriptions, with explicit-implicit dual grounding and one-to-many object matching.


<details>
  <summary>Details</summary>
Motivation: Existing STVG studies focus on object-centric descriptive instructions but neglect task-oriented reasoning crucial for embodied agents to accomplish goal-directed interactions. There's a need to bridge perception and interaction in embodied scenarios.

Method: Created ToG-Bench benchmark with 100 annotated clips from ScanNet videos containing 2,704 task-oriented grounding instructions. Used semi-automated pipeline combining foundation model annotation and human refinement. Introduced task-level evaluation metrics for multi-object and explicit-implicit grounding.

Result: Benchmarked seven state-of-the-art MLLMs, revealing intrinsic challenges of task-oriented STVG and substantial performance gaps across explicit-implicit and multi-object grounding scenarios.

Conclusion: ToG-Bench highlights the difficulty of bridging perception and interaction in embodied scenarios and provides a valuable benchmark for advancing task-oriented video grounding capabilities needed for general embodied intelligence.
...
TL;DR: CartoMapQA is a new benchmark for evaluating Visual-Language Models' ability to understand cartographic maps through question-answering tasks, revealing significant challenges in map interpretation.


<details>
  <summary>Details</summary>
Motivation: While Visual-Language Models (LVLMs) have advanced in integrating visual and textual information, their ability to interpret cartographic maps remains largely unexplored. There's a need to evaluate and improve LVLMs' map understanding capabilities for real-world applications like navigation and urban planning.

Method: The authors created CartoMapQA, a benchmark dataset with over 2000 samples, each containing a cartographic map, a question (open-ended or multiple-choice), and a ground-truth answer. The tasks cover low-, mid-, and high-level map interpretation skills including symbol recognition, embedded information extraction, scale interpretation, and route-based reasoning. They evaluated both open-source and proprietary LVLMs on this benchmark.

Result: Evaluation revealed persistent challenges: models frequently struggle with map-specific semantics, exhibit limited geospatial reasoning capabilities, and are prone to Optical Character Recognition (OCR)-related errors. The benchmark successfully isolates these weaknesses in current LVLMs.

Conclusion: CartoMapQA provides a valuable tool for guiding future improvements in LVLM architectures, supporting the development of models better equipped for real-world applications that depend on robust and reliable map understanding. The dataset and source code are publicly available to the research community.
...
TL;DR: A proof-of-concept framework using YOLOv8/v11 and SAM for automated dance movement analysis achieves ~94% precision/89% recall in detecting dancers and quantifying movements from video without specialized equipment.


<details>
  <summary>Details</summary>
Motivation: To develop automated dance movement analysis using contemporary computer vision techniques without requiring specialized equipment or markers, enabling quantitative tracking and analysis of dance performances.
...
Result: Achieved ~94% detection precision and 89% recall on a 49-second Ghanaian AfroBeats video; SAM segmentation achieved ~83% IoU; primary dancer executed 23% more steps with 37% higher motion intensity and used 42% more space than secondary dancers.

Conclusion: The framework demonstrates technical feasibility for automated dance analysis but represents early-stage work with limitations including single-video validation and lack of systematic ground truth; establishes foundation for future systematic validation studies.
...
TL;DR: BlackCAtt is a black-box adversarial attack method that uses minimal causal pixel sets to create explainable, imperceptible attacks on object detectors, outperforming baselines by 2.7-5.75x across different attack types.
...
Method: BlackCAtt combines causal pixels with bounding boxes to create adversarial attacks. It identifies minimal, causally sufficient pixel sets to construct attacks that lead to loss, modification, or addition of bounding boxes. The approach treats object detectors as black boxes and works across different architectures and sizes.

Result: On COCO test dataset, BlackCAtt outperforms baseline methods: 2.7x better at removing detections, 3.86x better at changing detections, and 5.75x better at triggering new spurious detections. Attacks are imperceptible and demonstrate the power of causal pixels.

Conclusion: BlackCAtt provides an effective black-box attack method that is explainable, imperceptible, reproducible, and architecture-agnostic. The use of causal pixels leads to more precisely targeted attacks, offering insights into attack mechanisms for better defense development.
...
TL;DR: Vision-based text compression (like DeepSeek-OCR) doesn't actually outperform simple alternatives for language modeling, despite excitement about optical context compression.


<details>
  <summary>Details</summary>
Motivation: To test whether vision-based compression (like DeepSeek-OCR) actually provides benefits for language modeling, beyond just text reconstruction, and to challenge assumptions about optical compression's superiority.

Method: Compared DeepSeek-OCR's vision encoder against simple alternatives: parameter-free mean pooling and a learned hierarchical encoder, testing both reconstruction quality and language modeling performance at matched compression ratios.

Result: Simple approaches match or surpass vision-based compression for text reconstruction, and outperform it for language modeling. Vision-based compression fails to beat simple truncation for language modeling tasks.

Conclusion: The excitement around optical context compression outpaces the evidence; vision-based compression doesn't provide unique advantages for language modeling compared to simpler alternatives.
...
<details>
  <summary>Details</summary>
Motivation: Manual brain tumor image classification is time-consuming and labor-intensive, and existing shallow CNN models have suboptimal accuracy. There's a need for more efficient and accurate automated classification methods.

Method: Proposes an improved ResNet34-based model with: 1) multi-scale input module as first layer, 2) Inception v2 module as residual downsampling layer, 3) channel attention mechanism to weight important features, and 4) multi-scale feature extraction.
...
Conclusion: The improved network achieves higher accuracy with fewer parameters, making it more efficient and effective for brain tumor image classification compared to manual methods and standard ResNet34.
...
TL;DR: CodeVision is a code-as-tool framework that enables multimodal LLMs to generate code as a universal interface for image operations, improving robustness to orientation changes and natural corruptions through two-stage training with SFT and RL.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs are brittle with significant performance degradation on images with simple orientation changes or natural corruptions, highlighting the need for more robust tool-based reasoning beyond narrow, fixed tool registries.

Method: Proposes CodeVision, a flexible code-as-tool framework where models generate code as a universal interface to invoke any image operation. Uses two-stage training: 1) SFT on high-quality dataset for complex multi-turn tool composition and error recovery, 2) RL with novel dense process reward function to encourage strategic and efficient tool use.

Result: Experiments on Qwen2.5-VL and Qwen3-VL series show significant performance improvements, with emergent capabilities including flexible tool composition, efficient chained execution, and robust error recovery from runtime feedback.

Conclusion: CodeVision provides a scalable solution for robust multimodal reasoning by using code as a universal tool interface, overcoming limitations of fixed tool registries and enabling more flexible, error-resilient visual reasoning.
...
### [51] [FloodDiffusion: Tailored Diffusion Forcing for Streaming Motion Generation](https://arxiv.org/abs/2512.03520)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for text-driven human motion generation rely on chunk-by-chunk or auto-regressive approaches with diffusion heads, which may not be optimal for streaming scenarios with time-varying text prompts. The authors aim to develop a framework that can generate text-aligned, seamless motion sequences with real-time latency.

Method: The authors adopt a diffusion forcing framework tailored for time-series generation under time-varying control events. They identify that vanilla diffusion forcing fails for motion generation and introduce three key improvements: (1) training with bi-directional attention instead of casual attention, (2) implementing a lower triangular time scheduler instead of random scheduling, and (3) using a continuous time-varying approach for text conditioning.
...
Conclusion: The diffusion forcing-based framework with the three proposed improvements successfully addresses the streaming human motion generation problem, demonstrating superior performance over existing methods while maintaining real-time capabilities.
...
TL;DR: AdaptVision is an efficient vision-language model that adaptively acquires visual tokens through a coarse-to-fine approach, using reinforcement learning with decoupled policy optimization to balance accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current VLMs have high computational overhead due to processing many visual tokens, and existing efficient methods use fixed-ratio compression without adapting to task requirements. The paper aims to enable VLMs to autonomously determine the minimum visual tokens needed per sample.

Method: AdaptVision uses a coarse-to-fine approach: initially processes compressed tokens from low-res images, then selectively invokes a bounding box tool to crop key regions when needed. Trained with reinforcement learning using Decoupled Turn Policy Optimization (DTPO), which separates learning into tool learning (correct tool use) and accuracy improvement (response refinement).
...
Conclusion: The proposed adaptive visual token acquisition paradigm enables VLMs to balance accuracy and efficiency effectively, outperforming existing methods by intelligently determining when to acquire additional visual information.
...
TL;DR: OpenTrack3D: A mesh-free framework for open-vocabulary 3D instance segmentation that uses visual-spatial tracking and MLLMs to overcome limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary 3D instance segmentation methods have two key limitations: (1) reliance on dataset-specific proposal networks or mesh-based superpoints, making them inapplicable in mesh-free scenarios and limiting generalization, and (2) weak textual reasoning of CLIP-based classifiers that struggle with compositional and functional user queries.

Method: OpenTrack3D uses a visual-spatial tracker to construct cross-view consistent object proposals online from RGB-D streams. It leverages 2D open-vocabulary segmentation masks lifted to 3D point clouds using depth, extracts mask-guided instance features using DINO feature maps, and fuses visual and spatial cues for instance consistency. The core pipeline is mesh-free with optional superpoints refinement when mesh is available. CLIP is replaced with a multi-modal large language model (MLLM) for enhanced compositional reasoning.

Result: Extensive experiments on diverse benchmarks (ScanNet200, Replica, ScanNet++, SceneFun3D) demonstrate state-of-the-art performance and strong generalization capabilities.

Conclusion: OpenTrack3D provides a generalizable and accurate framework for open-vocabulary 3D instance segmentation that works in mesh-free environments and handles complex user queries through improved compositional reasoning with MLLMs.
...
TL;DR: Stable Signer is a new sign language production model that simplifies the traditional pipeline by redefining SLP as a hierarchical end-to-end task with only text understanding and pose-to-video generation, achieving 48.6% improvement over SOTA.


<details>
  <summary>Details</summary>
Motivation: Previous SLP approaches suffer from accumulated errors across multiple stages (Text2Gloss, Gloss2Pose, Pose2Vid) due to inaccuracies in text conversion, pose generation, and video rendering. The field has made slow progress because of these cascading errors in the traditional redundant pipeline structure.

Method: 1. Streamlines traditional SLP pipeline to hierarchical end-to-end task with only text understanding (Prompt2Gloss, Text2Gloss) and Pose2Vid stages. 2. Introduces Sign Language Understanding Linker (SLUL) for text understanding. 3. Uses SLP-MoE hand gesture rendering expert block for pose-to-video generation. 4. Trains SLUL with Semantic-Aware Gloss Masking Loss (SAGM Loss).

Result: The proposed Stable Signer model achieves 48.6% performance improvement compared to current state-of-the-art generation methods in sign language production.

Conclusion: By simplifying the SLP pipeline to a hierarchical end-to-end approach and introducing specialized components (SLUL and SLP-MoE), Stable Signer significantly outperforms existing methods and addresses the error accumulation problem in traditional multi-stage approaches.
...
TL;DR: PULSE is a unified multi-task vision-language framework for cardiac image analysis that combines anatomical segmentation, disease classification, and clinical report generation in a single architecture, enabling generalization across imaging modalities and datasets.


<details>
  <summary>Details</summary>
Motivation: Current cardiac image analysis is fragmented across separate tasks (segmentation, classification, report generation) requiring different networks and data regimes, lacking a unified framework that can handle all objectives while generalizing across modalities and datasets.

Method: Built on self-supervised representations with composite supervision balancing region overlap learning, pixel-wise classification fidelity, and boundary-aware IoU refinement. Uses multi-scale token reconstruction decoder for segmentation and shared global representations for classification and clinical text generation.

Result: PULSE learns task-invariant cardiac priors, generalizes robustly across datasets, and can adapt to new imaging modalities with minimal supervision, moving toward a scalable foundation-style cardiac analysis framework.

Conclusion: PULSE represents a significant advancement toward a unified cardiac analysis framework that transitions from pixels to structures to clinical reasoning within a single architecture, overcoming fragmentation in current approaches.
...
TL;DR: BlurDM integrates blur formation process into diffusion models for dynamic scene deblurring, using dual-diffusion forward scheme and latent space implementation to enhance existing methods.
...
Method: BlurDM implicitly models blur formation through dual-diffusion forward scheme (diffusing both noise and blur), derives dual denoising and deblurring formulation for reverse process, and performs in latent space for efficient integration.
...
Conclusion: BlurDM successfully integrates blur formation into diffusion models, providing a flexible prior generation network that improves deblurring performance across multiple datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLM benchmarks focus on static, high-quality images and ignore critical failure modes in real-world deployments where transient visual corruption induces hallucinations that persist across subsequent frames in continuous visual streams.
...
Result: Experiments on 16 state-of-the-art VLMs show substantial robustness gaps: GPT-4o achieves only 78.5% recovery rate, while open-source models struggle with temporal consistency (<60%). UIR achieves 15.3% accuracy improvement for annotation.

Conclusion: DIQ-H provides a comprehensive platform for evaluating VLM reliability in real-world deployments, revealing critical robustness limitations in current models and enabling better assessment of temporal degradation handling.
...
Result: Extensive experiments on PartNet-Mobility dataset demonstrate effectiveness, achieving superior performance over previous methods in both qualitative and quantitative evaluations.

Conclusion: GAOT successfully bridges the gap between text prompts and articulated 3D object generation through a novel three-phase approach combining diffusion models and hypergraph learning.
...
TL;DR: The paper introduces Text Embedding Perturbation (TEP) for test-time scaling in text-to-image diffusion models, which enhances generative diversity and quality by complementing spatial noise with text embedding perturbations in the frequency domain.


<details>
  <summary>Details</summary>
Motivation: Current test-time scaling methods in text-to-image diffusion models focus on search strategies and reward models, but ignore the impact of the stochastic characteristics of noise on performance. The authors aim to explore how different forms of randomness affect generation quality and diversity.

Method: The method introduces step-based text embedding perturbation that combines frequency-guided noise schedules with spatial noise perturbation. It adapts perturbation intensity selectively based on frequency-specific contributions to generation and tolerance to perturbation, creating complementary effects in the frequency domain.

Result: The approach demonstrates significant improvements on multiple benchmarks with almost no additional computation. It can be seamlessly integrated into existing test-time scaling methods and enhances both generative diversity and quality.

Conclusion: Text embedding perturbation represents a new format of randomness that complements spatial noise in diffusion models, with spatial noise favoring low-frequency components and text embedding perturbation enhancing high-frequency details, leading to improved generation performance.
...
TL;DR: GLASTE is a new scene text editing framework that addresses inconsistency and length-insensitivity challenges by incorporating global-local awareness, style vectorization, and affine fusion for better text replacement in scene images.


<details>
  <summary>Details</summary>
Motivation: Existing scene text editing methods suffer from two major problems: inconsistency (edited patches don't blend well with surroundings) and length-insensitivity (poor handling of significant text length changes). These limitations prevent effective text replacement while preserving original style and background.

Method: Proposes GLASTE framework with: 1) Global-local combination structure for simultaneous high-level context and local feature incorporation, 2) Joint global and local losses for consistency, 3) Text style vectorization independent of image size, 4) Affine fusion to fill target text while maintaining aspect ratio.
...
Conclusion: GLASTE successfully addresses key challenges in scene text editing through its global-local aware design, style vectorization approach, and affine fusion technique, demonstrating superior performance over existing methods.
...
TL;DR: DIG is a training-free frame selection framework for long-form video understanding that adapts between uniform sampling for global queries and query-aware selection for localized queries, outperforming existing methods while scaling to 256 frames.


<details>
  <summary>Details</summary>
Motivation: Current LMMs for long-form video understanding face limitations due to context length constraints and high computational costs of processing dense video tokens. Existing query-aware frame selection methods incur significant overhead, but the paper questions whether such complex mechanisms are always necessary.

Method: The paper first identifies a query typology distinguishing global vs. localized queries. It then proposes DIG: a training-free framework that uses efficient uniform sampling for global queries, and activates a specialized pipeline to extract query-relevant frames for localized queries.
...
Conclusion: Complex query-aware frame selection is not universally necessary; adaptive strategies based on query type (global vs. localized) provide both efficiency and effectiveness for long-form video understanding with LMMs.
...
TL;DR: UniComp is a video compression framework that uses information uniqueness to maximize visual fidelity under computational constraints, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current attention-based compression methods have limitations. The paper aims to develop a compression framework that maximizes information fidelity of video representations under constrained computational budgets from an information-theoretic perspective.

Method: Formulates compression as minimizing conditional entropy between retained and full tokens. Introduces information uniqueness to measure intrinsic redundancy. Designs three modules: Frame Group Fusion (semantic frame grouping), Token Allocation (adaptive resource allocation), and Spatial Dynamic Compression (fine-grained spatial compression).
...
Conclusion: Information uniqueness plays a pivotal role in token compression efficacy, and UniComp demonstrates superior performance in video compression by maximizing information fidelity under computational constraints.
...
TL;DR: This paper investigates whether treating sketches as sequences is justified and examines which internal orders matter most for sketch representation learning, finding that absolute coordinates outperform relative ones and non-autoregressive decoders beat autoregressive ones.


<details>
  <summary>Details</summary>
Motivation: Despite advances in sketch representation learning, there's still uncertainty about whether the temporal aspect of sketches (their sequential nature) is truly relevant to representation quality, and which specific ordering aspects matter most.

Method: The study investigates treating sketches as sequences and compares different ordering approaches, examining traditional positional encodings, absolute vs. relative coordinates, and autoregressive vs. non-autoregressive decoders.

Result: Absolute coordinates consistently outperform relative ones for modeling sketches as sequences, non-autoregressive decoders outperform autoregressive counterparts, and the importance of temporality depends on both the order considered and the specific task being evaluated.

Conclusion: While treating sketches as sequences with traditional positional encodings is valid, absolute coordinates work better than relative ones, non-autoregressive approaches are superior, and temporal importance varies by order and task - suggesting a nuanced relationship between sketch temporality and representation quality.
...
TL;DR: CSCL is a two-stage pretraining framework that uses contrastive learning and multiple instance learning to create universal, transferable WSI representations by aligning H&E with IHC stains, addressing inter-stain misalignment issues.


<details>
  <summary>Details</summary>
Motivation: Universal WSI representations are crucial for computational pathology. While multi-stain data (H&E + IHC) provides richer biological information, progress is limited by scarce well-aligned datasets and inter-stain misalignment issues that degrade feature quality.

Method: Two-stage framework: 1) Patch-wise contrastive alignment with lightweight adapter to improve H&E-IHC compatibility, 2) Slide-level representation learning with MIL using cross-stain attention fusion and global alignment modules to integrate stain-specific features and enforce consistency.

Result: Experiments on cancer subtype classification, IHC biomarker status classification, and survival prediction show consistent gains, producing high-quality, transferable H&E slide-level representations.

Conclusion: CSCL successfully addresses inter-stain misalignment and creates universal WSI representations by leveraging a curated five-stain dataset, demonstrating improved performance across multiple pathology tasks with transferable H&E features.
...
TL;DR: PSA introduces multi-level pooled KV representations for efficient attention, replacing binary masks with adaptive pooling levels to reduce information loss while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard attention mechanisms have quadratic complexity, limiting scalability. Current sparse attention methods use binary masks that discard entire key-value blocks, causing significant information loss at high sparsity levels.

Method: Pyramid Sparse Attention (PSA) replaces binary masking with multi-level pooled KV representations. Each query block dynamically allocates lower pooling levels to critical KV blocks and higher levels to less important ones, creating a continuum between full retention and complete pruning. Uses hardware-friendly kernel with decoupled block-tile design.

Result: PSA outperforms or achieves comparable performance to existing sparse attention baselines across video understanding and generation benchmarks, with superior efficiency-quality trade-offs while preserving contextual information and visual fidelity.

Conclusion: PSA effectively mitigates information loss in sparse attention mechanisms through adaptive multi-level pooling, offering a versatile solution for efficient attention in both video understanding and generation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Need for quick, easy bot identification methods in surveys and online processes that leverage human perceptual advantages over algorithmic processing.

Method: Display hidden numbers with same pixel texture as background; motion and scale differences make numbers perceptible to humans across frames but not to frame-by-frame algorithmic analysis.

Result: State-of-the-art multimodal AI models fail to extract correct values; 99.5% of human participants solved task in 10.7 seconds average with no negative usability effects.

Conclusion: DOT-BI effectively differentiates humans from bots using motion perception, is user-friendly, and shows promise for adoption in surveys and online processes.
...
Conclusion: BBF successfully addresses limitations of existing methods by providing a flexible, context-aware framework that can handle complex motion patterns and audio-visual synchronization through multimodal conditioning and progressive training.
...
TL;DR: This thesis presents innovations in generative models: improving normalizing flow efficiency with 6 architectural advances, and applying generative models to 5 real-world computer vision problems including agricultural quality assessment, geological mapping, privacy preservation, and art restoration.
...
Method: Two-part approach: 1) Six architectural improvements to normalizing flows including invertible convolution layers, Quad-coupling layers, parallel inversion algorithms, and Affine-StableSR super-resolution model. 2) Application of generative models to five practical problems using Conditional GANs for agricultural assessment, stacked autoencoders for geological mapping, face detection/inpainting for privacy preservation, and diffusion models for art restoration.

Result: Developed more efficient normalizing flow architectures with proven invertibility conditions and reduced parameter counts. Successfully applied generative models to achieve good accuracy in seed purity testing, improved feature extraction for geological mapping, effective privacy preservation for autonomous driving data, and unified handling of multiple degradation types in art restoration.

Conclusion: The thesis demonstrates significant advancements in both the theoretical efficiency of normalizing flows and their practical application across diverse computer vision domains, showing that generative models can effectively address real-world challenges while maintaining performance and addressing issues like data scarcity, privacy concerns, and computational efficiency.
...
TL;DR: HyperRNA is a generative model using hypergraphs and encoder-decoder architecture to solve RNA inverse folding - designing nucleotide sequences that fold into desired secondary structures.


<details>
  <summary>Details</summary>
Motivation: RNA inverse folding is crucial for RNA design but challenging due to complex sequence-structure relationships. Existing methods need improvement for better sequence generation that ensures proper folding into target structures.

Method: Three-stage framework: 1) Preprocessing constructs graph structures from RNA backbone atom coordinates using 3-bead coarse-grained representation; 2) Encoding captures higher-order dependencies and biomolecular interactions via attention embedding and hypergraph-based encoder; 3) Decoding generates RNA sequences autoregressively.
...
TL;DR: CloseUpAvatar is a novel articulated human avatar representation that maintains rendering quality for close-up views while handling general camera motions, using textured planes with adaptive texture switching based on camera distance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create an articulated human avatar representation that can handle more general camera motions while preserving rendering quality for close-up views, addressing limitations of previous approaches that struggle with wide camera orientation ranges.

Method: The method represents avatars as textured planes with two sets of learnable textures (low and high-frequency). It automatically switches to high-frequency textures for close cameras and gradually reduces their impact as cameras move farther away, adjusting rendering quality based on camera distance.

Result: Experiments on the ActorsHQ dataset show both qualitative and quantitative improvements over existing methods in rendering from novel wide-range camera positions, while maintaining high FPS by limiting required primitives.

Conclusion: CloseUpAvatar enables realistic avatar rendering across a wider range of camera orientations than previous approaches, with adaptive quality based on camera distance and efficient performance.
...
TL;DR: HBFormer is a Hybrid-Bridge Transformer architecture that combines U-shaped encoder-decoder with Swin Transformer backbone and introduces a Multi-Scale Feature Fusion decoder to better integrate local details with global context for medical image segmentation, achieving state-of-the-art results on challenging datasets.


<details>
  <summary>Details</summary>
Motivation: Current Vision Transformers with shifted window-based self-attention struggle to effectively fuse local details with global context, which is particularly detrimental for challenging segmentation tasks like microtumors and miniature organs where both fine-grained boundary definition and broad contextual understanding are crucial.
...
Result: Comprehensive experiments on challenging medical image segmentation datasets (multi-organ, liver tumor, and bladder tumor benchmarks) demonstrate that HBFormer achieves state-of-the-art results, showing outstanding capabilities in microtumor and miniature organ segmentation.

Conclusion: HBFormer effectively addresses the limitation of existing Vision Transformers by creating a powerful feature bridge that captures long-range dependencies and refines object boundaries, making it particularly effective for challenging medical segmentation tasks requiring both local detail and global context integration.
...
<details>
  <summary>Details</summary>
Motivation: Partial dental point clouds often have large missing regions due to occlusion and limited scanning views, which bias encoder-only global features and force decoders to hallucinate structures rather than accurately reconstruct missing areas.

Method: Proposes a retrieval-augmented framework that integrates a prototype memory into standard encoder-decoder pipelines. After encoding partial input into a global descriptor, the model retrieves the nearest manifold prototype from a learnable memory and fuses it with query features through confidence-gated weighting before decoding. The memory self-organizes into reusable tooth-shape prototypes without requiring tooth-position labels.

Result: Experiments on the self-processed Teeth3DS benchmark demonstrate consistent improvements in Chamfer Distance, with visualizations showing sharper cusps, ridges, and interproximal transitions compared to baseline methods.

Conclusion: The approach provides a simple yet effective way to exploit cross-sample regularities for more accurate and faithful dental point-cloud completion. The module is plug-and-play and compatible with common completion backbones while keeping the same training losses.
...
<details>
  <summary>Details</summary>
Motivation: 2D foundation models for monocular video analysis lack 3D consistency, causing spatial misalignment and temporal flickering in complex 3D environments, limiting their ability to understand scene geometry and motion.

Method: Two-part iterative optimization: 1) Sequential optimization updates motion and semantic fields in stages for local consistency; 2) Global optimization jointly refines all attributes for long-term coherence. Uses 3D confidence maps to adjust motion priors, adaptive resampling for under-represented regions, and iterative semantic refinement with SAM2 prompts.
...
TL;DR: LAMP uses LLMs as motion planners to translate natural language into 3D trajectories for objects and cameras via a motion DSL, enabling improved motion controllability from text descriptions.


<details>
  <summary>Details</summary>
Motivation: Existing video generation interfaces lack sophisticated motion control for complex cinematic scenes. While text, layout, and motion conditioning exist, motion control for object dynamics and camera trajectories remains limited despite being essential for scene composition.

Method: LAMP leverages LLMs as motion planners with a motion domain-specific language (DSL) inspired by cinematography. It uses LLMs' program synthesis capabilities to generate structured motion programs from natural language, which are deterministically mapped to 3D trajectories for objects and cameras. A large-scale procedural dataset pairs text descriptions with motion programs and trajectories.

Result: LAMP demonstrates improved performance in motion controllability and alignment with user intent compared to state-of-the-art alternatives. It establishes the first framework for generating both object and camera motions directly from natural language specifications.

Conclusion: LAMP successfully bridges natural language descriptions to explicit 3D motion trajectories using LLMs as motion planners, advancing motion controllability in video generation and enabling more complex cinematic scene composition from text.
...
TL;DR: ReCamDriving is a vision-based framework for generating novel-trajectory driving videos using camera control and 3D Gaussian Splatting renderings for geometric guidance.


<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations: repair-based approaches fail to restore complex artifacts, and LiDAR-based methods rely on sparse, incomplete cues. There's a need for precise camera-controllable video generation with explicit geometric guidance.

Method: Two-stage training: first stage uses camera poses for coarse control; second stage incorporates dense 3DGS renderings for fine-grained viewpoint and geometric guidance. Includes 3DGS-based cross-trajectory data curation strategy to eliminate train-test gap in camera transformations.

Result: Achieves state-of-the-art camera controllability and structural consistency. Created ParaDrive dataset with over 110K parallel-trajectory video pairs. Demonstrates superior performance through extensive experiments.

Conclusion: ReCamDriving provides an effective framework for camera-controllable novel-trajectory video generation using 3DGS renderings, overcoming limitations of previous methods and enabling scalable multi-trajectory supervision from monocular videos.
...
TL;DR: MSVP introduces a multi-scale visual prompting method that learns global, mid-scale, and local prompt maps fused with input images via lightweight 1×1 convolution, improving performance on small-image benchmarks like MNIST, Fashion-MNIST, and CIFAR-10 with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Prior visual prompting research focuses on large Vision Transformers and high-resolution datasets like ImageNet, while small-image benchmarks (MNIST, Fashion-MNIST, CIFAR-10) widely used in education, prototyping, and research have received little attention for prompting techniques.

Method: Multi-Scale Visual Prompting (MSVP) learns global, mid-scale, and local prompt maps that are fused with input images using lightweight 1×1 convolution. The method is backbone-agnostic, adds less than 0.02% parameters, and works with CNN and Vision Transformer architectures.

Result: MSVP yields consistent performance improvements across MNIST, Fashion-MNIST, and CIFAR-10 benchmarks using simple CNN, ResNet-18, and small Vision Transformer backbones, with negligible computational overhead. Ablation studies on prompt scales, fusion strategies, and architectures demonstrate effectiveness.

Conclusion: Multi-scale prompting provides an effective inductive bias even on low-resolution images, offering a simple, generic, and efficient approach to adapt vision models for small-image benchmarks with minimal parameter addition.
...
TL;DR: Colon-X introduces ColonVQA (largest multimodal colonoscopy dataset) and ColonReason (clinical reasoning dataset), develops ColonR1 model that outperforms fine-tuning by 25.22% under data-scarce conditions.


<details>
  <summary>Details</summary>
Motivation: Advance multimodal intelligence in colonoscopy by addressing the critical gap between multimodal understanding and clinical reasoning, as current MLLMs lack robustness and trustworthiness for clinical applications.

Method: 1) Built ColonVQA dataset with 1.1M+ VQA entries across 76 clinical findings; 2) Evaluated 22 MLLMs for generalizability and reliability; 3) Created ColonReason dataset via multi-expert debating; 4) Developed ColonR1 model with task-adaptive rewarding and gradient-stable optimization.

Result: ColonR1 achieves 56.61% overall accuracy under data-scarce conditions, outperforming supervised fine-tuning by 25.22%, establishing new reasoning-enabled baseline for multimodal colonoscopy analysis.

Conclusion: Colon-X provides comprehensive data foundation and advances clinical reasoning capabilities in colonoscopy, with publicly available resources to support community-wide research in multimodal medical AI.
...
TL;DR: ConvRot enables efficient 4-bit quantization for diffusion transformers using group-wise rotation to suppress outliers, achieving 2.26× speedup and 4.05× memory reduction while maintaining image quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion transformers face deployment challenges due to increasing memory footprint and inference latency as model size grows. Existing rotation-based quantization methods from LLMs have substantial overhead and struggle with row-wise outliers in diffusion transformers.
...
Conclusion: ConvRot effectively addresses deployment challenges in diffusion transformers by enabling efficient 4-bit quantization with minimal quality loss, making large diffusion models more practical for real-world applications.
...
TL;DR: GaussianBlender is a feed-forward framework for instant text-driven 3D stylization that learns disentangled geometry/appearance representations from 3D Gaussians and applies text-conditioned edits via latent diffusion, achieving high-fidelity results without per-asset optimization.


<details>
  <summary>Details</summary>
Motivation: Current text-to-3D stylization methods require time-intensive per-asset optimization and suffer from multi-view inconsistency due to limitations in 2D image editors, making them impractical for large-scale production in game development, VR, and digital arts.

Method: Learns structured, disentangled latent spaces from spatially-grouped 3D Gaussians with controlled information sharing for geometry and appearance, then uses a latent diffusion model to apply text-conditioned edits on these learned representations in a feed-forward manner.

Result: Delivers instant, high-fidelity, geometry-preserving, multi-view consistent stylization that surpasses methods requiring per-instance test-time optimization, enabling practical, democratized 3D stylization at scale.

Conclusion: GaussianBlender represents a pioneering feed-forward framework that unlocks practical, large-scale 3D stylization by overcoming the limitations of existing optimization-based methods through structured latent learning and text-conditioned diffusion editing.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to explore active visual perception as a superior approach to passive systems, particularly for complex environments where static sensing is insufficient, with applications in robotics, autonomous vehicles, HCI, and surveillance.

Method: The paper provides a comprehensive overview and analysis of active visual perception systems, examining their capabilities to direct attention, move sensors, and interact with objects to acquire more informative data.

Result: The analysis identifies both the significant promise of active visual perception and key challenges including real-time processing of complex visual data, decision-making in dynamic environments, and integrating multimodal sensory inputs.

Conclusion: Active visual perception offers powerful capabilities for dynamic environment interaction but requires overcoming technical challenges in processing, decision-making, and multimodal integration for broader adoption across various applications.
...
TL;DR: SUSS is a new perceptual similarity score that models images as structured multivariate Normal distributions trained generatively, achieving human-aligned similarity judgments with interpretable explanations.


<details>
  <summary>Details</summary>
Motivation: Existing perceptual similarity measures have limitations: deep perceptual losses like LPIPS use complex, non-linear features with unknown invariances, while hand-crafted measures like SSIM are interpretable but miss key perceptual properties. There's a need for a method that combines human alignment with interpretability.

Method: SUSS models each image through perceptual components represented as structured multivariate Normal distributions. It trains these components generatively and self-supervised to assign high likelihood to human-imperceptible augmentations. The final score is a weighted sum of component log-probabilities with weights learned from human perceptual datasets. Unlike feature-based methods, SUSS learns image-specific linear transformations of residuals in pixel space.

Result: SUSS aligns closely with human perceptual judgments, shows strong perceptual calibration across diverse distortion types, and provides localized, interpretable explanations through decorrelated residuals and sampling. It demonstrates stable optimization behavior and competitive performance when used as a perceptual loss for downstream imaging tasks.

Conclusion: SUSS offers a novel approach to perceptual similarity that combines human alignment with interpretability, addressing limitations of both deep feature-based methods and hand-crafted measures through its generative, structured modeling framework.
...
TL;DR: DINO-RotateMatch combines dataset-adaptive image pairing using DINO with rotation-aware keypoint matching (ALIKED + Light Glue) for improved large-scale 3D reconstruction from Internet images.


<details>
  <summary>Details</summary>
Motivation: Address challenges of image matching in large-scale 3D reconstruction from unstructured Internet images, particularly handling orientation variations and efficiently pairing relevant images in large collections.

Method: Integrates dataset-adaptive image pairing strategy using DINO for semantic retrieval of relevant image pairs, with rotation-aware keypoint extraction and matching using rotation-based augmentation, ALIKED for keypoint detection, and Light Glue for matching.
...
Conclusion: Combining self-supervised global descriptors (DINO) with rotation-enhanced local matching provides a robust and scalable solution for large-scale 3D reconstruction from Internet images.
...
TL;DR: PosA-VLA improves Vision-Language-Action models by using pose-conditioned supervision to anchor visual attention to task-relevant regions, reducing redundant actions and improving precision in embodied tasks.
...
Method: Proposes PosA-VLA framework with pose-conditioned supervision to anchor visual attention, guiding perception toward task-relevant regions without needing auxiliary perception modules like segmentation or grounding networks.
...
Conclusion: The pose-conditioned anchor attention mechanism enables better alignment between instruction semantics and actionable visual cues, improving action generation precision and efficiency for real-world VLA applications.
...
TL;DR: SelfDebias is an unsupervised test-time debiasing method for diffusion models that identifies semantic clusters in embedding space to guide generation toward uniform distributions, reducing biases without requiring labeled data.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models trained on large internet datasets (like LAION-5B) inherently learn and reproduce societal biases present in the training data, resulting in stereotypical outputs. Current approaches often require supervised data or external classifiers.

Method: SelfDebias uses an image encoder's embedding space to identify semantic clusters, then guides the diffusion process during inference by minimizing KL divergence between output distribution and uniform distribution. Works with any UNet-based diffusion model without requiring human annotations or external classifiers.
...
Conclusion: SelfDebias provides a fully unsupervised, generalizable solution for debiasing diffusion models that doesn't require labeled data or external classifiers, effectively reducing biases while preserving image quality across various concepts and model architectures.
...
<details>
  <summary>Details</summary>
Motivation: VAR models accelerate image generation through parallel token sampling within scales, but this can cause structural errors and suboptimal image quality due to error accumulation across scales.

Method: LSRS uses a lightweight scoring model to evaluate multiple candidate token maps at each scale during inference, selecting high-quality maps to guide subsequent scale generation, prioritizing early scales for structural coherence.

Result: LSRS improves VAR-d30 model's FID from 1.95 to 1.78 with only 1% additional inference time, and to 1.66 with 15% additional time, significantly enhancing generation quality with minimal overhead.
...
Method: Three-stage approach: 1) Segmentation using Contour and Detectron2, 2) Mapping symbols to Gardiner codes, 3) Translation using CNN model. Uses Morris Franken and EgyptianTranslation datasets.
...
Conclusion: The proposed deep learning method successfully addresses the challenges of hieroglyph translation, demonstrating promising results for automated ancient Egyptian text translation.
...
TL;DR: A robust video-based breath rate measurement method using mathematical transforms achieves <5% relative deviation from ground truth, with 0.57 respirations per minute MAE, outperforming previous methods and being resistant to subject movement.


<details>
  <summary>Details</summary>
Motivation: Existing video-based breath rate measurement methods either work only in near-ideal conditions or lack sufficient accuracy. There's a need for a more robust method that can handle real-world conditions with minimal hardware requirements.

Method: Uses mathematical transforms on video footage to measure breath rate. Tested on videos from 14 volunteers totaling over 2.5 hours, comparing results to reference data.

Result: Achieved relative deviation from ground truth of less than 5%, with average mean absolute error of 0.57 respirations per minute, which is noticeably better than previous works. Method is resistant to distortions from subject movement.

Conclusion: The proposed method enables remote breath rate measurement without significant limitations on subject behavior, offering improved robustness and accuracy over existing approaches using only standard cameras.
...
TL;DR: LUnet is a lean Unet architecture with constant channel count across layers that achieves comparable performance to standard Unet with 30x fewer parameters, challenging the need for complex pruning methods.


<details>
  <summary>Details</summary>
Motivation: Standard Unet architectures have large memory footprints that limit batch sizes and increase inference latency. While channel pruning can compress Unets, it requires lengthy optimization and may not generalize well across tasks.

Method: Proposed LUnet with compact, flat hierarchy where channels are not doubled as resolution is halved (constant channel count across layers). Evaluated on public MRI dataset and two internal CT datasets, comparing with state-of-the-art pruning (STAMP) and conventional Unets.

Result: STAMP pruning mainly targets layers with highest channel counts. Random channel elimination at pruning-identified or largest layers achieves similar/better performance. LUnet with fixed architecture and 30x fewer parameters achieves performance comparable to conventional Unets and pruned networks.

Conclusion: The final architecture structure is more crucial than channel selection strategy in pruning. Skip connections allow Unet bottleneck channels to be largely reduced. Lean Unet with constant channel count requires far fewer parameters while achieving superior performance to standard Unet for same parameter count.
...
TL;DR: HP-Net is a heatmap pooling network for video action recognition that extracts robust, concise pooled features from human body heatmaps and integrates them with multimodal data through spatial-motion co-learning and text refinement modules.


<details>
  <summary>Details</summary>
Motivation: Existing RGB video action recognition methods suffer from information redundancy, noise susceptibility, and high storage costs. The authors aim to fully harness useful video information by extracting more efficient and robust features.
...
Result: Extensive experiments on NTU RGB+D 60, NTU RGB+D 120, Toyota-Smarthome and UAV-Human benchmarks show HP-Net outperforms existing human action recognition methods, with pooled features demonstrating advantages over previous pose data and heatmap features.

Conclusion: HP-Net effectively addresses challenges in RGB video action recognition by extracting robust pooled features and integrating multimodal data, achieving state-of-the-art performance across multiple benchmarks.
...
TL;DR: CoDA is a dataset distillation framework that uses off-the-shelf text-to-image models without target-specific training, achieving state-of-the-art performance by aligning generated samples with the target dataset's intrinsic core distribution.


<details>
  <summary>Details</summary>
Motivation: Current dataset distillation methods have two key limitations: 1) most require diffusion models pre-trained on the full target dataset (contradicting DD's purpose and being expensive), and 2) methods using general text-to-image models suffer from distributional mismatch between web-scale priors and target-specific semantics.
...
Result: CoDA achieves performance on par with or superior to previous methods that require target-specific training, establishing new state-of-the-art accuracy of 60.4% at 50-images-per-class on ImageNet-1K. It works effectively across all benchmarks including ImageNet-1K and its subsets.

Conclusion: CoDA successfully addresses fundamental limitations in dataset distillation by enabling effective DD using only off-the-shelf text-to-image models, eliminating the need for expensive target-specific training while achieving superior performance through core distribution alignment.
...
TL;DR: A publicly available prostate biopsy dataset from Iraq to address AI model generalizability gaps in digital pathology, particularly for Middle Eastern populations.


<details>
  <summary>Details</summary>
Motivation: Current AI in digital pathology lacks diverse datasets, with most representing Western populations, limiting generalizability to less digitized regions like the Middle East.

Method: Collection of 339 whole-slide images from 185 prostate biopsy patients in Erbil, Iraq, with Gleason scores and ISUP grades from three pathologists, scanned using three different scanners (Leica, Hamamatsu, Grundium).

Result: A de-identified dataset in native formats enabling grading concordance analysis, color normalization, and cross-scanner robustness evaluation for pathology AI development.

Conclusion: This dataset addresses geographic diversity gaps in pathology AI by providing Middle Eastern population data to improve model generalizability and validation across global populations.
...
TL;DR: Small 5M-parameter vision transformers benefit from targeted pre-training but suffer diminishing returns; intermediate fine-tuning can actually harm downstream performance when tasks are dissimilar.


<details>
  <summary>Details</summary>
Motivation: Transformers require large parameters and data for strong performance, but this work explores how smaller 5M-parameter ViTs can benefit from different training strategies with limited resources.

Method: Experimented with three distinct pre-training, intermediate fine-tuning, and downstream datasets/training objectives on a small 5M-parameter vision transformer to measure marginal benefits.

Result: Pre-training and fine-tuning help but have diminishing returns; intermediate fine-tuning can harm downstream performance when task mechanics are dissimilar.

Conclusion: Small ViTs benefit most from targeted pre-training and careful data selection, while indiscriminate stacking of intermediate tasks wastes compute and can degrade performance.
...
TL;DR: CaravelMetrics is an automated computational framework for analyzing cerebrovascular morphology using skeletonization-derived graph representations to extract 15 morphometric, topological, fractal, and geometric features from 3D TOF-MRA scans.


<details>
  <summary>Details</summary>
Motivation: To develop a scalable, fully automated approach for quantitative cerebrovascular feature extraction that enables normative modeling and population-level studies of vascular health and aging, addressing the need for systematic analysis of cerebrovascular organization.

Method: The framework integrates atlas-based regional parcellation, centerline extraction, and graph construction to model vessel morphology through skeletonization-derived graph representations. It computes 15 features that can be estimated globally from the complete vascular network or regionally within arterial territories for multiscale characterization.
...
Conclusion: CaravelMetrics provides a scalable, fully automated framework for quantitative cerebrovascular feature extraction that supports normative modeling and population-level studies of vascular health and aging, enabling reproducible analysis of cerebrovascular organization.
...
<details>
  <summary>Details</summary>
Motivation: Need for accurate methods to detect local regrowth early during watch-and-wait surveillance for rectal cancer patients with clinical complete response, to manage care and prevent distant metastases.
...
Result: SSDCA achieved best performance: 81.76% balanced accuracy, 90.07% sensitivity, 72.86% specificity. Showed robustness to imaging artifacts and demonstrated discriminative feature learning via UMAP clustering.

Conclusion: SSDCA effectively distinguishes clinical complete response from local regrowth in rectal cancer surveillance, providing a robust tool for early detection during watch-and-wait management.
...
TL;DR: FRESCO improves zero-shot video editing by combining intra-frame and inter-frame correspondence for better spatial-temporal consistency in diffusion-based video manipulation.


<details>
  <summary>Details</summary>
Motivation: Current zero-shot video adaptation methods using text-to-image diffusion models rely on inter-frame correspondence in attention mechanisms, which provides insufficient constraints and leads to temporal inconsistency in manipulated videos.
...
Result: FRESCO achieves high spatial-temporal consistency with input videos, significantly enhancing visual coherence. It demonstrates effectiveness in video-to-video translation and text-guided video editing, generating high-quality, coherent videos that outperform current zero-shot methods.

Conclusion: FRESCO represents a significant advance over current zero-shot video editing methods by providing better spatial-temporal constraints through combined intra-frame and inter-frame correspondence, leading to more visually coherent manipulated videos.
...
TL;DR: UniMo is an autoregressive model that jointly models 2D human videos and 3D human motions in a unified framework, enabling simultaneous generation and understanding of both modalities for the first time.


<details>
  <summary>Details</summary>
Motivation: Current methods focus on generating one modality given another as condition or integrating with other modalities like text/audio. Unifying 2D videos and 3D motions for simultaneous optimization and generation remains unexplored due to their substantial structural and distributional differences.

Method: Models videos and 3D motions as unified token sequences using separate embedding layers to mitigate distribution gaps. Features a novel 3D motion tokenizer with temporal expansion strategy using single VQ-VAE to produce quantized motion tokens with multiple expert decoders for body shapes, translation, orientation, and poses.

Result: Extensive experiments demonstrate simultaneous generation of corresponding videos and motions while performing accurate motion capture. The method proves effectiveness of unified modeling through integrated sequence modeling strategy.
...
TL;DR: A framework that enhances ground-truth images for better supervision in real-world image restoration by generating perceptually improved ground truth through adaptive frequency mask-guided super-resolution fusion.


<details>
  <summary>Details</summary>
Motivation: Real-world image restoration models are limited by imperfect ground-truth images due to practical data acquisition constraints, creating a need for higher-quality supervision.

Method: Uses a conditional frequency mask generator to create adaptive frequency masks that guide optimal fusion of frequency components from original ground truth and super-resolved variants, producing enhanced ground truth images. These are used to train a lightweight output refinement network that integrates with existing restoration models.

Result: Extensive experiments show consistent improvement in restored image quality, with user studies validating the effectiveness of both supervision enhancement and output refinement.

Conclusion: The proposed framework successfully addresses limitations of imperfect ground-truth data in real-world image restoration by generating enhanced supervision through frequency-domain mixup, improving restoration quality while maintaining semantic fidelity.
...
Method: MUT3R analyzes self-attention maps across transformer layers to identify dynamic regions that are naturally down-weighted. It applies an attention-level gating module to suppress dynamic content in early transformer layers during inference, preventing artifact propagation through the feature hierarchy without retraining or fine-tuning.

Result: The framework improves temporal consistency and camera pose robustness across multiple dynamic benchmarks, stabilizing geometric reasoning in streaming scenarios while remaining training-free.

Conclusion: Pretrained transformers contain implicit motion cues that can be leveraged for motion-aware streaming reconstruction through attention-based gating, offering a simple training-free solution to motion-induced artifacts in 3D reconstruction.
...
TL;DR: TempR1 is a temporal-aware multi-task RL framework that enhances MLLMs' temporal understanding through systematic exposure to diverse temporal tasks and optimized cross-task learning.
...
Method: TempR1 uses a multi-task corpus with diverse temporal structures, builds on GRPO algorithm, categorizes temporal tasks into three correspondence types, and designs tailored localization rewards for each type to capture fine-grained temporal dependencies.

Result: TempR1 achieves state-of-the-art performance across multiple benchmarks and shows strong synergistic effects from joint optimization over complementary tasks, enhancing both generalization and single-task performance.

Conclusion: TempR1 establishes a scalable and principled paradigm for temporal reasoning in MLLMs, demonstrating effective cross-task optimization and improved temporal comprehension.
...
TL;DR: UniID is a unified tuning-free framework that combines text embedding and adapter-based approaches for face personalization, achieving high identity fidelity and flexible text controllability through identity-focused learning and normalized rescaling.


<details>
  <summary>Details</summary>
Motivation: Existing tuning-free face personalization methods struggle to simultaneously achieve high identity fidelity and flexible text controllability, with text embedding and adapter-based approaches each having limitations when used separately.

Method: UniID integrates both text embedding and adapter-based paradigms through identity-focused learning during training and a normalized rescaling mechanism at inference, allowing mutual reinforcement of identity-relevant information while preserving diffusion prior for non-identity attributes.
...
Conclusion: UniID provides a principled unified framework that successfully combines the strengths of both text embedding and adapter-based approaches for high-fidelity face personalization with flexible text control.
...
TL;DR: DirectDrag is a mask- and prompt-free image editing framework that enables precise drag-based manipulation using only point displacements, achieving high visual fidelity without manual masks or text prompts.


<details>
  <summary>Details</summary>
Motivation: Existing drag-based image editing methods heavily rely on manually provided masks and textual prompts to preserve semantic fidelity and motion precision. Removing these constraints creates a fundamental trade-off between visual artifacts (without masks) and poor spatial control (without prompts).
...
Conclusion: DirectDrag provides a novel mask- and prompt-free editing framework that enables precise and efficient manipulation with minimal user input while maintaining high image fidelity and accurate point alignment, addressing limitations of existing drag-based image editing methods.
...
TL;DR: The paper discovers that existing feed-forward 3D reconstruction models like VGGT can inherently distinguish distractor images without explicit outlier-rejection mechanisms, and identifies a specific layer with outlier-suppressing behavior that enables effective noise-filtering for 3D reconstruction.
...
Method: Through analysis under varying proportions of synthetic distractors, the authors identify a specific layer in VGGT that naturally exhibits outlier-suppressing behavior. They probe this layer to reveal discriminative internal representations that enable noise-filtering, which they leverage for outlier-view rejection without additional fine-tuning or supervision.

Result: Extensive experiments on both controlled and in-the-wild datasets demonstrate that this implicit filtering mechanism is consistent and generalizes well across diverse scenarios.

Conclusion: Feed-forward 3D reconstruction models possess inherent capabilities to distinguish distractor images, and their internal representations can be leveraged for effective outlier-view rejection without requiring additional training or supervision.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for modeling group actions on images either operate in high-dimensional data space where transformations apply uniformly, making disentanglement difficult, or require manual partitioning of latent variables into equivariant and invariant subspaces, limiting robust learning of group actions.

Method: End-to-end framework using learnable binary masks with straight-through estimation to dynamically partition latent representations into transformation-sensitive and invariant components. Formulated within a unified optimization framework that jointly learns latent disentanglement and group transformation mappings, compatible with any standard encoder-decoder architecture.
...
Conclusion: The framework successfully learns group actions on latent image manifolds without manual intervention, automatically discovering transformation-relevant structures and enabling effective disentangled representations for various image transformations.
...
TL;DR: NVRC-Lite extends Neural Video Representation Compression with lightweight neural representations using multi-scale feature grids and octree-based entropy coding, achieving better compression than C3 with significant speed improvements.


<details>
  <summary>Details</summary>
Motivation: To develop a lightweight version of NVRC that maintains state-of-the-art performance while addressing the computational complexity and slow coding speed issues of existing INR-based video codecs, particularly the impracticality of autoregressive models for entropy coding.

Method: Two key innovations: 1) Integration of multi-scale feature grids into lightweight neural representation to improve performance at low complexity, and 2) Replacement of autoregressive models with an octree-based context model for entropy coding high-dimensional feature grids to accelerate the entropy coding module.

Result: NVRC-Lite outperforms C3 (one of the best lightweight INR-based video codecs) with up to 21.03% BD-rate savings in PSNR and 23.06% in MS-SSIM, while achieving 8.4x encoding and 2.5x decoding speedup.

Conclusion: NVRC-Lite successfully extends NVRC toward lightweight representations with improved compression efficiency and significantly faster coding speeds, making INR-based video compression more practical for real-world applications.
...
TL;DR: C3G is a feed-forward framework that generates compact 3D Gaussians at essential spatial locations for efficient scene reconstruction and understanding from unposed sparse views, addressing redundancy and feature aggregation issues in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene reconstruction from unposed sparse views using per-pixel 3D Gaussian Splatting generate excessive redundant Gaussians, causing high memory overhead and sub-optimal multi-view feature aggregation, which degrades novel view synthesis and scene understanding performance.

Method: C3G introduces learnable tokens that aggregate multi-view features through self-attention to guide Gaussian generation, ensuring each Gaussian integrates relevant visual features across views. The framework estimates compact 3D Gaussians only at essential spatial locations and exploits learned attention patterns for efficient Gaussian decoding and feature lifting.

Result: Extensive experiments on pose-free novel view synthesis, 3D open-vocabulary segmentation, and view-invariant feature aggregation demonstrate C3G's effectiveness. The approach achieves superior memory efficiency and feature fidelity compared to existing methods.

Conclusion: A compact yet geometrically meaningful representation is sufficient for high-quality scene reconstruction and understanding, with C3G showing that minimizing redundancy while enabling effective feature lifting leads to better performance in 3D computer vision tasks.
...
TL;DR: RELIC is a unified framework for interactive world modeling that achieves real-time long-horizon streaming, consistent spatial memory, and precise user control simultaneously through autoregressive video-diffusion distillation with camera-aware compressed memory tokens.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for interactive world models address only one of three key requirements in isolation: real-time long-horizon streaming, consistent spatial memory, and precise user control. Achieving all three simultaneously is challenging because long-term memory mechanisms often degrade real-time performance.

Method: RELIC uses autoregressive video-diffusion distillation with highly compressed historical latent tokens encoded with relative actions and absolute camera poses in the KV cache. It fine-tunes a bidirectional teacher video model to generate beyond its 5-second training horizon and transforms it into a causal student generator using a memory-efficient self-forcing paradigm for full-context distillation over long durations.

Result: Implemented as a 14B-parameter model trained on Unreal Engine-rendered data, RELIC achieves real-time generation at 16 FPS with more accurate action following, more stable long-horizon streaming, and more robust spatial-memory retrieval compared to prior work.

Conclusion: RELIC establishes a strong foundation for next-generation interactive world modeling by successfully unifying real-time performance, long-term memory, and user control in a single framework.
...
<details>
  <summary>Details</summary>
Motivation: VLMs struggle with precise spatial reasoning needed for embodied applications, and current approaches rely on handcrafted prompting or fixed tool pipelines that limit optimal tool-use discovery.
...
Result: SpaceTools achieves SOTA on spatial understanding benchmarks (RoboSpatial-Home, BLINK, BOP-ASK) with +12% over SFT and +16% over RL baselines, and demonstrates reliable real-world manipulation with a 7-DOF robot.

Conclusion: DIRL successfully enables VLMs to coordinate multiple tools for precise spatial reasoning, overcoming limitations of previous approaches and demonstrating practical embodied applications.
...
Method: Introduces a progressive three-stage training strategy: Perturbed Supervised Fine-Tuning, Reinforcement Learning for Visual-Reality Alignment, and Reinforcement Learning from Aesthetic Feedback. Also develops a complete workflow coupling the trained LMM with generative models for layer-controllable iterative editing.
...
TL;DR: SimFlow simplifies VAE-based normalizing flows by fixing variance to a constant, eliminating complex noise pipelines while improving reconstruction and generation quality.


<details>
  <summary>Details</summary>
Motivation: Prior VAE-based normalizing flows suffer from two main issues: 1) they require complex noise augmentation pipelines with extra noising/denoising steps, and 2) they use frozen pretrained VAE encoders leading to suboptimal reconstruction and generation quality.

Method: The key innovation is fixing the variance (normally predicted by VAE encoder) to a constant value (e.g., 0.5). This allows the encoder to output broader token distributions, enables the decoder to learn reconstruction from augmented tokens without extra noise, and simplifies the VAE evidence lower bound for stable joint training of NF with VAE.

Result: On ImageNet 256×256 generation, SimFlow achieves gFID of 2.15, outperforming state-of-the-art STARFlow (2.40). When integrated with REPA-E method, it achieves gFID of 1.91, setting new state-of-the-art among normalizing flows.

Conclusion: Fixing variance to a constant provides a simple yet effective solution to two major limitations in VAE-based normalizing flows, enabling better performance without complex noise augmentation pipelines while allowing joint training of NF and VAE.
...
TL;DR: Single-life learning trains distinct vision models on individual egocentric videos, achieving aligned geometric understanding across different lives and strong transfer performance comparable to diverse web data.


<details>
  <summary>Details</summary>
Motivation: To explore whether training vision models exclusively on egocentric videos from a single individual's life can lead to meaningful visual representations, leveraging the multiple viewpoints naturally captured within one person's experience.

Method: Train distinct visual encoders on separate egocentric video datasets each representing a different person's life (both indoor and outdoor). Use self-supervised learning on the multiple viewpoints within each life. Introduce a novel cross-attention-based metric to quantify functional alignment of internal representations across different single-life models.
...
Conclusion: The shared structure of the world leads to consistency in models trained on individual lives and provides a powerful signal for visual representation learning, establishing the viability and strength of the single-life learning paradigm.
...
TL;DR: Researchers develop a framework to monitor LLM safety by measuring "ethical entropy" dynamics, showing tuned models reduce ethical entropy by ~80% compared to base models, and create a monitoring pipeline to detect value drift.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety assessment relies on static benchmarks, but real-world failures are dynamic (value drift, jailbreak attacks, alignment degradation). Need operational framework to monitor ethical entropy changes over time.

Method: Define 5-way behavioral taxonomy, train classifier to estimate ethical entropy S(t) from model transcripts, measure entropy dynamics for base and instruction-tuned variants of four frontier models across stress tests.

Result: Base models show sustained entropy growth, while tuned variants suppress drift and reduce ethical entropy by roughly 80%. Estimate effective alignment work rate gamma_eff and embed S(t) and gamma_eff in monitoring pipeline.

Conclusion: Framework enables run-time oversight of value drift by raising alerts when entropy drift exceeds stability threshold, providing dynamic safety monitoring beyond static benchmarks.
...
TL;DR: This paper investigates vulnerabilities in EaaS watermarks against paraphrasing attacks and proposes WET, a robust watermarking technique using linear transformations to defend against imitation attacks on embedding services.


<details>
  <summary>Details</summary>
Motivation: EaaS providers need to protect their intellectual property from imitation attacks, but existing watermarking techniques are vulnerable to paraphrasing attacks that can bypass detection during model cloning.

Method: The paper first demonstrates that paraphrasing attacks can remove existing EaaS watermarks. Then proposes WET (Watermarking EaaS with Linear Transformation), which applies linear transformations to embeddings and verifies watermarks by reversing the transformation and comparing similarity between recovered and original embeddings.

Result: Paraphrasing attacks effectively bypass current state-of-the-art EaaS watermarks across various attack setups and datasets. WET demonstrates robustness against paraphrasing attacks with near-perfect verifiability, validated through detailed ablation studies.

Conclusion: Existing EaaS watermarks are vulnerable to paraphrasing attacks, but the proposed WET technique provides effective defense with strong robustness and verifiability against such attacks.
...
<details>
  <summary>Details</summary>
Motivation: LLMs exhibit choice-supportive bias (CSB) in evaluations, systematically favoring chosen options and compromising AI decision-making objectivity. Existing debiasing methods focus on demographic/social biases, leaving cognitive biases largely unaddressed.

Method: Reasoning Dependency Generation (RDG) framework automatically constructs balanced reasoning QA pairs that explicitly (un)model dependencies between choices, evidences, and justifications. Generates large-scale dataset with Contextual Dependency Data and Dependency Decouple Data for fine-tuning.
...
Conclusion: RDG pioneers an approach for addressing cognitive biases in LLMs, contributing to more reliable AI-assisted decision support systems by mitigating choice-supportive bias through data-driven fine-tuning.
...
TL;DR: This paper develops an open-source tool using language models to classify job vacancy texts by linking them to European frameworks (ESCO and EQF), comparing Sentence Linking and Entity Linking methods, and introducing annotated datasets for evaluation.
...
Method: Two main methodologies: Sentence Linking and Entity Linking. The study compares these approaches, introduces annotated datasets for evaluating occupation and qualification representation, and examines the use of generative large language models for the classification task.

Result: Development of an open-source tool incorporating both methodologies, creation of annotated datasets for evaluation, and findings that contribute to advancing job entity extraction and providing computational infrastructure for labor market analysis.

Conclusion: The research advances the state of the art in job entity extraction by linking job vacancy texts to European frameworks, provides computational tools for labor market analysis, and makes code publicly available to support further research in this domain.
...
TL;DR: InvertiTune: A framework combining controlled data generation with supervised fine-tuning to create efficient single-shot knowledge graph construction from text, outperforming larger LLMs and state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current Text2KG methods rely on iterative LLM prompting, making them computationally expensive and prone to missing complex relations distributed throughout text. There's a need for more efficient and accurate single-shot KG construction from longer texts.

Method: InvertiTune uses a controlled data generation pipeline that extracts subgraphs from large knowledge bases, applies noise filtering, and leverages LLMs to generate corresponding natural text descriptions. This creates realistic training datasets for supervised fine-tuning of lightweight models for single-shot KG construction.

Result: InvertiTune outperforms larger non-fine-tuned LLMs and state-of-the-art Text2KG approaches on the CE12k dataset. It also demonstrates stronger cross-dataset generalization on CrossEval-1200, a test set from three established benchmarks and CE12k.

Conclusion: The framework highlights the importance of realistic, high-quality training data for advancing efficient and high-performing Text2KG systems, enabling lightweight models to perform single-shot KG construction from complex text.
...
TL;DR: A framework for detecting and parsing causal explanations in political text using a lightweight causal language model that extracts cause-effect pairs for large-scale analysis.
...
Method: Developed a framework using a lightweight causal language model trained to detect and parse explanations in political text, returning structured cause-effect pairs for downstream analysis.
...
Conclusion: The framework provides a systematic approach for analyzing explanations in political text, addressing a gap in political science methodology and enabling large-scale study of causal claims in political discourse.
...
TL;DR: RMFT is a privacy-preserving fine-tuning method that reduces PII memorization in LLMs by 80%+ while maintaining minimal performance impact, outperforming deduplication methods.
...
Conclusion: RMFT effectively addresses PII memorization risks in LLMs with minimal utility impact, and the MaxTER framework provides Pareto-optimal evaluation for privacy-utility tradeoffs.
...
TL;DR: LLM-assisted pipeline analyzes bilingual discourse in Spanish-English and Spanish-Guaraní contexts, revealing systematic sociolinguistic patterns through automated annotation of 3,691 code-switched sentences.


<details>
  <summary>Details</summary>
Motivation: To develop computational methods for sociolinguistic analysis of bilingual discourse that can handle cross-linguistic and low-resource contexts, moving beyond manual annotation limitations.

Method: Created LLM-assisted annotation pipeline to automatically label topic, genre, and discourse-pragmatic functions across 3,691 code-switched sentences, integrating demographic metadata and enriching Spanish-Guaraní dataset with new topic annotations.

Result: Revealed systematic links between gender, language dominance, and discourse function in Miami Spanish-English data, and clear diglossic division between formal Guaraní and informal Spanish in Paraguayan texts, replicating earlier sociolinguistic observations with quantitative evidence.

Conclusion: LLMs can reliably recover interpretable sociolinguistic patterns traditionally requiring manual annotation, advancing computational methods for cross-linguistic and low-resource bilingual research.
...
TL;DR: PERCS dataset introduces persona-guided biomedical abstract summarization for four distinct audiences with varying medical literacy levels, enabling targeted communication and controllable summarization.


<details>
  <summary>Details</summary>
Motivation: Existing medical text simplification resources assume a single generic audience, overlooking the wide variation in medical literacy and information needs across different user groups, which limits accessibility and effectiveness.

Method: Created PERCS dataset with biomedical abstracts paired with summaries tailored to four personas (Laypersons, Premedical Students, Non-medical Researchers, Medical Experts), with physician-reviewed factual accuracy and persona alignment using detailed error taxonomy.

Result: Technical validation shows clear differences in readability, vocabulary, and content depth across personas; benchmarked four large language models using automatic evaluation metrics assessing comprehensiveness, readability, and faithfulness.

Conclusion: PERCS dataset supports research on persona-specific communication and controllable biomedical summarization, addressing the need for audience-targeted medical text simplification to improve health literacy.
...
TL;DR: Idea-Gated Transformer separates semantic planning from syntactic generation using an auxiliary "Idea Head" that predicts future bag-of-words distributions, creating concept vectors that gate vocabulary during generation to prevent topic drift.


<details>
  <summary>Details</summary>
Motivation: Autoregressive LLMs trained on Next-Token Prediction suffer from "Topic Drift" where generation wanders from initial prompts due to reliance on local associations rather than global planning. While scaling model size helps, the fundamental myopia of NTP objective remains.

Method: Introduces Idea-Gated Transformer with auxiliary "Idea Head" trained to predict bag-of-words distribution for future context window, creating latent "Concept Vector" that actively gates main vocabulary during generation. Uses differentiable gating mechanism that suppresses semantically irrelevant tokens in real-time.

Result: On WikiText-103, Idea-Gated model achieves comparable validation perplexity to standard GPT-2 baseline but exhibits significantly superior Domain Retention. Gating mechanism successfully locks generation into specific semantic clusters (Finance, Science) and resists associative drift.

Conclusion: Idea-Gated Transformer offers parameter-efficient path toward more controllable language modeling by separating semantic planning from syntactic generation, effectively preventing topic drift through real-time vocabulary gating based on predicted future semantic concepts.
...
TL;DR: HBLR is a novel framework that combines confidence-aware symbolic translation with hypothesis-driven backward reasoning to improve logical reasoning in LLMs, outperforming existing methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning approaches rely on forward reasoning paradigms that suffer from redundant inference paths, hallucinated steps, and semantic drift, leading to inefficient and unreliable logical reasoning despite its importance for AI capabilities.

Method: HBLR integrates two key components: 1) Confidence-aware symbolic translation that converts only high-confidence text spans to logical form (like FOL) while keeping uncertain content in natural language, with a translation reflection module ensuring semantic fidelity; 2) Hypothesis-driven backward reasoning that simulates human deductive thinking by assuming conclusions are true and recursively verifying premises, enhanced by a reasoning reflection module that identifies and corrects flawed inference steps.
...
Conclusion: The HBLR framework successfully addresses limitations of forward reasoning approaches by combining selective symbolic translation with backward deductive reasoning, resulting in more efficient and reliable logical reasoning for LLMs.
...
TL;DR: Higher-Order Attention Network (Hon) enhances Transformer representational power using recursive self-attention to capture multi-hop relationships, breaking the linear bottleneck with O(1) additional parameters.


<details>
  <summary>Details</summary>
Motivation: Standard first-order attention mechanisms in Transformers suffer from low-rank bottlenecks and struggle to capture intricate, multi-hop relationships within single layers, limiting their representational power.

Method: Hon uses a recursive framework where Query and Key vectors are dynamically refined through nested self-attention mechanisms before final attention computation, with parameter-efficient weight-sharing across recursive steps.

Result: Theoretical analysis shows Hon breaks the linear bottleneck of standard attention, and empirical results demonstrate it outperforms standard Transformers on multiple benchmarks.

Conclusion: Higher-order attention through recursive refinement of Query/Key representations significantly enhances Transformer capabilities while maintaining parameter efficiency.
...
TL;DR: Researchers created the Portal Dialogue Corpus - 11.5 hours of spoken dialogue from Portal 2 co-op gameplay, featuring complex linguistic phenomena rarely found in standard dialogue datasets.


<details>
  <summary>Details</summary>
Motivation: Cooperative video games provide rich language data with complex coordination, communication, and reasoning under uncertainty, but existing dialogue corpora lack these challenging linguistic phenomena.

Method: Collected 11.5 hours of spoken human dialogue from Portal 2 co-op mode, comprising 24.5K utterances, with player videos, audio, transcripts, game state data, and manual/automatic annotations.

Result: Identified unique linguistic phenomena including complex spatial reference, clarification/repair mechanisms, and ad-hoc convention formation that rarely appear in chitchat or task-oriented dialogue corpora.

Conclusion: The publicly released Portal Dialogue Corpus provides valuable resources for analyzing language use in complex, situated, collaborative problem-solving scenarios.
...
<details>
  <summary>Details</summary>
Motivation: LoRA's low-rank assumption often leads to unsatisfactory performance in parameter-efficient fine-tuning of LLMs. The authors aim to improve LoRA by incorporating inductive bias to better simulate full fine-tuning's parameter update process.

Method: Dual LoRA separates low-rank matrices into two groups: magnitude group (controls whether/how far to update parameters) and direction group (decides forward/backward movement). This is achieved by adding ReLU function to magnitude group and sign function to direction group.

Result: Experiments across NLP tasks (NLG, NLU, commonsense reasoning) on GPT-2, RoBERTa, DeBERTa, and LLaMA-1/2/3 show Dual LoRA consistently outperforms LoRA and its state-of-the-art variants with same number of trainable parameters.

Conclusion: Dual LoRA effectively improves LoRA performance by better simulating full fine-tuning's parameter update process through separated magnitude and direction control, achieving superior results across diverse NLP tasks and model architectures.
...
TL;DR: PretrainZero is a reinforcement active learning framework that extends RL from domain-specific post-training to general pretraining, enabling self-supervised learning from pretraining corpus without verifiable labels or rewards.
...
Method: PretrainZero uses: 1) Active pretraining with a unified reasoning policy to identify informative content from pretraining corpus, 2) Self-supervised learning directly from base models on Wikipedia corpus without labels or fine-tuning, 3) Verification scaling by tackling increasingly challenging masked spans.

Result: PretrainZero improves Qwen3-4B-Base by 8.43, 5.96 and 10.60 points on MMLU-Pro, SuperGPQA and math average benchmarks. The pretrained models can also serve as reasoning foundation models for downstream RLVR tasks.

Conclusion: PretrainZero successfully extends RL to general pretraining, breaking the verification data-wall for general reasoning and significantly enhancing reasoning abilities of pretrained base models through reinforcement active learning.
...
TL;DR: Top-k Attention mechanism reduces LLM inference costs while maintaining performance by focusing on most relevant context tokens, validated through decoding experiments and training strategies.


<details>
  <summary>Details</summary>
Motivation: High computational costs of LLMs in long-context modeling hinder advancement of tasks like agents and multimodal applications, requiring more efficient attention mechanisms.

Method: Investigates Top-k Attention mechanism in both decoding and training phases: exact Top-k Decoding, native Top-k Attention training strategy, approximate Top-k algorithms, and theoretical analysis from entropy perspective.

Result: Top-k Decoding achieves comparable or better performance than full attention on HELMET and LongBench v2; training-inference consistency enhances performance; downstream performance correlates with approximation fidelity; Top-k Attention SFT reduces entropy in downstream tasks.

Conclusion: Top-k Attention effectively reduces LLM inference costs while maintaining performance, with low-entropy states better adapted to Top-k Decoding, offering practical efficiency improvements for long-context applications.
...
TL;DR: Reasoning strategies in LLMs for abstractive summarization show mixed results: explicit reasoning improves fluency but reduces factual faithfulness, while implicit reasoning in Large Reasoning Models shows opposite pattern. More reasoning doesn't always help and can hurt factual consistency.


<details>
  <summary>Details</summary>
Motivation: While LLMs excel at analytical tasks like mathematics and code generation, their effectiveness for abstractive summarization is widely assumed but largely unverified. The paper aims to bridge this gap by systematically evaluating reasoning strategies for summarization.

Method: Tailored general reasoning strategies to summarization domain, then conducted systematic large-scale comparative study of 8 reasoning strategies and 3 Large Reasoning Models across 8 diverse datasets, assessing both summary quality and faithfulness.

Result: Reasoning is not a universal solution - effectiveness depends on specific strategy and context. Found trade-off between summary quality and factual faithfulness: explicit reasoning improves fluency but reduces factual grounding, while implicit reasoning in LRMs shows inverse pattern. Increasing LRM's internal reasoning budget doesn't improve and can even hurt factual consistency.

Conclusion: Effective summarization demands faithful compression rather than creative over-thinking. The study reveals nuanced relationship between reasoning strategies and summarization performance, challenging assumptions about universal benefits of reasoning for summarization tasks.
...
TL;DR: INDI-PROP: First ideologically grounded fine-grained narrative dataset for Indian propaganda analysis with multi-level annotation and GPT-4o-mini guided frameworks for bias, narrative, and persuasive technique classification.


<details>
  <summary>Details</summary>
Motivation: Narratives serve as cognitive and emotional scaffolds for propaganda, organizing persuasive techniques into coherent stories that justify actions and create ideological identification. There's a need for fine-grained narrative classification in biased news articles, particularly for Indian media covering polarizing socio-political events.

Method: Created INDI-PROP dataset with 1,266 articles on CAA and Farmers' protest, annotated at three hierarchical levels: ideological article-bias, event-specific fine-grained narrative frames, and persuasive techniques. Proposed two GPT-4o-mini guided frameworks: FANTA (multi-hop reasoning integrating information extraction and contextual framing) and TPTC (two-stage decomposition of persuasive cues).

Result: The evaluation shows substantial improvement over underlying baselines for bias, narrative, and persuasive technique classification using the proposed frameworks.

Conclusion: The paper introduces the first ideologically grounded fine-grained narrative dataset for Indian propaganda analysis and demonstrates effective GPT-4o-mini guided frameworks for hierarchical classification tasks, advancing propaganda analysis in news media.
...
TL;DR: Proposes an interpretable framework for factual consistency assessment in LLMs using atomic fact decomposition and weighted metrics to address hallucination issues, especially in clinical domains.


<details>
  <summary>Details</summary>
Motivation: LLMs generate plausible but incorrect arguments (hallucinations), which is critical in high-stakes domains like clinical applications. Existing evaluation metrics fail to adequately assess factual consistency and lack interpretability.

Method: Decomposes text into atomic facts and introduces a flexible, schema-free methodology with weighted metrics (not absolute) and a mechanism to control assessment complexity in intricate domains.

Result: Benchmarked on popular general and clinical datasets; code released to support fact-aware model training in future research.

Conclusion: The proposed interpretable framework addresses limitations of existing factual consistency assessment methods and provides better evaluation for hallucination detection, particularly in critical domains.
...
TL;DR: This study provides the first comprehensive empirical mapping of GenAI adoption, usage patterns, and literacy in Italy, revealing widespread adoption for sensitive tasks, GenAI becoming a primary information source despite low digital literacy, and a significant gender divide where women are half as likely to adopt GenAI.


<details>
  <summary>Details</summary>
Motivation: While GenAI chatbots hold societal promise, they risk widening digital divides due to uneven adoption and low awareness of limitations. There's a need to understand adoption patterns, usage behaviors, and literacy levels to address potential inequities.

Method: The study uses newly collected survey data from 1,906 Italian-speaking adults to conduct the first comprehensive empirical mapping of GenAI adoption, usage patterns, and literacy in Italy.

Result: Findings show: 1) Widespread adoption for both work and personal use, including sensitive tasks like emotional support and medical advice; 2) GenAI is supplanting other technologies to become a primary information source despite low user digital literacy; 3) Significant gender divide where women are half as likely to adopt GenAI and use it less frequently than men; 4) Literacy is a key predictor of adoption but only partially explains the gender disparity.

Conclusion: The study highlights the dual need for targeted educational initiatives and further investigation into underlying barriers to equitable participation that competence alone cannot explain, as GenAI becomes increasingly integrated into daily life while potentially exacerbating existing digital divides.
...
TL;DR: The paper introduces Hydro-SE Bench, a 4,000-question benchmark for evaluating LLMs in Hydro-Science and Engineering across nine subfields, revealing performance gaps in domain-specific knowledge despite decent scores in science-related areas.


<details>
  <summary>Details</summary>
Motivation: Hydro-SE is critical for water supply, clean energy, and disaster mitigation, requiring interdisciplinary expertise. While LLMs show potential in this domain, their knowledge and application abilities haven't been sufficiently evaluated, creating a need for a comprehensive benchmark.

Method: Created Hydro-SE Bench with 4,000 multiple-choice questions covering nine subfields, evaluating LLMs across three dimensions: basic conceptual knowledge, engineering application ability, and reasoning/calculation ability.

Result: Commercial LLMs scored 0.74-0.80 accuracy, while small-parameter LLMs scored 0.41-0.68. Models performed well in natural/physical science subfields but struggled with domain-specific knowledge like industry standards and hydraulic structures. Model scaling mainly improved reasoning/calculation abilities.

Conclusion: LLMs show promise but have significant gaps in practical engineering applications. The benchmark provides clear training targets for developers and practical guidance for Hydro-SE researchers, highlighting areas needing improvement for real-world deployment.
...
TL;DR: LLMs represent syntactic agreement as a meaningful functional category with overlapping neural units across different agreement types and languages, revealing systematic syntactic organization in their internal representations.


<details>
  <summary>Details</summary>
Motivation: While LLMs can reliably distinguish grammatical from ungrammatical sentences, it remains unclear how grammatical knowledge is represented within these models and whether different syntactic phenomena recruit shared or distinct components.

Method: Used functional localization approach inspired by cognitive neuroscience to identify LLM units most responsive to 67 English syntactic phenomena across seven open-weight models. Analyzed cross-lingual patterns in English, Russian, Chinese, and conducted broader analysis across 57 diverse languages.

Result: Different types of syntactic agreement (subject-verb, anaphor, determiner-noun) recruit overlapping sets of units, suggesting agreement constitutes a meaningful functional category. This pattern holds across languages, and structurally similar languages share more units for subject-verb agreement.

Conclusion: Syntactic agreement—a critical marker of syntactic dependencies—constitutes a meaningful category within LLMs' representational spaces, revealing systematic organization of syntactic knowledge in language models.
...
TL;DR: AITutor-EvalKit is a language technology application for evaluating AI tutors' pedagogical quality, providing demonstration software, model inspection, and data visualization tools for education stakeholders and the ACL community.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive tool for assessing the pedagogical quality of AI tutors, supporting learning, and enabling collection of user feedback and annotations for the education and ACL communities.

Method: Develops an application using language technology that includes software for demonstration and evaluation, model inspection capabilities, and data visualization tools.

Result: Creates AITutor-EvalKit - a practical tool that enables evaluation of AI tutor quality, supports learning, and facilitates user feedback collection for education stakeholders and the ACL community.

Conclusion: AITutor-EvalKit serves as a valuable resource for evaluating AI tutor pedagogical quality, with applications for both educational assessment and research community needs.
...
<details>
  <summary>Details</summary>
Motivation: Long-context dialogue systems suffer from State Inertia, where static constraints prevent models from resolving conflicts between evolving user intents and established historical context, limiting their ability to adapt to changing conversation dynamics.

Method: Proposes DZ-TDPO, a non-destructive alignment framework that synergizes conflict-aware dynamic KL constraints with a learnable temporal attention bias to regulate attention without destructive weight updates.

Result: Achieves state-of-the-art win rates (86.2% on Phi-3.5) on Multi-Session Chat dataset with robust zero-shot generalization. Larger Qwen2.5-7B model achieves near-perfect alignment (99.4% win rate) with negligible perplexity overhead.

Conclusion: TAI (Temporal Attention Inertia) can be alleviated via precise attention regulation rather than destructive weight updates, preserving general capabilities across model scales. Reveals a "Capacity-Stability Trade-off" where larger models achieve better alignment with minimal performance overhead.
...
TL;DR: ESPO is a new RL framework for diffusion LLMs that treats entire sequence generation as a single action using ELBO as a likelihood proxy, outperforming token-level methods by 20-40 points on reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: RL works well for autoregressive LLMs but fails for diffusion LLMs due to fundamental mismatch - diffusion models generate through iterative denoising without token-level conditional probabilities needed for token-level RL objectives like GRPO.

Method: Proposes ELBO-based Sequence-level Policy Optimization (ESPO) that treats sequence generation as single action, uses ELBO as tractable sequence-level likelihood proxy, with per-token normalization of importance ratios and robust KL-divergence estimation for stable training.

Result: ESPO significantly outperforms token-level baselines, achieving 20-40 point improvements on Countdown task, with consistent gains on math and coding benchmarks across mathematical reasoning, coding, and planning tasks.

Conclusion: ESPO establishes sequence-level optimization as principled and effective paradigm for RL in diffusion LLMs, addressing fundamental likelihood approximation challenge that prevented RL adaptation to diffusion models.
...
TL;DR: Doublespeak is an in-context representation hijacking attack that replaces harmful keywords with benign tokens in examples, causing LLMs to interpret innocuous prompts as harmful instructions, bypassing safety alignment.


<details>
  <summary>Details</summary>
Motivation: To demonstrate vulnerabilities in current LLM safety alignment by showing that semantic meaning can be manipulated at the representation level through simple in-context examples, revealing insufficient protection mechanisms.

Method: Systematically replace harmful keywords with benign tokens across multiple in-context examples preceding a harmful request, causing internal representations of benign tokens to converge toward harmful ones through semantic overwrite across model layers.

Result: Achieves 74% attack success rate on Llama-3.3-70B-Instruct with single-sentence context override; broadly transferable across model families; optimization-free; uses interpretability tools to show layer-by-layer semantic convergence.

Conclusion: Reveals new attack surface in LLM latent space; current alignment strategies are insufficient and should operate at representation level rather than just surface text; highlights need for representation-level safety mechanisms.
...
TL;DR: Adapting DoLa contrastive decoding to T5/FLAN-T5 encoder-decoder models to evaluate instruction following capabilities, showing mixed results across task categories.


<details>
  <summary>Details</summary>
Motivation: Contrastive decoding methods like DoLa have only been implemented in decoder-only architectures and studied for factuality improvement, but their impact on encoder-decoder models and instruction following capabilities remains unexplored.

Method: Adapt DoLa (Decoding by Contrastive Layers) for T5 and FLAN-T5 model families, evaluate impact on instruction following, and conduct layer-by-layer analysis of logit evolution to quantify DoLa's effect on token output probabilities.

Result: DoLa improves faithfulness of text generation for certain categories of tasks but harms others, with mixed results across different instruction types.

Conclusion: First implementation of contrastive decoding in encoder-decoder architecture reveals task-dependent effects on instruction following, with layer analysis providing insights into DoLa's impact on token probabilities.
...
<details>
  <summary>Details</summary>
Motivation: LLMs show strong text classification performance but output depends heavily on prompt wording. There's limited research on prompt engineering for classification tasks in domains like psychology where constructs have precise, theory-driven definitions that may not be well represented in pre-training data.

Method: Empirical framework for optimizing LLM performance via prompt engineering. Evaluated five strategies: codebook-guided empirical prompt selection, automatic prompt engineering, persona prompting, chain-of-thought reasoning, and explanatory prompting with zero-shot and few-shot classification across three constructs and two models.

Result: Persona, chain-of-thought, and explanations don't fully address performance loss from badly worded prompts. Most influential prompt features are construct definition, task framing, and examples. Best classifications aligned with expert judgments came from few-shot prompt combining codebook-guided empirical prompt selection with automatic prompt engineering.

Conclusion: Researchers should generate and evaluate many prompt variants (human-crafted and automatically generated), select based on empirical performance in training data, and validate in holdout sets. This provides practical, systematic, theory-driven method for optimizing LLM prompts when alignment with expert judgment is critical.
...
TL;DR: Fine-tuning small LLMs on verbalized medical consensus rules enables accurate early prediction with faithful explanations, outperforming larger LLMs and addressing the challenge of forecasting irregular clinical time series.


<details>
  <summary>Details</summary>
Motivation: Current ML models for early medical prediction focus too much on accuracy while neglecting faithful explanations needed for clinical trust. Medical consensus guidelines provide structured reasoning frameworks that can be taught to LLMs.

Method: Fine-tune small LLMs on verbalized instantiations of medical consensus rules (using Sepsis-3 as example). Evaluate using derivation correctness (faithful reasoning) and value correctness (prediction accuracy). Address time series forecasting challenge with multimodal integration of time series model outputs.

Result: Fine-tuned small models outperform larger LLMs using one-shot learning and models trained on medical texts. Achieve near-perfect derivation correctness for rules/exceptions on unseen patient data. Time series forecasting integration improves prediction of irregular clinical variables.

Conclusion: Teaching LLMs medical consensus rules enables accurate early prediction with faithful explanations. The main bottleneck is not out-of-distribution generalization but forecasting irregular clinical time series, which can be improved with multimodal approaches.
...
TL;DR: FusedKV reduces KV cache memory by 50% through learnable fusion of bottom and middle layer KV caches for top layers, outperforming standard Transformer decoders.


<details>
  <summary>Details</summary>
Motivation: Transformer decoders face prohibitive memory requirements for KV cache at long sequences. Existing cross-layer sharing methods underperform compared to within-layer methods like GQA, creating a need for more efficient KV cache approaches.

Method: Analyzed information flow of keys/values in top layers, finding values come from bottom layers while keys use both bottom and middle layers. Proposed FusedKV with learnable fusion of most informative KV caches from bottom and middle layers, operating on post-RoPE keys to preserve positional info. Also created FusedKV-Lite as cross-layer sharing variant using bottom-layer values and middle-layer keys.

Result: Achieved 50% reduction in cache memory while obtaining lower validation perplexity than standard Transformer decoders across LLMs from 332M to 4B parameters. FusedKV-Lite reduces I/O overhead with slight perplexity increase.
...
TL;DR: Models trained on diverse linguistic data (standard + non-standard) outperform standard-only models across all tasks without compromising benchmark accuracy, highlighting the importance of linguistic diversity for inclusive, generalizable language models.


<details>
  <summary>Details</summary>
Motivation: Standard text filtering for language models often excludes non-standard linguistic varieties (dialectal, historical, informal), reducing model robustness and reinforcing representational biases. The paper argues that language models should capture the full spectrum of language variation rather than relying solely on standardized text.

Method: Focusing on Basque (morphologically rich, low-resource language), the authors construct new corpora combining standard, social media, and historical sources. They pre-train the BERnaT family of encoder-only models in three configurations: standard, diverse, and combined. They also propose an evaluation framework that separates NLU tasks into standard and diverse subsets to assess linguistic generalization.

Result: Models trained on both standard and diverse data consistently outperform those trained on standard corpora alone, improving performance across all task types without compromising standard benchmark accuracy.
...
TL;DR: The paper introduces BRAND, a bilingual dataset for evaluating religious bias in multilingual language models, focusing on four South Asian religions, revealing persistent biases particularly against Islam.


<details>
  <summary>Details</summary>
Motivation: Despite advances in large language models, religious contexts remain challenging for bias detection due to potential severe misunderstandings from minor errors. Multilingual models often misrepresent religions and struggle with accuracy in religious contexts, especially for sensitive subjects like religion.

Method: Created BRAND (Bilingual Religious Accountable Norm Dataset) containing over 2,400 entries focusing on Buddhism, Christianity, Hinduism, and Islam. Used three different prompt types in both English and Bengali to evaluate model performance and bias.

Result: Models perform better in English than Bengali and consistently display bias toward Islam, even when answering religion-neutral questions. Findings reveal persistent bias in multilingual models when similar questions are asked in different languages.

Conclusion: The research highlights persistent religious bias in multilingual language models, particularly against Islam, and connects these findings to broader HCI issues regarding religion and spirituality, emphasizing the need for improved bias detection in sensitive religious contexts.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of adapting large language models to low-resource languages like Tibetan, which suffers from data scarcity and cross-lingual drift issues, particularly for morphologically rich and underrepresented languages.

Method: Two-stage approach: 1) Continual Pretraining (CPT) to establish Tibetan linguistic grounding, 2) Supervised Fine-Tuning (SFT) for task and translation specialization. Layer-wise analysis across 435 layers in Qwen3-4B was conducted to understand adaptation dynamics.

Result: Significant improvements: perplexity decreased from 2.98 to 1.54; Chinese→Tibetan translation quality improved substantially (BLEU: 0.046 → 0.261; chrF: 2.2 → 6.6). Adaptation primarily concentrated on embedding and output heads, with mid-late MLP projections encoding domain-specific transformations.

Conclusion: CPT constructs a Tibetan semantic manifold while SFT sharpens task alignment with minimal representational disruption. This study provides the first quantitative exploration of Tibetan adaptation dynamics for LLMs and offers an open, reproducible framework for extending multilingual foundation models to low-resource settings.
...
TL;DR: Tokenizer adaptation via continued BPE training for vocabulary extension and leaf-based pruning for vocabulary reduction improves efficiency and utilization while preserving model quality.


<details>
  <summary>Details</summary>
Motivation: Current tokenizer adaptation methods for transferring pre-trained language models to new domains/languages often result in inefficient vocabulary extension with many unused tokens and lack controlled vocabulary reduction methods.

Method: Proposes two complementary methods: 1) Continued BPE training that adapts pre-trained tokenizers by continuing the BPE merge learning process on new data, and 2) Leaf-based vocabulary pruning that removes redundant tokens while preserving model quality.

Result: Experiments across multiple languages and model families show improved tokenization efficiency, better utilization of added vocabulary, and effective vocabulary reduction without compromising model quality.

Conclusion: The proposed methods provide practical tools for controlled vocabulary modification, addressing both extension and pruning aspects of tokenizer adaptation, released as an open-source package.
...
TL;DR: AugServe is an efficient inference framework that improves effective throughput and reduces latency for augmented LLM services through adaptive request scheduling and dynamic token batching.


<details>
  <summary>Details</summary>
Motivation: Current augmented LLM inference systems suffer from FCFS scheduling causing head-of-line blocking and static batch token limits that can't adapt to fluctuating loads, degrading effective throughput and service quality.

Method: Two-stage adaptive request scheduling strategy: stage I optimizes scheduling order using inference features, stage II refines decisions with runtime information. Also includes dynamic token batching based on hardware status and real-time load.

Result: Achieves 4.7-33.1x and 3.3-13.2x higher effective throughput than vLLM and InferCept, while reducing time-to-first-token by up to 96.3% and 95.0% respectively.

Conclusion: AugServe significantly improves inference serving efficiency for augmented LLMs by addressing scheduling bottlenecks and adapting to dynamic conditions, enhancing user experience through better SLO compliance.
...
TL;DR: Jina-VLM is a 2.4B parameter vision-language model achieving SOTA multilingual VQA performance among open 2B-scale models, using SigLIP2 vision encoder, Qwen3 language backbone, and attention-pooling connector for token-efficient arbitrary-resolution image processing.


<details>
  <summary>Details</summary>
Motivation: To create an efficient yet powerful vision-language model that excels at multilingual visual question answering while maintaining competitive text-only capabilities, addressing the need for token-efficient processing of arbitrary-resolution images.

Method: Combines SigLIP2 vision encoder with Qwen3 language backbone through an attention-pooling connector, enabling token-efficient processing of arbitrary-resolution images while maintaining model efficiency at 2.4B parameters.
...
Conclusion: Jina-VLM demonstrates that efficient 2B-scale vision-language models can achieve top-tier multilingual VQA performance through careful architecture design combining modern vision encoders, language backbones, and token-efficient connectors.
...
<details>
  <summary>Details</summary>
Motivation: To enable models to leverage cognitive skills (like verification, backtracking, retrying) that aren't exhibited by base models, without relying on distillation from stronger models.

Method: Uses samples from the model itself, rearranged to provide training data in the format of cognitive skills during supervised fine-tuning (SFT) stage prior to RL, creating "silver" SFT traces that prime the model for skill acquisition during RL.

Result: SkillFactory SFT initialization helps models generalize to harder task variants post-RL, cognitive skills are effectively used by models, and RLed SkillFactory models are more robust to regression on out-of-domain tasks than RLed base models.

Conclusion: Inductive biases learned prior to RL help models learn robust cognitive skill use, and SkillFactory's approach of using model-generated "silver" traces is effective for skill acquisition.
...
<details>
  <summary>Details</summary>
Motivation: The paper aims to address fundamental problems in traditional AI alignment approaches that attempt to encode fixed human values, which face structural instability due to the is-ought gap, value pluralism, and the extended frame problem.

Method: The paper proposes a philosophical framework with three components: 1) articulating the "specification trap" argument against content-based value specification, 2) introducing "syntropy" as an information-theoretic framework for multi-agent alignment dynamics, and 3) establishing a functional distinction between genuine and simulated moral capacity based on compatibilist theories of guidance control.

Result: The paper develops a philosophical framework that generates specific, falsifiable predictions about value emergence and moral agency in artificial systems, though empirical validation remains pending and is being developed in a separate project.

Conclusion: AI alignment should be reconceived as architecting syntropic, reasons-responsive agents through process-based, multi-agent, developmental mechanisms rather than attempting to encode fixed human value content, representing a paradigm shift in alignment research.
...
TL;DR: Weight-Calculatism is a novel cognitive architecture for AGI based on logical atoms and weight calculations, offering radical explainability and value alignment.


<details>
  <summary>Details</summary>
Motivation: Current AI systems lack explainability and value alignment, creating fundamental challenges for trustworthy AGI development.

Method: Deconstructs cognition into indivisible Logical Atoms with two operations (Pointing and Comparison), formalizes decision-making via Weight = Benefit * Probability model, implemented with graph algorithms and global workspace workflow.

Result: Achieves transparent, human-like reasoning and robust learning in unprecedented scenarios, demonstrating practical AGI foundation.

Conclusion: Weight-Calculatism provides a viable pathway toward trustworthy, aligned AGI with radical explainability and traceable value alignment.
...
TL;DR: Symbolic-solver-integrated approach enhances LLMs on constraint satisfaction problems with ample search space, while conventional long CoTs work better for deductive problems with shallow reasoning depth.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models (LRMs) using long Chains of Thought (CoTs) incur substantial token overhead and can "overthink" leading to incorrect answers. The paper explores when symbolic-solver-integrated approaches can enhance conventional CoT methods.

Method: Symbolic-solver-integrated approach leverages LLMs' code generation capabilities to translate reasoning tasks into executable code, then solves them with symbolic solvers. Compares this with conventional long CoT approaches.

Result: Symbolic-solver-integrated method only helps when problems require limited implicit reasoning but involve ample search space. Latest LLMs (GPT-4o) perform better on deductive problems with shallow reasoning depth, while symbolic-solver integration significantly improves performance on constraint satisfaction problems requiring repeated backtracks. With declarative exemplars, even CodeLlama-13B can outperform GPT-4o on difficult Zebra puzzles.

Conclusion: The choice between conventional CoT and symbolic-solver-integrated approaches depends on problem characteristics: CoT works better for deductive reasoning with shallow depth, while symbolic solvers excel at constraint satisfaction problems with large search spaces.
...
TL;DR: Active inference agents with different preference distributions (hard/soft goals with/without goal shaping) are compared in grid world navigation, showing goal shaping improves performance but reduces exploration of environment dynamics.
...
Method: Four preference distribution definitions are considered: hard vs soft goals, with vs without goal shaping (intermediate goals). Four agents with these different preference distributions are compared in a grid world navigation task.
...
Conclusion: The specification of preference distributions significantly impacts agent performance in active inference, with goal shaping providing a trade-off between exploitation and exploration that favors exploitation at the cost of reduced environment learning.
...
TL;DR: The paper introduces a method for evaluating LLM agents' zero-shot cooperative intelligence in mixed-motive social environments using the Concordia simulation platform, revealing significant gaps in current agents' ability to generalize to novel cooperative scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods fail to measure how well LLM agents' social interaction capabilities generalize to novel social situations, particularly in mixed-motive environments where cooperation is crucial for real-world deployment.

Method: Uses Concordia, a natural language multi-agent simulation environment, to evaluate agents' ability to cooperate in zero-shot, mixed-motive scenarios. The method tests agents' capacity to identify and exploit opportunities for mutual gain across diverse partners and contexts, with evaluation conducted through the NeurIPS 2024 Concordia Contest across scenarios ranging from negotiation to collective action problems.

Result: Empirical results reveal significant gaps between current agent capabilities and the robust generalization required for reliable cooperation, particularly in scenarios demanding persuasion and norm enforcement.

Conclusion: Current LLM agents lack the robust generalization capabilities needed for reliable cooperation in novel social situations, highlighting the need for improved evaluation methods and agent development focused on zero-shot cooperative intelligence in mixed-motive environments.
...
TL;DR: Argos is an agentic reward system that selects optimal scoring functions to evaluate multimodal reasoning models across final accuracy, spatiotemporal localization, and reasoning quality, achieving SOTA results while reducing reward hacking.


<details>
  <summary>Details</summary>
Motivation: Current multimodal reinforcement learning (MMRL) models use sparse, outcome-based rewards, which provide limited guidance. Richer rewards from reasoning tokens could improve learning, but it's challenging to compute informative rewards due to varying scoring needs across samples and noisy teacher signals.

Method: Argos selects from a pool of teacher-model derived and rule-based scoring functions to simultaneously evaluate: (1) final response accuracy, (2) spatiotemporal localization of referred entities and actions, and (3) reasoning process quality. It's used across both supervised fine-tuning data curation and RL training.

Result: Achieves state-of-the-art results across multiple agentic tasks including spatial reasoning, visual hallucination, robotics, and embodied AI benchmarks. Demonstrates that SFT alone is insufficient (agents collapse to ungrounded solutions) and that Argos reduces reward-hacking in MMRL.

Conclusion: Argos provides a principled approach to agentic reward scoring that improves multimodal reasoning model training. Theoretical justification is provided through pareto-optimality concepts. The agentic verifier is critical for maintaining grounded solutions during RL training.
...
TL;DR: A communication-constrained multi-agent reinforcement learning framework that addresses lossy communication in real-world scenarios by distinguishing between lossy/lossless messages and quantifying their impact on global rewards.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent reinforcement learning with communication struggles with lossy communication in real-world scenarios due to limited scalability and robustness, making them unsuitable for complex dynamic environments.

Method: Proposes a generalized communication-constrained model to characterize communication conditions, uses it as a learning prior to distinguish lossy/lossless messages, decouples their impact on decision-making using dual mutual information estimator, and introduces a framework quantifying communication impact into global reward.
...
Conclusion: The proposed communication-constrained multi-agent reinforcement learning framework effectively addresses lossy communication challenges in real-world multi-agent systems, improving learning of cooperative policies in complex dynamic environments.
...
Method: Hierarchical multi-agent architecture with task planning, execution, and self-assessment/feedback mechanisms that evaluate actions and outcomes from independent context, enabling detection and correction of strategic errors.

Result: Successfully reproduced key materials science results on lithium-ion conduction and alloy segregation, coordinating dozens of parallel simulations (43 hours each). In Kaggle tasks, produced solutions competitive with human-engineered baselines from minimal natural-language instructions.

Conclusion: PARC demonstrates the potential of hierarchical multi-agent systems with self-assessment/feedback to enable AI systems capable of independent, large-scale scientific and analytical work without human intervention.
...
TL;DR: RP-ReAct is a multi-agent framework that decouples strategic planning from execution to improve reliability and efficiency in complex enterprise tasks, addressing issues with single-agent architectures and context window limitations.


<details>
  <summary>Details</summary>
Motivation: Autonomous agents struggle with complex enterprise tasks requiring coordination of multiple tools and diverse data sources due to: 1) single-agent architectures causing trajectory instability, and 2) local open-weight models having smaller context windows that get rapidly consumed by large tool outputs.

Method: RP-ReAct introduces a multi-agent approach with a Reasoner Planner Agent (RPA) for strategic planning using Large Reasoning Models, and Proxy-Execution Agents (PEA) that translate sub-steps into tool interactions using ReAct. Includes context-saving strategy to manage large tool outputs via external storage and on-demand access.

Result: RP-ReAct achieves superior performance and improved generalization on the ToolQA benchmark using six open-weight reasoning models, outperforming state-of-the-art baselines across diverse complex tasks and domains, with enhanced robustness and stability across different model scales.

Conclusion: The RP-ReAct framework provides an effective and deployable agentic solution for enterprises by fundamentally decoupling planning from execution, addressing key limitations of current autonomous agents in complex enterprise domains.
...
TL;DR: PAN programming model disentangles agent workflow logic from inference-time strategies, enabling easier experimentation and improved reliability with minimal code changes.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based agent programming approaches often entangle core workflow logic with inference-time strategies (like tree search), making it difficult to experiment with different strategies and improve agent reliability.

Method: Introduces "probabilistic angelic nondeterminism" (PAN) programming model that separates workflow description from inference strategies. Implemented as EnCompass framework in Python using decorators to compile agent workflows into search spaces.

Result: Three case studies demonstrate the framework allows programmers to quickly improve agent reliability and easily switch between different inference-time strategies with minimal additional coding.

Conclusion: PAN provides a practical approach to agent programming that disentangles workflow logic from inference strategies, enabling more flexible experimentation and reliability improvements in LLM-based agents.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses systematic misalignment between existing theoretical models and real-world economic complexities in retail optimization, identifying three critical gaps: data modality mismatch with unstructured textual sources, dynamic feature entanglement in modeling nonlinear price elasticity, and operational infeasibility from multi-tier business constraints.

Method: The framework introduces a tri-level architecture: 1) Hybrid knowledge fusion engine using LLMs for deep semantic parsing of unstructured text into structured features, 2) Game-theoretic constrained optimization mechanism with bilateral utility functions for supply chain reconciliation, and 3) Interpretable decision distillation interface using LLM-guided symbolic regression with economic priors as hard constraints.
...
Conclusion: DeepRule establishes a closed-loop pipeline that unifies unstructured knowledge injection, multi-agent optimization, and interpretable strategy synthesis for practical economic intelligence in retail assortment and pricing optimization.
...
TL;DR: MemVerse is a plug-and-play memory framework that combines fast parametric recall with hierarchical retrieval-based memory to enable scalable multimodal AI agents with reliable memory capabilities.


<details>
  <summary>Details</summary>
Motivation: Current AI agents lack reliable memory, leading to catastrophic forgetting, poor long-horizon reasoning, and incoherent operation in multimodal/interactive environments.

Method: Model-agnostic framework with short-term memory for recent context and hierarchical knowledge graphs for long-term memory. Includes periodic distillation to compress essential knowledge into parametric models for fast recall.
...
Conclusion: MemVerse bridges memory limitations in AI agents, providing scalable, adaptive multimodal intelligence through hierarchical memory organization and knowledge distillation.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based automatic heuristic design research often considers only a single role, limiting diversity and quality. There's a need for a more sophisticated approach that leverages multi-role collaboration to enhance heuristic generation.

Method: RoCo coordinates four specialized LLM-guided agents: explorer (creative diversity), exploiter (efficiency refinements), critic (evaluation and feedback), and integrator (synthesis and balance). They interact in a structured multi-round process with feedback, refinement, and elite mutations guided by short-term and long-term reflections.
...
TL;DR: Omni-AutoThink: An adaptive reasoning framework for Omni models that dynamically adjusts reasoning depth based on task difficulty, improving performance across multimodal tasks.


<details>
  <summary>Details</summary>
Motivation: Existing Omni models exhibit rigid reasoning behaviors - either overthinking simple problems or failing to reason when necessary. There's a need for adaptive reasoning that adjusts to task complexity.

Method: Two-stage framework: (1) Adaptive Supervised Fine-Tuning with reasoning-augmented data, and (2) Adaptive Reinforcement Learning (GRPO) that optimizes reasoning based on task complexity and rewards. Also includes a comprehensive multimodal reasoning benchmark.

Result: Experimental results show significant improvement in adaptive reasoning performance compared to previous baselines across text-only, text-audio, text-visual, and text-audio-visual modalities.

Conclusion: Omni-AutoThink successfully addresses rigid reasoning limitations in Omni models through adaptive reasoning, with benchmark data and code to be publicly released for community use.
...
TL;DR: Static-DRA is a configurable hierarchical tree-based agent for deep research tasks with tunable Depth and Breadth parameters to balance research quality against computational costs.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of static RAG pipelines in handling complex multi-turn research tasks by creating a controllable deep research agent that allows users to balance research comprehensiveness with computational efficiency.

Method: Hierarchical tree-based static workflow with Supervisor, Independent, and Worker agents, featuring user-tunable Depth and Breadth parameters for granular control over research intensity and parallel sub-topic investigation.

Result: Achieved overall score of 34.72 on DeepResearch Bench using RACE framework with depth=2, breadth=5, and gemini-2.5-pro model; experiments show increasing Depth and Breadth parameters leads to more in-depth research and higher evaluation scores.

Conclusion: Static-DRA provides a pragmatic, resource-aware solution with transparent user control over deep research processes, offering a balance between research quality and computational costs.
...
TL;DR: A logic programming framework for autonomous agents that reasons about policy compliance with penalty-based reasoning, allowing deviation when necessary while minimizing negative consequences.


<details>
  <summary>Details</summary>
Motivation: Prior work focused only on ensuring compliance, but real-world scenarios sometimes require deviation from policies to achieve high-stakes goals. Also, modeling non-compliant behavior helps policymakers simulate realistic human decision-making.

Method: Extends Gelfond and Lobo's Authorization and Obligation Policy Language (AOPL) to incorporate penalties, integrates Answer Set Programming (ASP) for reasoning, develops automated translation from extended AOPL to ASP, and refines ASP-based planning algorithms to account for incurred penalties.

Result: Experiments in two domains show the framework generates higher-quality plans that avoid harmful actions while sometimes improving computational efficiency. It distinguishes between non-compliant plans by prioritizing those with minimal penalties.

Conclusion: The framework enhances autonomous decision-making and informs policy refinement by providing penalty-based reasoning, ensuring well-formed policies, accounting for policy priorities, and improving explainability through explicit identification of rule violations and consequences.
...
TL;DR: A benchmark with executable simulation environment for evaluating LLM-based agents in industrial automation tasks, specifically Blocksworld problem with five complexity levels, using Model Context Protocol for standardized tool interface.
...
Method: Introduces benchmark with executable simulation environment for Blocksworld problem with five complexity categories. Uses Model Context Protocol (MCP) as standardized tool interface to connect diverse agent architectures without implementation-specific modifications.

Result: Demonstrates benchmark's applicability through single-agent implementation, establishing quantitative metrics for comparing LLM-based planning and execution approaches.

Conclusion: Provides a standardized benchmark for systematic evaluation of LLM-based agents in industrial automation planning tasks, enabling fair comparison across different agent architectures.
...
TL;DR: LATTICE is a new 3D asset generation framework that uses VoxSet representation and a two-stage pipeline to bridge the quality gap between 2D and 3D generative models, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: 3D generation is fundamentally more challenging than 2D due to the need to predict both spatial structure and detailed geometric surfaces from scratch. Existing 3D representations have computational complexity issues and lack structured, scalable encoding schemes, creating a quality and scalability gap between 3D and 2D generative models.

Method: Proposes VoxSet, a semi-structured representation that compresses 3D assets into latent vectors anchored to a coarse voxel grid. LATTICE uses a two-stage pipeline: first generating a sparse voxelized geometry anchor, then producing detailed geometry using a rectified flow transformer.

Result: Achieves state-of-the-art performance on various aspects, supports arbitrary resolution decoding, low-cost training, and flexible inference schemes. The method enables efficient and position-aware generation with strong token-level test-time scaling.

Conclusion: LATTICE offers a significant step toward scalable, high-quality 3D asset creation by bridging the quality and scalability gap between 3D and 2D generative models through its simple yet effective VoxSet representation and two-stage generation pipeline.
